package com.fiberhome.filink.onenetserver.receiver;
...
public class FiLinkOneNetResponseResolver implements ResponseResolver {
    ...
    public AbstractResOutputParams resolveRes(AbstractResInputParams inputParams) {
        //将参数转换成filink实体
        FiLinkOneNetInputParams resInputParams;
        FiLinkProtocolBean fiLinkProtocolBean;
        try {
            resInputParams = (FiLinkOneNetInputParams) inputParams;
        ...
        //参数信息
        FiLinkOneNetOutputParams resParams = new FiLinkOneNetOutputParams();
        //帧头
    ...
    private Map<String, Object> resolveData(ByteBuf dataBuf, FiLinkCommonHeader commonHeader,
                                            FiLinkProtocolBean fiLinkProtocolBean, FiLinkOneNetOutputParams resOutputParams) {
        ...
        if (dataParams == null || dataParams.size() == 0) {
            log.info("{} : dataParams is null", data.getCmdId());
    ...
    private void resolveDataParam(DataParam dataParam, ByteBuf dataBuf, Map<String, Object> dataResponseMap) {
        ...
        if (length == null) {
            ...
            if (StringUtils.isEmpty(referenceId)) {
                log.info("{} : reference is error", dataParam.getName());
                return;
            }
