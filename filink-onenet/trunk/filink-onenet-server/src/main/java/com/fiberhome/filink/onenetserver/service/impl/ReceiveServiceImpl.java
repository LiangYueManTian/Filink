...
public class ReceiveServiceImpl implements ReceiveService {
        ...
        try {
            if (obj != null){
                ...
                if (dataRight){
                    ...
                    //数据的信息摘要错误
                    log.warn("[onenet push]: signature error ");
                    msgBody = null;
                }
                if (msgBody == null) {
                    //数据的信息为空
                    log.warn("[onenet push]: msgBody is null ");
                    return OK;
                }
                ...
                log.info("[onenet push]: receive data {}", hexData);
                deviceMsg.setHexData(hexData);
                kafkaSender.sendMsg(deviceMsg);
            }else {
                //数据推送请求为空
                log.warn("[onenet push]: body empty error");
            }
        } catch (Exception e) {
            log.error("[onenet push]: receive error", e);
    ...
    public String check(String msg, String nonce, String signature) {
        ...
        if (AnalysisUtil.checkToken(msg,nonce,signature, token)){
            log.info("[onenet push first check]: check success, msg {}" , msg);
            return msg;
        } else {
            //验证失败
