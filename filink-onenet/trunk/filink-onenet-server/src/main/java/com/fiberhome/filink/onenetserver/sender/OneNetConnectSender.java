...
@Slf4j
@Component
public class OneNetConnectSender extends AbstractInstructSender {
    ...
    protected void send(AbstractReqParams abstractReqParams, String data) {
        if (abstractReqParams instanceof FiLinkOneNetConnectParams) {
            FiLinkOneNetConnectParams oneNetConnectParams = (FiLinkOneNetConnectParams) abstractReqParams;
            sendUtil.sendData(oneNetConnectParams, data);
        } else {
    ...
    protected String getReqData(AbstractReqParams abstractReqParams) {
        ...
        try {
            fiLinkOneNetConnectParams = (FiLinkOneNetConnectParams) abstractReqParams;
        } catch (Exception e) {
            throw new RequestException("filink request params parse exception");
        }
        String equipmentId = fiLinkOneNetConnectParams.getEquipmentId();
        FiLinkProtocolBean protocolBean = protocolUtil.getProtocolBean(fiLinkOneNetConnectParams);
        ...
        //生成流水号
        Integer serialNum = fiLinkOneNetConnectParams.getSerialNumber();
        if (serialNum == null) {
            serialNum = getSerialNum(equipmentId);
            fiLinkOneNetConnectParams.setSerialNumber(serialNum);
        }
        //解析获取请求帧数据包
        String hexData = requestResolver.resolveUdpReq(fiLinkOneNetConnectParams);
        //请求帧
        String cmdId = abstractReqParams.getCmdId();
        log.info("[sendMsg-resolve] : get request {} of the control {}", hexData, equipmentId);
        if (CmdType.REQUEST_TYPE == fiLinkOneNetConnectParams.getCmdType()) {
            //构造redis参数
            ...
            dataMap.put(OneNetParamsKey.IMEI, fiLinkOneNetConnectParams.getImei());
            dataMap.put(OneNetParamsKey.APP_ID, fiLinkOneNetConnectParams.getAppId());
            ...
            if (CmdId.SET_CONFIG.equals(cmdId)) {
                ...
                unlockPushBean.setToken(fiLinkOneNetConnectParams.getToken());
                unlockPushBean.setAppKey(fiLinkOneNetConnectParams.getAppKey());
                unlockPushBean.setPhoneId(fiLinkOneNetConnectParams.getPhoneId());
                //将用户token存入redis
    ...
    private synchronized Integer getSerialNum(String equipmentId) {
        try {
            ...
            log.error("get serial number from redis failed", e);
        }
        return 1;
    }
