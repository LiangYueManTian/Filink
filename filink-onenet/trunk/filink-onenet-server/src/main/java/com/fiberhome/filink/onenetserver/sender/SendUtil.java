...
public class SendUtil {
    ...
    public void sendData(FiLinkOneNetConnectParams oneNetConnectParams, String data) {
        ...
        String equipmentId = oneNetConnectParams.getEquipmentId();
        String imei = oneNetConnectParams.getImei();
        if (StringUtils.isEmpty(productId)) {
            log.error("[sendMsg] : oneNet productId of the control {} is null", equipmentId);
            return;
        }
        //根据产品ID获取秘钥
        ...
        try {
            ...
            log.info("[sendMsg] : the control {} write  response content, response errorNo: {}, error: {}",
                    equipmentId, oneNetResponse.getErrorno(), oneNetResponse.getError());
        } catch (Exception e) {
            log.error("[sendMsg] : send to OneNET error", e);
