...
public class OneNetServiceImpl implements OneNetService {
    ...
    public boolean updateOneNetHost(HostBean hostBean) {
        ...
        try {
            ...
            log.error("[update http config]: set http config:{} to redis failed,　{}", host, e);
            return false;
        }
        log.info("[update http config]: update http config:{} success", host);
        return true;
    }
    ...
    public Result createDevice(CreateDevice createDevice) {
        String name = createDevice.getTitle();
        try {
            log.info("[addControl]: create device {}, imei is :{}", name, createDevice.getImei());
            ...
            if (!(operation.getErrorno() == 0 && OneNetHttpConstants.CREATE_SUCCESS.equals(operation.getError()))) {
                log.warn("[addControl]: create device {} failed, return {}", name, operation.toString());
            ...
            log.info("[addControl]: create device {} success, return id:{}", name, createResult.getDeviceId());
            return ResultUtils.success(OneNetResultCode.SUCCESS,
                    I18nUtils.getSystemString(OneNetI18n.CREATE_SUCCESS), createResult);
        } catch (Exception e) {
            log.error("[addControl]: create  device {} error {}", name, e);
        }
    ...
    public Result deleteDevice(DeleteDevice deleteDevice) {
        String deviceId = deleteDevice.getDeviceId();
        try {
            log.info("[deleteControl]: delete device id: {}", deviceId);
            //根据产品ID获取HTTP安全鉴权Token
            ...
            if (!(operation.getErrorno() == 0 && OneNetHttpConstants.CREATE_SUCCESS.equals(operation.getError()))) {
                log.warn("[deleteControl]: delete device id {} failed, return: {}", deviceId, operation.toString());
                throw new FilinkOneNetException("oneNet response: delete device error");
            }
            log.info("[deleteControl]: delete device success");
            return ResultUtils.success(OneNetResultCode.SUCCESS,
                    I18nUtils.getSystemString(OneNetI18n.DELETE_SUCCESS));
        } catch (Exception e) {
            log.error("[deleteControl]: delete device id {} error {}", deviceId, e);
        }
