...
public class FiLinkNewMsgBusinessHandler implements MsgBusinessHandler {
    ...
    public void handleMsg(AbstractResOutputParams abstractResOutputParams) throws Exception {
        FiLinkOneNetOutputParams outputParams = (FiLinkOneNetOutputParams) abstractResOutputParams;
        //指令id
        ...
        if (CmdType.RESPONSE_TYPE == cmdType && cmdOk != CmdOk.SUCCESS) {
            log.info("the equipmentId : " + equipmentId + " cmd : " + cmdId + " response is error>>>>>>");
            //移除重发指令中的上一个升级包指令
        ...
        if (deviceInfo == null || deviceInfo.getAreaInfo() == null) {
            throw new ResponseException("device info or area info is null>>>>>>>>>>>>");
        }
        //下发设备缓存开锁命令
        ...
        //过滤相同流水号的指令
        if (!CmdId.FILE_UPLOAD.equals(cmdId)
                && !CmdId.UPGRADE_ADVISE.equals(cmdId)
                && !CmdId.UPGRADE_DATA.equals(cmdId)
                && filterRepeatCmd(equipmentId, outputParams.getCmdType(), serialNumber)) {
            log.info("the equipmentId : " + equipmentId + " , cmd : " + cmdId + " is repeat>>>>>>>>>>>>");
            return;
        }
        Map<String, Object> dataSource = outputParams.getParams();
        DeviceLog deviceLog = setDeviceLogCommonInfo(deviceInfo, control);
        //根据不同指令进行不同处理
    ...
    private void updateDeployStatus(String equipmentId, ControlParam control) {
        String deploying = DeployStatus.DEPLOYING.getCode();
        //如果是部署中则进行部署状态下发
        if (deploying.equals(control.getDeployStatus())) {
            ...
            if (StringUtils.isEmpty(dataMap)) {
                log.error("deploy status cmd redis is null : " + equipmentId);
            } else {
                FiLinkOneNetConnectParams oceanConnectParams = new FiLinkOneNetConnectParams();
                oceanConnectParams.setAppId(control.getProductId());
                oceanConnectParams.setImei(control.getImei());
    ...
    private void refreshHeatBeatTime(String equipmentId, ControlParam control) {
        ...
        Object restartTimeObj = configMap.get(ParamsKey.RESTART_TIME);
        if (restartTimeObj == null) {
            throw new ResponseException("the control : " + equipmentId + "restart time is null>>>>>>>>>>>>>");
        }
        ...
        //重启时间
        long restartTime = Integer.parseInt(restartTimeObj.toString()) * 60;
        //心跳周期
        ...
        //设置离线过期时间
        RedisUtils.set(offLineKey, equipmentId, 2 * hearBeatCycle);
        //设置失联过期时间
        RedisUtils.set(outOfConcatKey, equipmentId, 2 * restartTime);
    }
    ...
    private void handleCmd(String cmdId, String deviceId, String equipmentId, Map<String, Object> dataSource,
        ...
        switch (cmdId) {
            ...
            case CmdId.UPGRADE_ADVISE:
                resolveUpgradeAdvise(equipmentId, dataSource, serialNumber, control);
                break;
            //电子锁升级数据包
            case CmdId.UPGRADE_DATA:
                resolveUpgradeData(equipmentId, serialNumber, control);
                break;
            //部署状态
     ...
     * @param control     主控
     */
    private void updateDeviceStatusAlarm(String equipmentId, ControlParam control) {
        log.info("update alarm status to alarm : " + equipmentId);
        String alarmCode = DeviceStatus.Alarm.getCode();
        if (!alarmCode.equals(control.getDeviceStatus())) {
            //将状态更新为正常
            updateDeviceStatus(equipmentId, alarmCode);
        }
    }
    ...
    private void updateDeviceStatusNormal(String equipmentId, ControlParam control, String deviceId) {
        ...
        if (offlineCode.equals(deviceStatus)
            ...
            //将状态更新为正常
            updateDeviceStatus(equipmentId, normalCode);
            //发送通信中断告警恢复
     ...
     * @param status      状态
     */
    private void updateDeviceStatus(String equipmentId, String status) {
    ...
    private void resolveDeployStatus(String equipmentId, Integer currentSerialNumber, Map<String, Object> dataSource) {
        ...
        if (currentSerialNumber >= serialNumber) {
            ...
            log.error("deploy status currentSerialNum : " + currentSerialNumber + " , cmd serialNum : " + serialNumber);
        }
    }
     ...
     */
    private void resolveUpgradeData(String equipmentId, int serialNumber, ControlParam controlParam) {
        //移除重发指令中的上一个升级包指令
        String key = equipmentId + Constant.SEPARATOR + serialNumber;
        RedisUtils.hRemove(RedisKey.CMD_RESEND_BUFFER, key);
        //获取redis中数据包信息
        ...
        if (upgradeMap == null) {
            throw new ResponseException("upgrade info is null in the redis>>>>>>");
        }
        resolveData(equipmentId, controlParam, upgradeMap);
    }
     ...
     * @param dataSource   元数据
     */
    private void resolveUpgradeAdvise(String equipmentId, Map<String, Object> dataSource,
                                      int serialNumber, ControlParam controlParam) {
        //移除重发指令中的上一个升级包指令
        String key = equipmentId + Constant.SEPARATOR + serialNumber;
        RedisUtils.hRemove(RedisKey.CMD_RESEND_BUFFER, key);
        //获取续传包序号
        String continuePackageNum = dataSource.get(ParamsKey.CONTINUE_PACKAGE_NUM).toString();
        log.info("220A currentPackage : " + continuePackageNum);
        //获取redis中该设备升级包信息
        Map upgradeMap = (Map) RedisUtils.hGet(RedisKey.DEVICE_UPGRADE, equipmentId);
        if (upgradeMap == null) {
            throw new ResponseException("upgrade info is null in the redis>>>>>>");
        ...
        //设置当前包序号
        upgradeMap.put(ParamsKey.PACKAGE_NUM, continuePackageNum);
        resolveData(equipmentId, controlParam, upgradeMap);
    }
     ...
     * @param controlParam 主控信息
     * @param upgradeMap   设备升级redis信息
     */
    private void resolveData(String equipmentId, ControlParam controlParam, Map upgradeMap) {
        //收到电子锁升级控制响应,发送升级数据包
        //获取当前包序号
        int packageNum = Integer.parseInt(upgradeMap.get(ParamsKey.PACKAGE_NUM).toString());
        //获取总包数
        ...
        //根据当前包序号获取文件
        String packageData = getCurrentFileData(packageNum, packageSum, fileKey, maxUpgradeLen, controlParam);
        //获取crc
    ...
    private void resolveUpgradeSuccess(ControlParam control, Map<String, Object> dataSource,
        ...
        if (Constant.UPGRADE_SUCCESS.equals(upgradeResult)) {
            upgradeMsg = I18nUtils.getString(OneNetI18n.UPGRADE_SUCCESS);
        } else {
            upgradeMsg = I18nUtils.getString(OneNetI18n.UPGRADE_FAIL);
        }
        deviceLog.setRemarks(upgradeMsg);
        //保存设施日志
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.UPGRADE_SUCCESS_EVENT),
                DeviceEventType.UPGRADE_SUCCESS_EVENT);
        //判断是否需要处理
        if (isNeedToResolve(equipmentId, CmdId.UPGRADE_SUCCESS, currentTime)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.UPGRADE_SUCCESS + " is not the newest>>>>>>>>");
            return;
        }
        //软硬件版本信息
        ...
        control.setActualValue(null);
        controlFeign.updateControlParam(control);
    }
    ...
    private void resolveFileUpload(ControlParam control, String deviceId, Map<String, Object> dataSource,
        ...
        //拼接redis中key值
        String key = equipmentId + index;
        //从redis中获取文件包
        Map<String, Object> picMap = (Map<String, Object>) RedisUtils.hGet(RedisKey.PIC_UPLOAD, key);
        if (picMap == null) {
            picMap = new HashMap<>(64);
        }
        picMap.put(ParamsKey.INDEX, index);
        picMap.put(ParamsKey.PACKAGE_SUM, packageSum);
        picMap.put(packageNum, dataContent);
        RedisUtils.hSet(RedisKey.PIC_UPLOAD, key, picMap);
        log.info("current picNum : " + packageNum + " , size : " + picMap.size());
        //判断包是否全部传输完毕
        if (packageNum.equals(String.valueOf(packageSum))) {
            //拼接图片文件
            mergePic(equipmentId, index, packageSum, picMap, deviceId);
            //清除redis中图片信息
            RedisUtils.hRemove(RedisKey.PIC_UPLOAD, key);
            //清除redis中图片基本信息
            RedisUtils.hRemove(RedisKey.DEVICE_ADVISE, key);
        }
    }
    ...
    private void mergePic(String equipmentId, String index, int packageSum, Map<String, Object> picMap, String deviceId) {
        //从redis获取该文件基本信息
        String key = equipmentId + index;
        Map<String, String> adviseMap = (Map<String, String>) RedisUtils.hGet(RedisKey.DEVICE_ADVISE, key);
        if (adviseMap == null) {
    ...
    private void resolveFileAdvise(String equipmentId, Map<String, Object> dataSource) {
        ...
        //拼接redis中key值
        String key = equipmentId + index;
        RedisUtils.hSet(RedisKey.DEVICE_ADVISE, key, adviseMap);
    }
     ...
     * 设置设施日志公共信息
     *
     * @param deviceInfoDto 设施对象
     * @param control       控制器信息
     * @return 设施日志对象
     */
    private DeviceLog setDeviceLogCommonInfo(DeviceInfoDto deviceInfoDto, ControlParam control) {
        //设施日志
        ...
        //设置设施日志信息
        deviceLog.setAreaId(deviceInfoDto.getAreaInfo().getAreaId());
        deviceLog.setDeviceId(deviceInfoDto.getDeviceId());
    ...
    private void resolveDoorStateChange(ControlParam controlParam, Map<String, Object> dataSource,
        ...
        if (Constant.OPEN_STATE.equals(doorState)) {
            setRemarks(deviceLog, doorNum, I18nUtils.getString(OneNetI18n.OPEN_DOOR_MSG));
        } else {
            setRemarks(deviceLog, doorNum, I18nUtils.getString(OneNetI18n.CLOSE_DOOR_MSG));
        }
        //设置时间
        deviceLog.setCurrentTime(time);
        //保存设施日志
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.DOOR_STATE_CHANGE_EVENT),
                DeviceEventType.DOOR_STATE_CHANGE_EVENT);
        //判断是否需要处理
        if (isNeedToResolve(equipmentId, CmdId.DOOR_STATE_CHANGE, time)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.DOOR_STATE_CHANGE + " is not the newest>>>>>>>>");
            return;
        }
        //更新门状态
    ...
    private void resolveCloseLockUpload(ControlParam controlParam, Map<String, Object> dataSource,
        ...
        //拼接附加信息
        setRemarks(deviceLog, lockNum, I18nUtils.getString(OneNetI18n.CLOSE_LOCK_MSG));
        //设置时间
        deviceLog.setCurrentTime(time);
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.CLOSE_LOCK_EVENT),
                DeviceEventType.CLOSE_LOCK_EVENT);
        //判断是否需要处理
        if (isNeedToResolve(equipmentId, CmdId.CLOSE_LOCK_UPLOAD, time)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.CLOSE_LOCK_UPLOAD + " is not the newest>>>>>>>>");
            return;
        }
        //更新门锁状态
    ...
    private void resolveUnlockResponse(Map<String, Object> dataSource, String equipmentId, Integer serialNumber) {
        ...
        if (paramList != null && paramList.size() > 0) {
            ...
            String msg = I18nUtils.getString(OneNetI18n.UNLOCK_RESULT);
            stringBuilder.append(msg);
            //成功信息
            String success = I18nUtils.getString(OneNetI18n.UNLOCK_SUCCESS);
            //失败信息
            String fail = I18nUtils.getString(OneNetI18n.UNLOCK_FAIL);
            for (Map<String, Object> paramMap : paramList) {
                String slotNum = paramMap.get(ParamsKey.SLOT_NUM).toString();
                String result = paramMap.get(ParamsKey.RESULT).toString();
                String suffix;
                if (LockResult.SUCCESS.equals(result)) {
                    log.info("doorNum : " + slotNum + " open lock success");
                    suffix = slotNum + ":" + success + ",";
                } else {
                    log.info("doorNum : " + slotNum + " open lock fail");
                    suffix = slotNum + ":" + fail + ",";
                }
            ...
            socketMessage.setMsg(socketMsg);
    ...
    private void pushAppMsg(UnlockPushBean pushBean) {
        ...
        pushBean.setTitle(I18nUtils.getString(OneNetI18n.UNLOCK_TITLE));
        //app推送
    ...
    private void resolveSleep(ControlParam control, Map<String, Object> dataSource,
        ...
        //保存设施日志
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.SLEEP_EVENT), DeviceEventType.SLEEP_EVENT);
        //判断时间是否需要处理
        if (isNeedToResolve(equipmentId, CmdId.SLEEP, time)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.SLEEP + " is not the newest>>>>>>>>");
            return;
        }
        //将主控状态更新为休眠
    ...
    private void resolveOpenLockUpload(ControlParam control, Map<String, Object> dataSource, DeviceLog deviceLog,
        ...
        String lockNum = dataSource.get(ParamsKey.LOCK_NUM).toString();
        //拼接附加信息
        setRemarks(deviceLog, lockNum, I18nUtils.getString(OneNetI18n.OPEN_LOCK_MSG));
        //设置日志信息
        deviceLog.setCurrentTime(time);
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.OPEN_LOCK_EVENT),
                DeviceEventType.OPEN_LOCK_EVNET);
        //判断是否需要处理
        if (isNeedToResolve(equipmentId, CmdId.OPEN_LOCK_UPLOAD, time)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.OPEN_LOCK_UPLOAD + " is not the newest>>>>>>>>");
            return;
        }
        //更新门锁状态
        Lock lock = setDoorLockInfo(equipmentId, lockNum, lockState, time);
    ...
    private void resolveSetConfig(String equipmentId, Integer currentSerialNumber) {
        ...
        if (currentSerialNumber >= serialNumber) {
            ...
            log.error("set config currentSerialNum : " + currentSerialNumber + " , cmd serialNum : " + serialNumber);
        }
    }
    ...
    private void resolveActive(String equipmentId, ControlParam control,
        ...
        //消息时间
        long time = Long.parseLong(dataSource.get(ParamsKey.TIME).toString()) * 1000;
        log.info("active time is :" + String.valueOf(time));
        //发送回包
        ...
        //保存设施日志
        saveDeviceLog(deviceLog, DeviceLogType.EVENT,
                I18nUtils.getString(DeviceLogNameI18n.ACTIVE_EVENT), DeviceEventType.ACTIVE_EVENT);
        if (isNeedToResolve(equipmentId, CmdId.ACTIVE, time)) {
            log.info("the control : " + equipmentId + " cmd : " + CmdId.ACTIVE + " is not the newest>>>>>>>>");
            return;
        }
        //获取软硬件版本
    ...
    private void updateSleepControlInfo(ControlParam control, Map<String, Object> dataSource) {
        ...
        //将map转成json
        control.setActualValue(JSONObject.toJSONString(result));
        controlFeign.updateControlParam(control);
    }
    ...
    private void sendResponse(ControlParam control, String cmdId, Map<String, Object> params, Integer serialNumber) {
        FiLinkOneNetConnectParams fiLinkReqOceanConnectParams = new FiLinkOneNetConnectParams();
        //设置指令类型
        ...
        //设置imei
        fiLinkReqOceanConnectParams.setImei(control.getImei());
    ...
    private void sendRequest(String equipmentId, String cmdId, Map<String, Object> params, ControlParam controlParam) {
        ...
        fiLinkReqOceanConnectParams.setParams(params);
        //设置平台id
        fiLinkReqOceanConnectParams.setImei(controlParam.getImei());
        //产品ID
