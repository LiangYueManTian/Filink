...
//        AreaInfo areaInfo = new AreaInfo();
    ...
    public void addDeviceData() {
        ...
        for (int h = 0; h < aid.size(); h++) {
            int a = 10000;
//        int a=60000;
            for (int i = 1; i <= a; i++) {
                DeviceInfo deviceInfo = new DeviceInfo();
                deviceInfo.setDeviceId(NineteenUUIDUtils.uuid());
                deviceInfo.setDeviceName(generateDeviceName());
                deviceInfo.setDeviceType(generateDeviceType());
                deviceInfo.setDeviceStatus(generateDeviceStatus());
                deviceInfo.setDeviceCode(UUID.randomUUID().toString().substring(0, 18));
                deviceInfo.setAddress("湖北省XX市XX区XX街道");
                deviceInfo.setDeployStatus(generateDeployStatus());
                deviceInfo.setProvinceName("湖北省");
                deviceInfo.setCityName("XX市");
                deviceInfo.setDistrictName("XX区");
                deviceInfo.setPositionBase(generatePositionTwo());
                deviceInfo.setRemarks("remark" + i);
                deviceInfo.setCreateUser("1");
                long currentTimeMillis = System.currentTimeMillis();
                deviceInfo.setCreateTime(new Timestamp(currentTimeMillis));
                deviceInfo.setUpdateTime(new Timestamp(currentTimeMillis));

                deviceInfo.setAreaId(aid.get(h));

                deviceInfoList.add(deviceInfo);

                if (i % 10000 == 0) {
                    deviceInfoDao.addtDeviceList(deviceInfoList);
    ...
    /**
     * 生成在湖北省范围类的经纬度坐标 35000
     *
     * @return
     */
    private String generatePositionTwo() {
        Random random = new Random();

        double lat = random.nextDouble() * (113.255269 - 110.412887) + 110.412887;
        double lng = random.nextDouble() * (31.920474 - 30.30596) + 30.30596;
     ...
     * @return
     */
    private String generateDeployStatus() {
        String deployStatus = "1";
        Random random = new Random();
        //生成[0,6)之间的整数
     ...
     * @return
     */
    private String generateDeviceStatus() {
