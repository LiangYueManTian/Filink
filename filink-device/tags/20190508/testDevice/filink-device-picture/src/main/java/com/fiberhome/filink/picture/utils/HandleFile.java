...
public class HandleFile {
    ...
    public Boolean generatedPic(List<DevicePicResp> devicePicRespList,Export export, ExportDto exportDto) throws Exception {
        ...
        log.info("Folder generated successfully");
        //生成压缩文件并更新进度
        generatedZip(key,export,exportDto);
        log.info("Compressed file generated successfully");
        //上传到文件服务器并保存到数据库
        uploadToFastDfs(key,export,exportDto);
        log.info("Uploaded to the file server successfully");
        return true;
    }
     ...
     * @return void
     */
    public void uploadToFastDfs(String key,Export export,ExportDto exportDto) {
        ...
        if (StringUtils.isEmpty(zipUrl)) {
            ...
            throw new ExportPicStopException(I18nUtils.getSystemString(PicRelationI18nConstants.FAILED_TO_UPLOAD_FILE_TO_FILE_SERVER));
        } else {
            //远程更新任务状态为已完成
     ...
     */
    public void changeTaskStatusToUnusual(String taskInfoId) {
        //将任务状态设置为异常状态
     ...
     * @return Boolean
     */
    public Boolean updateTaskFileNumById(ExportDto exportDto,String key) {
        //已经生成文件数量递增并保存到数据库
        ...
        log.info("---------------------Remote update build file progress--------------------return-----" + !updateTaskFileNumBoolean);
     ...
     *
     */
    public void writeZip(File file, String parentPath, ZipOutputStream zos,String key,ExportDto exportDto) {
