...
public class AreaInfoServiceImpl extends ServiceImpl<AreaInfoDao, AreaInfo> implements AreaInfoService {
    ...
    public Result queryAreaNameIsExist(AreaInfo areaInfo) {
        ...
        if (resultAreaInfo != null) {
            return ResultUtils.warn(AreaResultCodeConstant.NAME_IS_EXIST, I18nUtils.getSystemString(AreaI18nConstant.NAME_IS_EXIST));
        }
    ...
    public Result queryAreaListByItem(QueryCondition<AreaInfoDto> queryCondition) {
        if (queryCondition.getBizCondition() == null) {
            return ResultUtils.warn(AreaResultCodeConstant.PARAM_NULL, I18nUtils.getSystemString(AreaI18nConstant.PARAM_NULL));
        }
    ...
    public Result deleteAreaByIds(List<String> areaIds) {
        ...
        for (String areaId : areaIds) {
            ...
            if (childrenIds.size() != 0) {
                ...
                for (String childId : childrenIds) {
                    ...
                    if (!areaIds.contains(childId)) {
                        return ResultUtils.warn(AreaResultCodeConstant.HAVE_CHILD, I18nUtils.getSystemString(AreaI18nConstant.HAVE_CHILD));
                    }
                }
            }
            //查询该区域下的关联设施
            ...
            if (deviceInfoList != null && deviceInfoList.size() != 0) {
                return ResultUtils.warn(AreaResultCodeConstant.HAVE_DEVICE, I18nUtils.getSystemString(AreaI18nConstant.HAVE_DEVICE));
            }
        }
        //获取删除条数
    ...
    public Result exportArea(ExportDto exportDto) {
        ...
        try {
            ...
            fe.printStackTrace();
            return ResultUtils.warn(AreaResultCodeConstant.EXPORT_NO_DATA, I18nUtils.getSystemString(AreaI18nConstant.EXPORT_NO_DATA));
        } catch (FilinkExportDataTooLargeException fe) {
            return getExportToLargeMsg(fe);
        } catch (FilinkExportTaskNumTooBigException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(AreaResultCodeConstant.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS, I18nUtils.getSystemString(AreaI18nConstant.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            e.printStackTrace();
            return ResultUtils.warn(AreaResultCodeConstant.FAILED_TO_CREATE_EXPORT_TASK, I18nUtils.getSystemString(AreaI18nConstant.FAILED_TO_CREATE_EXPORT_TASK));
        }
    ...
    public List<Department> getDepartmentList() {
        ...
        try {
            departmentList = departmentFeign.queryAllDepartment();
        } catch (Exception e) {
            log.error("get department faile : {}",e);
        }
        if (departmentList == null) {
