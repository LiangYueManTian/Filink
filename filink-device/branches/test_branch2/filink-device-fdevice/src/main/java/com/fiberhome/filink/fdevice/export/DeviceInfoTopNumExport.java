 ...
 */
@Component
public class DeviceInfoTopNumExport extends AbstractExport {
    /**
     * 获取导出数据
     *
     * @param condition 条件
    ...
    protected List queryData(QueryCondition condition) {
        ...
        List<DeviceInfoForTopNum> deviceInfoForTopNumList = new ArrayList<>();
        for (Object object : objectList) {
            JSONObject json = JSONObject.fromObject(object);
            DeviceInfoForTopNum deviceTop = new DeviceInfoForTopNum();
            deviceTop.jsonToBean(json);
            deviceInfoForTopNumList.add(deviceTop);
        }
        return deviceInfoForTopNumList;
    }
