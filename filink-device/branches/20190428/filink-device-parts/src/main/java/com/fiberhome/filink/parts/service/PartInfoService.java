...
public interface PartInfoService extends IService<PartInfo> {
    ...
    static User convertObjectToUser(Object userObj) {
        //校验是否有值
        if (userObj == null) {
            throw new FilinkPartsException(I18nUtils.getString(PartsI18n.USER_SERVER_ERROR));
        }
        User user = JSON.parseObject(JSON.toJSONString(userObj), User.class);
        //校验用户信息
        if (user == null || user.getDepartment() == null || user.getRole() == null) {
            throw new FilinkPartsException(I18nUtils.getString(PartsI18n.USER_AUTH_INFO_ERROR));
        }
