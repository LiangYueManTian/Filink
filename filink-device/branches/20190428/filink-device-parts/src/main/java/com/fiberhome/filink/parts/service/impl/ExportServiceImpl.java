...
public class ExportServiceImpl implements ExportService {
    ...
    public Result exportPartList(ExportDto exportDto) {
        Export export = null;
        try {
            export = partInfoExport.insertTask(exportDto, SERVER_NAME, I18nUtils.getString(PartsI18n.PART_INFO_LIST_NAME));
        } catch (FilinkExportNoDataException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(PartsResultCode.EXPORT_NO_DATA, I18nUtils.getString(PartsI18n.EXPORT_NO_DATA));
        } catch (FilinkExportDataTooLargeException fe) {
            fe.printStackTrace();
            String string = I18nUtils.getString(PartsI18n.EXPORT_DATA_TOO_LARGE);
            ...
            fe.printStackTrace();
            return ResultUtils.warn(PartsResultCode.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS, I18nUtils.getString(PartsI18n.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            e.printStackTrace();
            return ResultUtils.warn(PartsResultCode.FAILED_TO_CREATE_EXPORT_TASK, I18nUtils.getString(PartsI18n.FAILED_TO_CREATE_EXPORT_TASK));
        }
        addLogByExport(exportDto);
        partInfoExport.exportData(export);
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(PartsI18n.THE_EXPORT_TASK_WAS_CREATED_SUCCESSFULLY));
    }
