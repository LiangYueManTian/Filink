...
import com.fiberhome.filink.picture.utils.*;
...
public class PicRelationInfoServiceImpl extends ServiceImpl<PicRelationInfoDao, PicRelationInfo> implements PicRelationInfoService {
    ...
    public Result imageListByPage(QueryCondition<DevicePicReq> queryCondition) {
        ...
        //获取权限信息
        Result result = this.getPermissionsInfo(queryCondition);
        if (PicRelationResultCode.USER_SERVER_ERROR == result.getCode()) {
            //用户服务异常提示语
            return result;
        } else if (0 == result.getCode() && ObjectUtils.isEmpty(result.getData())) {
            //用户权限设置有误，直接返回空
            return result;
        }
    ...
    public Result batchDownLoadImages(ExportDto exportDto) throws Exception {
        ...
        if (ObjectUtils.isEmpty(devicePicRespList)) {
            return ResultUtils.warn(PicRelationConstants.FAIL, I18nUtils.getString(PicRelationI18nConstants.DOWNLOAD_FILE_OVER_THE_MIN_NUM));
        }

        //最大导出图片数
        if (maxExportPicDataSize < devicePicRespList.size()) {
            return ResultUtils.warn(PicRelationConstants.FAIL, I18nUtils.getString(PicRelationI18nConstants.DOWNLOAD_FILE_OVER_THE_MAX_NUM));
        }

        //创建任务对象
        exportDto.setListName(I18nUtils.getString(PicRelationI18nConstants.PIC_LIST));
        ...
        if (!ObjectUtils.isEmpty(result)) {
            if (0 == result.getCode()) {
                ...
                return ResultUtils.warn(PicRelationConstants.FAIL, I18nUtils.getString(PicRelationI18nConstants.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
            } else if (PicRelationResultCode.TASK_DOES_NOT_EXIST == result.getCode()) {
                return ResultUtils.warn(PicRelationConstants.FAIL, I18nUtils.getString(PicRelationI18nConstants.TASK_DOES_NOT_EXIST));
            }
        }
        //获取数据
        ...
        return ResultUtils.success(PicRelationConstants.SUCCESS, I18nUtils.getString(PicRelationI18nConstants.INCREASE_TASK_SUCCESS));
    }
    ...
    public Result updateImageIsDeletedByIds(List<PicRelationInfo> picRelationInfos, String isDeleted) {
        ...
        if (this.checkProcParamsForUpdate(picRelationInfos)) {
            return ResultUtils.warn(PicRelationResultCode.PIC_PARAM_ERROR, I18nUtils.getString(PicRelationI18nConstants.PIC_PARAM_ERROR));
        }
        ...
        if (!ObjectUtils.isEmpty(picRelationInfos)) {
            ...
            if (picRelationInfos.size() != count) {
                throw new DeletePicRelationException(I18nUtils.getString(PicRelationI18nConstants.DELETE_PIC_FAIL));
            }
            // 保存删除图片操作日志
            this.saveOperatorLog(picRelationInfos, LogConstants.DATA_OPT_TYPE_DELETE);
        }
        return ResultUtils.success(PicRelationConstants.SUCCESS, I18nUtils.getString(PicRelationI18nConstants.DELETE_PIC_SUCCESS));
    }
    ...
    public Result uploadImages(List<BatchUploadPic> batchUploadPics) throws Exception {
        ...
        if (this.checkProcParamsForBatchUpload(batchUploadPics)) {
            return ResultUtils.warn(PicRelationConstants.FAIL, I18nUtils.getString(PicRelationI18nConstants.PIC_PARAM_ERROR));
        }
        //上传图片文件信息
        ...
        if (!this.getFileInfo(batchUploadPics, picMap, picRelationInfos)) {
            return ResultUtils.warn(PicRelationResultCode.UPLOAD_PIC_TYPE_FAIL, I18nUtils.getString(PicRelationI18nConstants.UPLOAD_PIC_TYPE_FAIL));
        }
        //批量上传图片
        if (!this.uploadFileImage(picMap, picsUrlMap, picRelationInfos)) {
            return ResultUtils.warn(PicRelationResultCode.UPLOAD_PIC_FAIL, I18nUtils.getString(PicRelationI18nConstants.UPLOAD_PIC_FAIL));
        }
        ...
        if (PicRelationConstants.FAIL.equals(this.saveImagesData(picRelationInfos).getCode())) {
            return ResultUtils.warn(PicRelationResultCode.SAVE_PIC_INFO_FAIL, I18nUtils.getString(PicRelationI18nConstants.SAVE_PIC_INFO_FAIL));
        }
        return ResultUtils.success(PicRelationConstants.SUCCESS, I18nUtils.getString(PicRelationI18nConstants.UPLOAD_PIC_SUCCESS));
    }
    ...
    public Result getLivePicInfoByDeviceIdsForApp(Set<String> deviceIds, String picNum) {
        ...
        return ResultUtils.success(livePicRespList);
    }
     ...
     * @return Map<String ,  I mageUploadBean>
     */
     ...
     * @return Boolean
     */
    public Boolean saveOperatorLog(List<PicRelationInfo> picRelationInfos, String operatorType) {
        Boolean flag = false;
        // 保存图片操作日志
     ...
     */
    public Result getPermissionsInfo(QueryCondition<DevicePicReq> queryCondition) {
        ...
        if (!ObjectUtils.isEmpty(loginUserId)) {
            ...
            if (adminUserId.equals(loginUserId)) {
                return ResultUtils.success(queryCondition);
            }
        }
        ...
        //校验是否有值
        if (userObj == null) {
            return ResultUtils.warn(PicRelationResultCode.USER_SERVER_ERROR, I18nUtils.getString(PicRelationI18nConstants.USER_SERVER_ERROR));
        }
