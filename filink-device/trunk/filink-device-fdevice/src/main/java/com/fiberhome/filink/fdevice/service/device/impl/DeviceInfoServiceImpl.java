...
public class DeviceInfoServiceImpl extends ServiceImpl<DeviceInfoDao, DeviceInfo>
    ...
    @Value("${lockServerState}")
    private boolean lockServerState;

    @Value("${rfidServerState}")
    private boolean rfidServerState;
     ...
     */
    @Override
    public Result listDeviceForSelectionLock(QueryCondition<DeviceInfo> queryCondition) {
        // 构造分页条件
        Page page = MpQueryHelper.myBatiesBuildPage(queryCondition);
        //添加当前用户权限
        if (!DeviceConstant.ADMIN.equals(RequestInfoUtils.getUserId())) {
            if (addUserLockPermission(queryCondition)) {
                return getDeviceInfoPageEmpty(page);
            }
        }

        //若排序条件为空,默认时间排序
        setSortCondition(queryCondition);

        //修改like过滤条件值
        alterLikeFilterCondition(queryCondition.getFilterConditions());

        //根据查询条件查询设施数据
        List<String> deviceIdList = deviceInfoDao.selectDeviceIdList(
                queryCondition.getFilterConditions(), queryCondition.getSortCondition());
        //查询电子锁设施ID
        deviceIdList = getDeviceIdListHaveLock(deviceIdList);
        //无有权限电子锁设施
        ...
        Integer count = deviceIdList.size();
        //开始条数
        ...
        List<String> list;
        if (end <= count) {
            list = deviceIdList.subList(begin, end);
        } else if (begin < count) {
            list = deviceIdList.subList(begin, count);
        } else {
            return getDeviceInfoPageEmpty(page);
        }
        //查询设施信息
        List<DeviceInfoDto> deviceInfoDtoList = deviceInfoDao.selectDeviceInfoListByIds(list);
        //添加门锁信息
        if (addLockForDeviceInfo(list, deviceInfoDtoList)) {
            //电子锁服务异常
            return getDeviceInfoPageEmpty(page);
        }
        PageBean pageBean = MpQueryHelper.myBatiesBuildPageBean(page, count, deviceInfoDtoList);
     ...
     *
     * @param list              设施ID
     * @param deviceInfoDtoList 设施信息
     * @return true电子锁服务异常 false 正常
     */
    private boolean addLockForDeviceInfo(List<String> list, List<DeviceInfoDto> deviceInfoDtoList) {
        //查询所有门锁
        List<Lock> lockList = lockFeign.lockListByDeviceIds(list);
        if (lockList == null) {
            log.warn("Lock Server is Exception");
            return true;
        }
     ...
     * @param queryCondition 查询条件
     */
    private void setSortCondition(QueryCondition<DeviceInfo> queryCondition) {
        SortCondition sortCondition = queryCondition.getSortCondition();
        if (sortCondition == null || StringUtils.isEmpty(sortCondition.getSortField())
                || StringUtils.isEmpty(sortCondition.getSortRule())) {
            sortCondition = new SortCondition();
     ...
     * 获取权限数据中有电子锁的设施ID
     *
     * @param deviceIdList 权限数据设施ID
     * @return 有电子锁的设施ID
     */
    private List<String> getDeviceIdListHaveLock(List<String> deviceIdList) {
        if (!CollectionUtils.isEmpty(deviceIdList)) {
            List<String> deviceIdListOfLock = lockFeign.queryDeviceIdListOfLock();
            if (CollectionUtils.isEmpty(deviceIdListOfLock)) {
                //无电子锁设施
                deviceIdList = new ArrayList<>();
            } else {
                //取含有电子锁设施ID
                deviceIdList.retainAll(deviceIdListOfLock);
            }
        }
        return deviceIdList;
    }
     ...
     *
     * @param queryCondition 查询条件
     * @return true没有区域或电子锁权限 false有权限
     */
    private boolean addUserLockPermission(QueryCondition<DeviceInfo> queryCondition) {
        ...
        for (RoleDeviceType roleDeviceType : roleDeviceTypes) {
            String deviceTypeId = roleDeviceType.getDeviceTypeId();
            int i = deviceTypeId.indexOf("-1");
            if (i != -1) {
                deviceTypes.add(deviceTypeId.substring(0, i));
            }
        }
        //判断权限
        if (CollectionUtils.isEmpty(areaIds) || CollectionUtils.isEmpty(deviceTypes)) {
            return true;
        }
        queryCondition.getFilterConditions().add(DeviceInfoService.generateFilterCondition("areaId", "in", areaIds));
        queryCondition.getFilterConditions().add(DeviceInfoService.generateFilterCondition("deviceType", "in", deviceTypes));
        return false;
    }
     ...
     * @return 空设施集合
     */
    private Result getDeviceInfoPageEmpty(Page page) {
        PageBean pageBean = MpQueryHelper.myBatiesBuildPageBean(page, 0, new ArrayList<DeviceInfoDto>());
        return ResultUtils.pageSuccess(pageBean);
    }
    ...
    @Override
    public Result queryDeviceIsExistById(String id) {
        // 校验设施id
    ...
    private void checkAuthorization(String areaId, String deviceType) {
        if (!isAuthorized(areaId, deviceType)) {
            throw new FilinkDeviceIsNotAuthorizedException(I18nUtils.getSystemString(DeviceI18n.USER_DEVICE_NOT_AUTHORIZED));
        }
    }
...
//        AreaInfo areaInfo = new AreaInfo();
    ...
    public Result deleteDeviceByIds(String[] ids) {
        ...
        //如果rfid服务启动才删除rfid信息
        if (rfidServerState) {
            Result result1 = checkDeviceDelete(idList);
            if (result1 != null) {
                return result1;
            }
        }
...
//        }
        ...
        if (rfidServerState) {
            ...
            for (String id : idList) {
                ...
                if (rLDResult == null || !ResultCode.SUCCESS.equals(rLDResult.getCode())) {
                    log.error("recycling label failed，deviceId：{}", id);
                }
            }
        }
     ...
     * @param deviceIds
     */
    private Result checkDeviceDelete(List<String> deviceIds) {
        if (ObjectUtils.isEmpty(deviceIds)) {
            return ResultUtils.warn(DeviceResultCode.FAIL, I18nUtils.getSystemString(DeviceI18n.DELETE_DEVICE_FAIL));
        }
        Map<String, Boolean> map = deleteDeviceFeign.checkDevices(deviceIds);
        //是否存在业务
        boolean contains = map.values().contains(true);
        if (contains) {
            StringBuffer resultMsg = new StringBuffer();
            resultMsg.append(I18nUtils.getSystemString(DeviceI18n.DEVICE_NOT_DELETE_WITH_LABEL)).append("(");
            for (String key : map.keySet()) {
                Boolean isExits = map.get(key);
                if (isExits) {
                    //true 存在业务
                    String busName = RFIDEnum.getNameByLanguage(key);
                    resultMsg.append(busName).append(" ");
                }
            }
            return ResultUtils.warn(DeviceResultCode.DEVICE_NOT_DELETE_WITH_LABEL, resultMsg.append(")").toString());
        }
        return null;
    }
