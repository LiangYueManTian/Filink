...
import com.fiberhome.filink.fdevice.constant.device.DeviceMapConfigEnum;
...
public class DeviceMapConfigServiceImpl extends ServiceImpl<DeviceMapConfigDao, DeviceMapConfig> implements DeviceMapConfigService {
    ...
    public Result updateDeviceIconSize(DeviceMapConfig deviceMapConfig) {
        //校验参数
        if (checkDeviceMapConfig(deviceMapConfig) || deviceMapConfig.getConfigValue() == null) {
            throw new FilinkDeviceMapParamException();
        }
        //获取用户信息
        String userId = getUserId();
    ...
    public boolean insertConfigBatch(String userId) {
        if(StringUtils.isEmpty(userId)) {
            throw new FiLinkDeviceException(I18nUtils.getSystemString(DeviceMapI18n.CONFIG_USER_PARAM_ERROR));
        }
        //当前用户
        String createUser = getUserId();
        List<DeviceMapConfig> deviceMapConfigs = getDeviceMapConfigs(userId, createUser);
    ...
    public boolean insertConfigBatchUsers(List<String> userIds) {
        ...
        for (String userId : userIds) {
            List<DeviceMapConfig> deviceMapConfigList = getDeviceMapConfigs(userId, createUser);
            deviceMapConfigs.addAll(deviceMapConfigList);
        }
        Integer result = deviceMapConfigDao.insertConfigBatch(deviceMapConfigs);
        int size = userIds.size() * (DeviceType.values().length + 1);
        if (result != size) {
            throw new FilinkDeviceMapException();
        }
        return true;
     ...
     * @return 用户首页地图设施配置信息
     */
    private List<DeviceMapConfig> getDeviceMapConfigs(String userId, String createUser) {
        List<DeviceMapConfig> deviceMapConfigs = new ArrayList<>();
        //插入默认设施类型数据
        for (DeviceType deviceType: DeviceType.values()) {
            DeviceMapConfig deviceMapConfig = new DeviceMapConfig();
            deviceMapConfig.setConfigId(NineteenUUIDUtils.uuid());
        ...
        mapConfig.setConfigId(NineteenUUIDUtils.uuid());
