...
//    Integer selectDeviceCount(@Param("map") Map map);
     ...
     * @return 设施ID
     */
    List<String> selectDeviceIdList(@Param("filterList") List<FilterCondition> filterConditionList,
                                    @Param("sort") SortCondition sortCondition);


    /**
     * 选择器分页查询设施列表（通过设施ID）
     * @param deviceIdList 设施ID
     * @return 设施信息
     */
    List<DeviceInfoDto> selectDeviceInfoListByIds(List<String> deviceIdList);
    /**
     * 根据过滤条件查询设施数量
     *
     * @param filterConditionList 查询条件
