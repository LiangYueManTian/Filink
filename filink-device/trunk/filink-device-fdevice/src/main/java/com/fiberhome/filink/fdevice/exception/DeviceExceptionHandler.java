...
public class DeviceExceptionHandler extends GlobalExceptionHandler {
     ...
     * @return
     */
    @ExceptionHandler(FilinkDeviceIsNotAuthorizedException.class)
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public Result handlerFilinkDeviceIsNotAuthorizedException(FilinkDeviceIsNotAuthorizedException ex) {
        log.error("Bad things:", ex);
        return ResultUtils.warn(DeviceResultCode.DEVICE_IS_NOT_AUTHORIZED, ex.getMessage());
    }
    //==================================
    //===============区域===============
    //==================================
    ...
    public Result handlerAreaDirtyDataException(FiLinkAreaDirtyDataException ex) {
        ...
        if (StringUtils.isEmpty(msg)) {
            return ResultUtils.warn(AreaResultCode.DIRTY_DATA, I18nUtils.getSystemString(AreaI18nConstant.DIRTY_DATA));
        }
        return ResultUtils.warn(AreaResultCode.DIRTY_DATA, msg);
    }
    ...
    public Result handlerAreaDateBaseException(FiLinkAreaDateBaseException ex) {
        ...
        String msg = ex.getMessage();
        if (StringUtils.isEmpty(msg)) {
            return ResultUtils.warn(AreaResultCode.DATE_BASE_ERROR, I18nUtils.getSystemString(AreaI18nConstant.DATA_BASE_ERROR));
        }
        return ResultUtils.warn(AreaResultCode.DATE_BASE_ERROR, ex.getMessage());
    }
    ...
    public Result handlerAreaNoDataPermissionsException(FilinkAreaNoDataPermissionsException ex) {
        log.error("Bad things:", ex);
        return ResultUtils.warn(AreaResultCode.NO_DATA_PERMISSIONS, I18nUtils.getSystemString(AreaI18nConstant.NO_DATA_PERMISSIONS));
    }
    ...
    public Result handlerAreaDateFormatException(FiLinkAreaDateFormatException ex) {
        log.error("Bad things:", ex);
        return ResultUtils.warn(AreaResultCode.PARAMETER_FORMAT_IS_INCORRECT, I18nUtils.getSystemString(AreaI18nConstant.PARAMETER_FORMAT_IS_INCORRECT));
    }
