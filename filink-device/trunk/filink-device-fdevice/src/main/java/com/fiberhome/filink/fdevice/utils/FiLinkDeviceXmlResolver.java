...
public class FiLinkDeviceXmlResolver {
     ...
     */
    public static Map<String, Map<String, FiLinkDeviceConfig>> resolveDeviceXml(InputStream inputStream) throws Exception {
        SAXReader saxReader = new SAXReader();
        Document document = saxReader.read(inputStream);
        Element deviceConfigs = document.getRootElement();
        List<Element> deviceConfigListElement = deviceConfigs.elements();
        Map<String, Map<String, FiLinkDeviceConfig>> config = new HashMap<>(64);
        //解析deviceConfig
        if (deviceConfigListElement != null && deviceConfigListElement.size() > 0) {
            for (Element deviceConfigElement : deviceConfigListElement) {
                DeviceConfig deviceConfig = resolveDeviceConfig(deviceConfigElement);
                String language = deviceConfig.getLanguage();
                Map<String, FiLinkDeviceConfig> configMap = deviceConfig.getConfigMap();
                config.put(language, configMap);
            }
        }
        return config;
    }
     ...
     * 解析deviceConfig节点
     *
     * @param deviceConfigElement deviceConfig节点对象
     * @return DeviceConfig
     */
    private static DeviceConfig resolveDeviceConfig(Element deviceConfigElement) {
        //解析语言
        String language = deviceConfigElement.attributeValue(LANGUAGE_NODE);
        //解析校验规则
        resolveRules(deviceConfigElement);
        //获取根节点
        List<Element> deviceElements = deviceConfigElement.element(DEVICE_NODE).elements();
        Map<String, FiLinkDeviceConfig> deviceConfigMap = new HashMap<>(64);
        for (Element deviceElement : deviceElements) {
            ...
            deviceConfigMap.put(key, deviceConfig);
        }
        DeviceConfig deviceConfig = new DeviceConfig();
        deviceConfig.setLanguage(language);
        deviceConfig.setConfigMap(deviceConfigMap);
        return deviceConfig;
    }
