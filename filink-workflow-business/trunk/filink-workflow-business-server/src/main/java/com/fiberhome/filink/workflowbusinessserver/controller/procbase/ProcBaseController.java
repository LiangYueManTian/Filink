...
public class ProcBaseController {
    ...
    public Result queryTitleIsExists(@RequestBody ProcBase procBase) {
        if (StringUtils.isEmpty(procBase.getTitle())) {
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_PARAM_ERROR, I18nUtils.getSystemString(ProcBaseI18n.PROC_NAME_NULL));
        }
        //新增及修改校验
        boolean isExist = procBaseService.queryTitleIsExists(procBase.getProcId(), procBase.getTitle());
        if (isExist) {
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_PARAM_ERROR, I18nUtils.getSystemString(ProcBaseI18n.PROC_NAME_SAME));
        }
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.PROC_NAME_AVAILABLE));
    }
     ...
     */
    @PostMapping("/downLoadProcByLoginUserForApp")
    public Result downloadProcByLoginUserForApp(@RequestBody ProcBaseReq procBaseReq) {
        return procBaseService.downloadProcByLoginUserForApp(procBaseReq);
    }
     ...
     * @return 是否存在关联工单
     */
    @PostMapping("/existsProcForDeptIdsAndAreaId")
    public Object existsProcForDeptIdsAndAreaId(@RequestBody ProcExistDeptIdsAndAreaIdReq req) {
        return procBaseService.existsProcForDeptIdsAndAreaId(req);
    }
     ...
     * @return
     */
    @PostMapping("/existsProcForDeptIds")
    public Result existsProcForDeptIds(@RequestBody List<String> deptIds) {
        return procBaseService.existsProcForDeptIds(deptIds);
    }
