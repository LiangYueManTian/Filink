...
public class ProcBaseServiceImpl extends ServiceImpl<ProcBaseDao, ProcBase> implements ProcBaseService {
    ...
    public ProcBase regenerateProcProcess(ProcessInfo processInfo) {
        ...
        //原来工单的信息
        ProcBase oldProcBase = procBaseDao.queryProcessByProcId(beforeProcId, nowDate);
        ...
        if (!ObjectUtils.isEmpty(oldProcBase)) {
            if (ProcBaseConstants.PROC_INSPECTION.equals(oldProcBase.getProcType())) {
            ...
            } else if (ProcBaseConstants.PROC_CLEAR_FAILURE.equals(oldProcBase.getProcType())) {
    ...
    public Result updateProcessById(ProcessInfo processInfo) {
        ...
        Long nowDate = System.currentTimeMillis()/1000L;
        //获取工单基础信息
        ProcBase procBase = procBaseDao.queryProcessByProcId(processInfo.getProcBaseReq().getProcId(), nowDate);
        if (ObjectUtils.isEmpty(procBase) || StringUtils.isEmpty(procBase.getProcId())){
        ...
        //获取流程名及流程id，用于保存操作日志
        BeanUtils.copyProperties(processInfo.getProcBase(),processInfo);
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.UPDATE_PROC_SUCCESS), true);
    }
    ...
    public Result updateProcessIsDeletedByIds(List<String> ids,String isDeleted) {
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.DELETE_PROC_SUCCESS));
    }
    ...
    public Result updateProcBaseStatusById(ProcBase procBase) {
        ...
        Long nowDate = System.currentTimeMillis()/1000L;
        //获取工单基础信息
        ProcBase procBaseInfo = procBaseDao.queryProcessByProcId(procBase.getProcId(), nowDate);
        ...
        if (ProcBaseConstants.PROC_STATUS_COMPLETED.equals(procBase.getStatus())) {
            log.info("complete proc : start update proc status");
            Map<String, String> map = new HashMap<>(WorkFlowBusinessConstants.MAP_INIT_VALUE);
            map.put("procType", procBaseInfo.getProcType());
            map.put("procId", procBase.getProcId());
            //回单时需要推送消息
            WebSocketMessage socketMessage = new WebSocketMessage();
            socketMessage.setChannelId(WorkflowBusinessResultCode.RECEIPT_MSG_RESULT_CODE + "");
            socketMessage.setChannelKey("workflowBusiness");
            socketMessage.setMsg(JSONObject.toJSONString(map));
            socketMessage.setMsgType(1);
            Message<WebSocketMessage> message = MessageBuilder.withPayload(socketMessage).build();
            workflowBusinessStreams.receiptProcOut().send(message);
            log.info("complete proc : end update proc status");
        }

        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.UPDATE_PROC_SUCCESS));
    }
    ...
    public Result getProcTypeByProcId(String id) {
        ...
        Long nowDate = System.currentTimeMillis()/1000L;
        //获取工单基础信息
        ProcBase procBaseInfo = procBaseDao.queryProcessByProcId(id, nowDate);
        if (ObjectUtils.isEmpty(procBaseInfo) || StringUtils.isEmpty(procBaseInfo.getProcId())){
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_NOT_EXIST,I18nUtils.getSystemString(ProcBaseI18n.PROC_IS_NOT_EXIST));
        }

        Map<String,String> statusMap = new HashMap<>(WorkFlowBusinessConstants.MAP_INIT_VALUE);
        //转换页面需要状态值
        //未完成
    ...
    public Result queryProcessById(String id) {
        ...
        Long nowDate = System.currentTimeMillis()/1000L;
        //获取工单基础信息
        ProcBaseResp procBaseResp = procBaseDao.queryProcessByProcId(id, nowDate);
        //工单不存在
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.QUERY_PROC_SUCCESS), processDetail);
    }
    ...
    public Result queryListRelatedProcByInspectionTaskIdPage(QueryCondition<ProcBaseReq> queryCondition) {
        ...
        //设置转义之后的工单值
        queryCondition = ProcBaseReq.getParseQueryCondition(queryCondition);
        // 构造分页条件
    ...
    public Result queryCountListProcUnfinishedProcStatus(QueryCondition<ProcBaseReq> queryCondition){
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.QUERY_PROC_STATUS_COUNT_SUCCESS),count);
    }
    ...
    public Result queryCountProcByStatus(ProcBaseReq procBaseReq) {
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.QUERY_PROC_STATUS_COUNT_SUCCESS),statusList.size());
    }
...
//        获取权限信息
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.QUERY_PROC_STATUS_COUNT_SUCCESS),maps);
    }
    ...
    public Result queryProcExitsForIds(List<String> ids,String type) {
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcBaseI18n.QUERY_PROC_SUCCESS), resultIds);
    }
    ...
    public Result assignProc(AssignProcReq req) {
        ...
        //只有流程状态为待指派的状态才能指派工单
        if (!ProcBaseConstants.PROC_STATUS_ASSIGNED.equals(procBase.getStatus())) {
            throw new AssignProcStatusErrorException();
        }
    ...
    public Result downloadProcByLoginUserForApp(ProcBaseReq procBaseReq) {
        ...
        //查询需要下载的工单信息
        List<ProcBaseRespForApp> procBaseRespForApps = this.searchLoginUserDownloadInfoForProcType(procType, procBaseReq);
        if (ObjectUtils.isEmpty(procBaseRespForApps)) {
            ...
            //没有需要下载的工单信息
            return ResultUtils.warn(WorkflowBusinessResultCode.NOT_PROC_CAN_DOWNLOAD, notProcCanDownloadMsg);
        }
    ...
    public Result checkTurnUser(TurnProcReq req) {
        ...
        if (req.getUserId().equals(procBaseInfo.getAssign())) {
            //提示不能指派给自己
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_ASSIGN_MYSELF, I18nUtils.getSystemString(ProcBaseI18n.PROC_ASSIGN_MYSELF));
        }
    ...
    public Result checkAssignUserInfo(String procId, String userId) {
        ...
        if (!isCheckUser) {
            //提示转办用户没有权限
            return ResultUtils.warn(WorkflowBusinessResultCode.ASSIGN_USER_NOT_HAVE_PERMISSION, I18nUtils.getSystemString(ProcBaseI18n.ASSIGN_USER_NOT_HAVE_PERMISSION));
        }
        return null;
    }
    ...
    public ProcBase getProcBaseByProcId(String procId) {
        Long nowDate = System.currentTimeMillis()/1000L;
        ProcBase procBase = procBaseDao.queryProcessByProcId(procId, nowDate);
        return procBase;
    }
    ...
    public ProcBase queryProcBaseById(String procId) {
        Long nowDate = System.currentTimeMillis()/1000L;
        return procBaseDao.queryProcessByProcId(procId, nowDate);
    }
    ...
    public Result revokeProc(RevokeProcReq req) {
        ...
        //只有流程状态为待处理的状态才能撤销工单
        if (!ProcBaseConstants.PROC_STATUS_PENDING.equals(procBase.getStatus())) {
            throw new RevokeProcStatusErrorException();
        }
    ...
    public ProcBase checkResultProcess(CheckSingleBackReq req) {
        ...
        if (!ProcBaseConstants.PROC_STATUS_SINGLE_BACK.equals(procBaseOne.getStatus())
            ...
            throw new CheckSingleBackProcStatusErrorException();
        }
    ...
    public Result downLoadProcForApp(ProcBaseReq procBaseReq) {
        ...
        if (ObjectUtils.isEmpty(procBaseReq.getProcId()) && ObjectUtils.isEmpty(procBaseReq.getProcType())) {
            ...
            //提示请根据工单编号和工单类型下载工单
            return ResultUtils.warn(WorkflowBusinessResultCode.APP_DOWNLOAD_CONDITION_WRONG, i18nMsg);
        }
        //工单类型
        String procType = "";
     ...
     * @return 返回工单下载信息
     */
    public List<ProcBaseRespForApp> searchLoginUserDownloadInfoForProcType(String procType, ProcBaseReq req) {
        ProcBaseReq procBaseReq = new ProcBaseReq();
        ...
        //分页页数
        procBaseReq.setPageNumber(req.getPageNumber());
     ...
     */
    @Override
    public Result existsProcForDeptIdsAndAreaId(ProcExistDeptIdsAndAreaIdReq req) {
        boolean returnFlag = false;
        if (!ObjectUtils.isEmpty(req)) {
            List<String> deptIds = req.getDeptIdList();
            String areaId = req.getAreaId();
            if (!ObjectUtils.isEmpty(deptIds) && !ObjectUtils.isEmpty(areaId)) {
                //销障工单关联部门
                List<ProcClearFailure> procClearFailureList = procClearFailureService.queryClearProcListByDeptIdsAndAreaId(deptIds, areaId);
                //查询巡检工单集合
                List<ProcInspection> procInspectionList = procInspectionService.queryInspectionProcListByDeptIdsAndAreaId(deptIds, areaId);
                boolean exist = true;
                //销障工单关联部门
                if (!ObjectUtils.isEmpty(procClearFailureList)) {
     ...
     */
    @Override
    public Result existsProcForDeptIds(List<String> deptIds) {
        boolean returnFlag = false;
        //销障工单关联部门
        List<ProcClearFailure> procClearFailureList = procClearFailureService.queryClearProcListByDeptIds(deptIds);
     ...
     */
    @Override
    public boolean validateProcIsPermission(String deviceType, String areaId, String userId) {
        //获取当前用户权限信息
        QueryCondition<ProcBaseReq> queryCondition = new QueryCondition<>();
        queryCondition.setBizCondition(new ProcBaseReq());
        queryCondition = this.getPermissionsInfo(queryCondition, userId);

        ProcBaseReq procBaseReq = queryCondition.getBizCondition();
        if (null != procBaseReq) {
            if (ObjectUtils.isEmpty(procBaseReq.getPermissionAreaIds()) && ObjectUtils.isEmpty(procBaseReq.getPermissionDeviceTypes())) {
                return true;
            }
            //是否有区域权限
            boolean isHaveAreaId = false;
            if (!ObjectUtils.isEmpty(procBaseReq.getPermissionAreaIds())) {
                List<String> areaList = new ArrayList<>();
                areaList.addAll(procBaseReq.getPermissionAreaIds());
                if (!ObjectUtils.isEmpty(areaId)) {
                    isHaveAreaId = areaList.contains(areaId);
                }
            }
            //是否有设施类型
            boolean isHaveDeviceType = false;
            if (!ObjectUtils.isEmpty(procBaseReq.getPermissionDeviceTypes())) {
                List<String> deviceTypeList = new ArrayList<>();
                deviceTypeList.addAll(procBaseReq.getPermissionDeviceTypes());
                if (!ObjectUtils.isEmpty(deviceType)) {
                    isHaveDeviceType = deviceTypeList.contains(deviceType);
                }
            }
            if (isHaveAreaId && isHaveDeviceType) {
                return true;
            }
        }
        return false;
