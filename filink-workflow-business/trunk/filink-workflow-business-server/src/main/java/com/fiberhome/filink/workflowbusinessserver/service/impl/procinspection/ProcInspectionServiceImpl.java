...
public class ProcInspectionServiceImpl extends ServiceImpl<ProcInspectionDao, ProcInspection> implements ProcInspectionService {
     ...
     */
    @Override
    public ProcInspection selectProcInspectionOne(ProcInspection procInspection) {
        return procInspectionDao.selectOne(procInspection);
    }
    ...
    public Result addInspectionProc(ProcInspectionReq req, String operate, String isAlarmViewCall) {
        ...
        //新增巡检工单成功提示信息
        String addInspectionSuccess = I18nUtils.getSystemString(ProcInspectionI18n.ADD_PROC_INSPECTION_SUCCESS);
    ...
    public Result addInspectionProcess(ProcInspectionReq req, String operate, boolean isRegenerate, String isAlarmViewCall) {
        ...
        log.info("start add proc the procId : {} the start time : {}, the procType : {} "
                ,procId , simpleDateFormat.format(System.currentTimeMillis()) , ProcBaseConstants.PROC_INSPECTION);
        if (ProcBaseConstants.PROC_RESOURCE_TYPE_2.equals(req.getProcResourceType())) {
            //巡检任务新增巡检工单
            log.info("start inspection task add  inspection proc the procId : {} the start time : {}, the inspectionTaskId : {} "
                    ,procId , simpleDateFormat.format(System.currentTimeMillis()) , req.getInspectionTaskId());
        }
        ...
        if (ObjectUtils.isEmpty(addProcBaseResult) || !successResultCode.equals(addProcBaseResult.getCode())) {
            return ResultUtils.success(WorkflowBusinessResultCode.PROC_ADD_ERROR, I18nUtils.getSystemString(ProcBaseI18n.ADD_PROC_FAIL));
        }
    ...
    public Result updateInspectionProc(ProcInspectionReq req, String operate) {
        ...
        if (null == procBase) {
            //巡检工单不存在
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_NOT_EXIST, I18nUtils.getSystemString(ProcBaseI18n.PROC_IS_NOT_EXIST));
        }
        ...
        //修改巡检工单成功提示信息
        String updateInspectionSuccess = I18nUtils.getSystemString(ProcBaseI18n.UPDATE_PROC_SUCCESS);
        return ResultUtils.success(successResultCode, updateInspectionSuccess);
    }
    ...
    public Result saveProcInspection(ProcInspectionReq req, String operate, String isAlarmViewCall) {
        ...
        if (ProcInspectionConstants.OPERATE_UPDATE.equals(operate)) {
            ...
            if (StringUtils.isEmpty(procId)) {
                //提示缺少工单编号
                return ResultUtils.warn(WorkflowBusinessResultCode.PROC_ID_LOSE, I18nUtils.getSystemString(ProcBaseI18n.PROC_ID_LOSE));
            }
            //修改巡检工单
    ...
    public int saveProcInspectionSpecific(ProcInspection procInspection) {
        ProcInspection procInfo = procInspectionDao.selectInspectionProcByProcId(procInspection.getProcId());
        //区分新增和修改
        if (!ObjectUtils.isEmpty(procInfo)) {
            return procInspectionDao.updateById(procInspection);
        } else {
    ...
    public Result completeInspectionProc(CompleteInspectionProcReq completeReq) {
        String format = "yyyy-MM-dd hh:mm:ss";
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(format);
        log.info("begin complete inspection proc , the procId : {}, the begin time : {}", completeReq.getProcId(), simpleDateFormat.format(System.currentTimeMillis()));
        //校验参数
        ...
        /*if (!ObjectUtils.isEmpty(procBase)) {
            boolean permission = procBaseService.validateProcIsPermission(procBase.getDeviceType(), procBase.getDeviceAreaId(), RequestInfoUtils.getUserId());
            if (!permission) {
                //提示工单没有权限
                return ResultUtils.warn(WorkflowBusinessResultCode.PROC_IS_NOT_PERMISSION, I18nUtils.getSystemString(ProcBaseI18n.PROC_IS_NOT_PERMISSION));
            }
        } else {
            //工单不存在
            return ResultUtils.warn(WorkflowBusinessResultCode.PROC_NOT_EXIST,I18nUtils.getSystemString(ProcBaseI18n.PROC_IS_NOT_EXIST));
        ...
        //新增巡检工单回单日志
        this.addInspectionCommitLog(procBase);
        log.info("end complete inspection proc , the procId : {}, the end time : {}", completeReq.getProcId(), simpleDateFormat.format(System.currentTimeMillis()));
        //巡检设施成功
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(ProcInspectionI18n.INSPECTION_DEVICE_SUCCESS));
    }
     ...
     * @param procBase 工单参数
     */
    public void addInspectionCommitLog(ProcBase procBase) {
        //新增操作日志
    ...
    public void updateProcBaseForCompleteInspection(CompleteInspectionProcReq completeReq, Date nowDate) {
        if (WorkFlowBusinessConstants.COMPLETE_TYPE_COMMIT.equals(completeReq.getOperate())) {
            ...
            procInspection.setProcId(completeReq.getProcId());
            procInspection.setUpdateUser(completeReq.getUserId());
            procInspection.setUpdateTime(nowDate);
            procInspection.setRealityCompletedTime(nowDate);
            //修改工单实际完成时间
            int updateNum = this.updateProcInspectionInfo(procInspection);
            if (updateNum < 0) {
                throw new InspectionDeviceException();
            }
        }
    ...
    public void updateInspectionCompleteCount(ProcInspection procInspectionOne, List<ProcInspectionRecord> completeInspectionList) {
        if (!ObjectUtils.isEmpty(procInspectionOne)) {
            ...
            updateProcInspection.setProcId(procInspectionOne.getProcId());
    ...
    public Result isInspectionDeviceAll(CompleteInspectionProcReq req) {
        ...
        if (!ObjectUtils.isEmpty(notInspection) && !ObjectUtils.isEmpty(inspectionRecordList)) {
            ...
            if (i != notInspection.size()) {
                //没有巡检完所有设施
                return ResultUtils.warn(WorkflowBusinessResultCode.INSPECTION_DEVICE_NOT_COMPLETE, I18nUtils.getSystemString(ProcInspectionI18n.INSPECTION_DEVICE_NOT_COMPLETE));
            }
    ...
    public boolean checkInspectionTaskStatusComplete(ProcInspection procInspectionOne) {
        ...
        //查询巡检任务
        List<ProcInspection> procInspectionList = this.selectNotInspectionProcInspection(procInspectionOne);
        if (!ObjectUtils.isEmpty(procInspectionList)) {
            if (1 == procInspectionList.size()) {
                ProcInspection procInspectionInfo = procInspectionList.get(0);
                if (!procId.equals(procInspectionInfo.getProcId())) {
                    return false;
                }
            } else {
                return false;
    ...
    public void completeProcInspectionDeviceRecord(CompleteInspectionProcReq completeReq) {
        ...
        if (WorkFlowBusinessConstants.COMPLETE_TYPE_COMMIT.equals(completeReq.getOperate())) {
            ...
            procInspectionParam.setRealityCompletedTime(new Date());
            procInspectionParam.setProcId(completeReq.getProcId());
            procInspectionParam.setUpdateTime(new Date());
            procInspectionParam.setUpdateUser(completeReq.getUserId());
            this.updateProcInspectionInfo(procInspectionParam);
    ...
    public Result exportListInspectionProcess(ExportDto exportDto) {
        ...
        try {
            exportRequestInfo = inspectionProcProcessExport.insertTask(exportDto, WorkFlowBusinessConstants.SERVER_NAME, I18nUtils.getSystemString(ProcBaseI18n.PROC_INSPECTION_PROCESS_LIST));
        } catch (FilinkExportNoDataException fe) {
            log.error("export inspection process no data exception", fe);
            ...
            log.error("export inspection process task num to big exception", fe);
            ...
            log.error("export inspection process exception", e);
        ...
        inspectionProcProcessExport.exportData(exportRequestInfo);
    ...
    public Result exportListInspectionComplete(ExportDto exportDto) {
        ExportRequestInfo exportRequestInfo = null;
        try {
            exportRequestInfo = inspectionProcCompleteExport.insertTask(exportDto, WorkFlowBusinessConstants.SERVER_NAME, I18nUtils.getSystemString(ProcBaseI18n.PROC_INSPECTION_COMPLETE_LIST));
        } catch (FilinkExportNoDataException fe) {
            log.error("export inspection complete no data exception", fe);
            return ResultUtils.warn(WorkflowBusinessResultCode.EXPORT_NO_DATA, I18nUtils.getSystemString(WorkflowBusinessI18n.EXPORT_NO_DATA));
        } catch (FilinkExportDataTooLargeException fe) {
            return WorkFlowBusinessMsg.getExportToLargeMsg(fe, maxExportDataSize);
        } catch (FilinkExportTaskNumTooBigException fe) {
            log.error("export inspection complete task num to big exception", fe);
            return ResultUtils.warn(WorkflowBusinessResultCode.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS, I18nUtils.getSystemString(ProcBaseI18n.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            log.error("export inspection complete task exception", e);
            return ResultUtils.warn(WorkflowBusinessResultCode.FAILED_TO_CREATE_EXPORT_TASK, I18nUtils.getSystemString(ProcBaseI18n.FAILED_TO_CREATE_EXPORT_TASK));
        }
        inspectionProcCompleteExport.exportData(exportRequestInfo);
        //新增日志
    ...
    public Result queryProcInspectionByProcInspectionId(ProcInspection procInspection) {
        ...
        procIdList.add(procInspection.getProcId());
        //查询工单的巡检记录信息
        List<ProcInspectionRecordVo> procInspectionRecordVoList = procInspectionRecordService.queryInspectionRecordByProcIds(procIdList);
        ...
        if (!ObjectUtils.isEmpty(resp)) {
            ...
            //已巡检设施百分比
            double completedPercent = CalculateUtil.calculatePercent(completedCount, deviceAllCount);
            //未巡检设施百分比
            ...
            if (!ObjectUtils.isEmpty(deviceAllCount) && !ObjectUtils.isEmpty(completedCount)) {
                ...
                //未巡检设施百分比
                processPercent = CalculateUtil.calculatePercent(processCount, deviceAllCount);
            }
            ...
            if (!ObjectUtils.isEmpty(procInspectionRecordVoList)) {
                ...
                for (ProcInspectionRecordVo procInspectionRecordVo : procInspectionRecordVoList) {
                    if (ProcInspectionConstants.INSPECTION_RESULT_SUCESS.equals(procInspectionRecordVo.getResult())) {
                        //正常的数量
                        normalDeviceCount = normalDeviceCount + 1;
                    } else {
                        //异常的数量
                        errorDeviceCount = errorDeviceCount + 1;
                    }
                }
                resp.setNormalInspectionCount(normalDeviceCount);
                resp.setErrorInspectionCount(errorDeviceCount);
                //巡检结果正常百分比
                normalPercent = CalculateUtil.calculatePercent(normalDeviceCount, deviceAllCount);
                //巡检结果异常百分比
                errorPercent = CalculateUtil.calculatePercent(errorDeviceCount, deviceAllCount);
            }
            resp.setInspectionCompletedPercent(completedPercent);
            resp.setInspectionProcessPercent(processPercent);
            resp.setNormalInspectionPercent(normalPercent);
            resp.setErrorInspectionPercent(errorPercent);
        }
     ...
     * @return 返回修改巡检工单状态的行数
     */
    @Override
    public int updateProcInspectionStatusById(ProcInspection procInspection) {
        return procInspectionDao.updateProcInspectionStatusById(procInspection);
    }
     ...
     */
    @Override
    public int updateProcInspectionRemarkById(ProcInspection procInspection) {
        return procInspectionDao.updateProcInspectionRemarkById(procInspection);
    }
     ...
     */
    @Override
    public int updateInspectionIsDeletedByIds(ProcBaseReq req) {
        return procInspectionDao.updateInspectionIsDeletedByIds(req);
    }
     ...
     */
    @Override
    public List<ProcBaseRespForApp> queryLoginUserInspectionListForApp(ProcBaseReq procBaseReq) {
        return procInspectionDao.queryLoginUserInspectionListForApp(procBaseReq);
    }
     ...
     */
    @Override
    public List<ProcBaseRespForApp> queryInspectionListForApp(ProcBaseReq procBaseReq) {
        return procInspectionDao.queryInspectionListForApp(procBaseReq);
    }
     ...
     */
    @Override
    public int updateProcInspectionAssignBatch(ProcBase procBase, List<String> list) {
        return procInspectionDao.updateProcInspectionAssignBatch(procBase, list);
    }
     ...
     */
    @Override
    public List<ProcInspection> queryInspectionProcListByDeptIds(List<String> deptIds) {
        return procInspectionDao.queryInspectionProcListByDeptIds(deptIds);
    }
     ...
     */
    @Override
    public List<ProcInspection> queryInspectionProcListByDeptIdsAndAreaId(List<String> deptIds, String areaId) {
        return procInspectionDao.queryInspectionProcListByDeptIdsAndAreaId(deptIds, areaId);
    }
     ...
     */
    @Override
    public List<ProcInspection> queryExistsProcForUserList(List<String> userIdList) {
        return procInspectionDao.queryExistsProcForUserList(userIdList);
    }
     ...
     */
    @Override
    public List<ProcInspection> queryInspectionRelateDeptByProcIds(List<String> list) {
        return procInspectionDao.queryInspectionRelateDeptByProcIds(list);
    }
     ...
     */
    @Override
    public int queryCountListProcInspectionUnfinishedProc(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryCountListProcInspectionUnfinishedProc(queryCondition);
    }
     ...
     */
    @Override
    public List<ProcBaseResp> queryListProcInspectionUnfinishedProcByPage(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryListProcInspectionUnfinishedProcByPage(queryCondition);
    }
     ...
     */
    @Override
    public int queryCountListProcInspectionHisProc(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryCountListProcInspectionHisProc(queryCondition);
    }
     ...
     */
    @Override
    public List<ProcBaseResp> queryListProcInspectionHisProcByPage(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryListProcInspectionHisProcByPage(queryCondition);
    }
    ...
    public List<ProcInspection> selectNotInspectionProcInspection(ProcInspection procInspection) {
        return procInspectionDao.selectNotInspectionProcInspection(procInspection);
    }
     ...
     */
    @Override
    public int queryCountListRelatedProcByInspectionTaskId(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryCountListRelatedProcByInspectionTaskId(queryCondition);
    }
     ...
     */
    @Override
    public List<ProcBaseResp> queryListRelatedProcByInspectionTaskIdPage(QueryCondition<ProcBaseReq> queryCondition) {
        return procInspectionDao.queryListRelatedProcByInspectionTaskIdPage(queryCondition);
    }
}
