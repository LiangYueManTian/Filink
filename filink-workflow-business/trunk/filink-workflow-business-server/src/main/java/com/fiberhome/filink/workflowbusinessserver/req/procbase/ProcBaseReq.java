...
public class ProcBaseReq extends ProcBase {
    ...
    public Long getLastDaysTime() {
        return lastDaysTime;
    }

    public void setLastDaysTime(Long lastDaysTime) {
        this.lastDaysTime = lastDaysTime;
    }

    /**
     * 获取开始查询条数
     */
    public Integer getStartQueryCount() {
        if (!ObjectUtils.isEmpty(this.pageNumber) && !ObjectUtils.isEmpty(this.batchDownloadProcNum)) {
            //计算条数
            Integer pageNumberValue = this.pageNumber - 1;
            return pageNumberValue * this.batchDownloadProcNum;
        }
        return 0;
    }
     ...
     * @return 返回工单查询条件
     */
    public static QueryCondition<ProcBaseReq> getParseQueryCondition(QueryCondition<ProcBaseReq> req) {
        if (!ObjectUtils.isEmpty(req.getBizCondition())) {
            ProcBaseReq procBaseReq = req.getBizCondition();
            String parseTitle = ProcBaseUtil.alterLikeValue(procBaseReq.getTitle());
            procBaseReq.setTitle(parseTitle);
            req.setBizCondition(procBaseReq);
        }
        return req;
    }
}
