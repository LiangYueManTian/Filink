...
import com.fiberhome.filink.workflowbusinessserver.controller.procbase.PerformaceTest;
...
public class ProcBaseServiceImpl extends ServiceImpl<ProcBaseDao, ProcBase> implements ProcBaseService {
    ...
    public Result addProcBase(ProcessInfo processInfo) {
        ...
        if (ProcBaseConstants.PROC_RESOURCE_TYPE_3.equals(processInfo.getProcBase().getProcResourceType())) {
            //todo 工单测试
            PerformaceTest.setWorkTime(processInfo.getProcBase().getDeviceId(),"saveEndTime",System.currentTimeMillis());
        }
...
//        获取权限信息
    ...
    public ProcessInfo assignProcProcess(AssignProcReq req, ProcBase procBase) {
        ...
        if (!ObjectUtils.isEmpty(procBase)) {
            ...
            if (ProcBaseConstants.PROC_INSPECTION.equals(procBase.getProcType())) {
                ...
                if (!ObjectUtils.isEmpty(req.getDepartmentList())) {
                    departmentId = req.getDepartmentList().get(0).getAccountabilityDept();
                ...
                String departmentName = "";
                if (!ObjectUtils.isEmpty(req.getDepartmentList())) {
                    List<String> departmentIdList = CastListUtil.getRelatedDepartmentIdList(req.getDepartmentList());
                    departmentName = procRelatedService.getDepartmentName(departmentIdList);
                }
                //关联部门的名称
                procBaseParam.setAccountabilityDeptName(departmentName);
                ...
                //修改销障工单
                ProcClearFailure procClearFailure = new ProcClearFailure();
                BeanUtils.copyProperties(procBaseParam, procClearFailure);
                procClearFailureService.updateClearFailure(procClearFailure);
    ...
    public List<String> getTurnRelatedDepartmentList(ProcBase procBase) {
        ...
        if (!ObjectUtils.isEmpty(procBase)) {
            if (ProcBaseConstants.PROC_INSPECTION.equals(procBase.getProcType())) {
                ...
                procIdList.add(procBase.getProcId());
                //查询关联部门
                List<ProcRelatedDepartment> procRelatedDepartmentList = this.queryProcRelateDeptByProcIds(procIdList);
                //关联部门编号集合
    ...
    public void saveProcRelate(ProcessInfo processInfo) {
        if (ProcBaseConstants.PROC_CLEAR_FAILURE.equals(processInfo.getProcBase().getProcType())) {
            ...
            procClearFailureParam.setProcId(processInfo.getProcBase().getProcId());
            procClearFailureParam.setIsDeleted(WorkFlowBusinessConstants.IS_NOT_DELETED);
            ProcClearFailure procClearFailure = procClearFailureService.selectClearFailureProcOne(procClearFailureParam);
            if (!ObjectUtils.isEmpty(processInfo.getProcRelatedDepartments())){
                Date createTime = null;
                if (!ObjectUtils.isEmpty(procClearFailure)) {
                    createTime = procClearFailure.getCreateTime();
                } else {
                ...
                for (int i = 0;i<processInfo.getProcRelatedDepartments().size();i++){
                    ...
                    processInfo.getProcRelatedDepartments().get(i).setProcCreateTime(createTime);
                }
                procBaseDao.addProcRelateDept(processInfo);
            }
    ...
    public List<ProcessInfo> queryProcRelate(List<ProcessInfo> processInfoList, Set<String> procIds, List<Department> departmentList, String methodType) {
        ...
        if (!ObjectUtils.isEmpty(procIds)) {
            ...
            //获取巡检工单关联部门信息
            procRelatedDepartments = ProcRelatedDepartment.getClearProcRelatedDepartment(processInfoList, procRelatedDepartments);
        ...
        if (!ObjectUtils.isEmpty(processInfo.getProcBase())) {
            if (!StringUtils.isEmpty(processInfo.getProcBase().getProcType())) {
                if (ProcBaseConstants.PROC_CLEAR_FAILURE.equals(processInfo.getProcBase().getProcType())) {
                    ...
                    String departmentName = procRelatedService.getRelatedDepartmentName(processInfo);
