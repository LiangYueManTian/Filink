...
public class ProcRelatedDepartment extends Model<ProcRelatedDepartment> {
     ...
     * @return 巡检工单关联部门信息
     */
    public static List<ProcRelatedDepartment> getClearProcRelatedDepartment(List<ProcessInfo> processInfoList, List<ProcRelatedDepartment> procRelatedDepartments) {
        ...
        if (!ObjectUtils.isEmpty(processInfoList) ) {
            for (ProcessInfo processInfo : processInfoList) {
                if (ProcBaseConstants.PROC_INSPECTION.equals(processInfo.getProcBase().getProcType())) {
