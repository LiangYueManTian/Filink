...
public class AlarmProcessServiceImpl implements AlarmProcessService {
    ...
    public Result procTimeOutCreateAlarm() {
        ...
        if (!ObjectUtils.isEmpty(procBaseInfoBeanList)) {
            for (ProcBaseInfoBean procBaseOne : procBaseInfoBeanList) {
                ...
                if (ProcBaseConstants.PROC_STATUS_ASSIGNED.equals(procBase.getStatus())) {
                    if (!ObjectUtils.isEmpty(procBase.getCreateUser())) {
                        createList.add(procBase.getCreateUser());
                    }
                }
            }
        }
        ...
        if (!ObjectUtils.isEmpty(procBaseInfoBeanMap)) {
            ...
            //修改工单告警已经生成
            procBaseDao.updateCreateAlarmInfo(procIds, nowDate);
        }
     ...
     * 流程列表map
     *
     * @param procRelatedDepartmentList 流程关联部门
     * @param procBaseInfoBeanMap       流程列表map
     * @return 流程列表map
     * @author hedongwei@wistronits.com
     * @date 2019/3/27 12:25
     */
