...
public class InspectionTaskServiceImpl extends ServiceImpl<InspectionTaskDao, InspectionTask> implements InspectionTaskService {
    ...
    public Result insertInspectionTask(InsertInspectionTaskReq req) {
        ...
        if (!this.insertInspectionTaskProcess(req)) {
            String inspectionTaskMsg = I18nUtils.getString(InspectionTaskI18n.FAIL_INSERT_INSPECTION_TASK);
            //提示新增巡检任务失败
            return ResultUtils.warn(InspectionTaskResultCode.FAIL_INSERT_INSPECTION_TASK, inspectionTaskMsg);
        }

        String successInsertInspection = I18nUtils.getString(InspectionTaskI18n.SUCCESS_INSERT_INSPECTION_TASK);
        //提示新增巡检任务成功
    ...
    public Result updateInspectionTask(UpdateInspectionTaskReq req) {
        ...
        if (ObjectUtils.isEmpty(inspectionTaskInfo)) {
            //不存在提示信息
            String notExistMsg = I18nUtils.getString(InspectionTaskI18n.INSPECTION_TASK_NOT_EXIST);
            //提示巡检任务不存在不能修改巡检任务数据
        ...
        if (!this.updateInspectionTaskProcess(req)) {
            //提示修改失败
            String updateError = I18nUtils.getString(WorkflowBusinessI18n.UPDATE_FAIL);
            return ResultUtils.warn(WorkflowBusinessResultCode.UPDATE_FAIL, updateError);
        }

        //修改巡检任务的提示信息
        String successUpdateInspection = I18nUtils.getString(WorkflowBusinessI18n.UPDATE_SUCCESS);
    ...
    public Result deleteInspectionTaskByIds(DeleteInspectionTaskByIdsReq req) {
        ...
        //修改巡检任务的提示信息
        String successUpdateInspection = I18nUtils.getString(WorkflowBusinessI18n.DELETE_SUCCESS);
        //返回提示信息
        return ResultUtils.success(ResultCode.SUCCESS, successUpdateInspection);
    }
        ...
        if (!ObjectUtils.isEmpty(list)) {
            ...
            if (!ObjectUtils.isEmpty(taskList)) {
                if (list.size() != taskList.size()) {
                    return ResultUtils.warn(InspectionTaskResultCode.EXIST_IS_DELETED_TASK_DATA, I18nUtils.getString(InspectionTaskI18n.EXIST_IS_DELETED_TASK_DATA));
                }
                return null;
            } else {
                return WorkFlowBusinessMsg.paramErrorMsg();
    ...
    public QueryCondition<QueryListInspectionTaskByPageReq> getInspectionTaskCondition(QueryCondition queryCondition, boolean isExport) {
        ...
        if (isExport) {
            procBaseReq = (ProcBaseReq)procBaseService.getPermissionsInfoForExport(procBaseReqCondition).getBizCondition();
        } else {
            procBaseReq = (ProcBaseReq)procBaseService.getPermissionsInfo(procBaseReqCondition).getBizCondition();
        }
    ...
    public Result openInspectionTaskBatch(OpenInspectionTaskBatchReq req) {
        ...
        //提示巡检任务开启成功
        String inspectionTaskOpenSuccess = I18nUtils.getString(InspectionTaskI18n.INSPECTION_TASK_OPEN_SUCCESS);
    ...
    public Result closeInspectionTaskBatch(CloseInspectionTaskBatchReq req) {
        ...
        //提示巡检任务关闭成功
        String inspectionTaskCloseSuccess = I18nUtils.getString(InspectionTaskI18n.INSPECTION_TASK_CLOSE_SUCCESS);
    ...
    public Result exportInspectionTask(ExportDto exportDto) {
        Export export = null;
        try {
            export = inspectionTaskListExport.insertTask(exportDto, WorkFlowBusinessConstants.SERVER_NAME, I18nUtils.getString(InspectionTaskI18n.INSPECTION_TASK_LIST));
        ...
        inspectionTaskListExport.exportData(export);
        //新增日志
    ...
    public void updateInspectionTaskCreateJob(UpdateInspectionTaskReq req, String inspectionTaskId) {
        ...
        if (!ObjectUtils.isEmpty(req.getTaskStartDate()) && !ObjectUtils.isEmpty(req.getTaskStartDate()) && !ObjectUtils.isEmpty(req.getTaskPeriod())) {
