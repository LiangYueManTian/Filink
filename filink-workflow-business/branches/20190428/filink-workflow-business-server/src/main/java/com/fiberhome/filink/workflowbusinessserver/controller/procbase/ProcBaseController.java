...
public class ProcBaseController {
    ...
    public Result queryTitleIsExists(@RequestBody ProcBase procBase) {
        if (StringUtils.isEmpty(procBase.getTitle())) {
            return ResultUtils.warn(ProcBaseResultCode.PROC_PARAM_ERROR, I18nUtils.getString(ProcBaseI18n.PROC_NAME_NULL));
        }
        //新增及修改校验
        boolean isExist = procBaseService.queryTitleIsExists(procBase.getProcId(), procBase.getTitle());
        if (isExist) {
            return ResultUtils.warn(ProcBaseResultCode.PROC_PARAM_ERROR, I18nUtils.getString(ProcBaseI18n.PROC_NAME_SAME));
        }
        return ResultUtils.success(ProcBaseResultCode.SUCCESS, I18nUtils.getString(ProcBaseI18n.PROC_NAME_AVAILABLE));
    }
    ...
    public Map<String, Object> queryExistsProcForAlarmList(@RequestBody List<String> alarmProcList) {
        if (!StringUtils.isEmpty(alarmProcList)) {
            return procBaseService.queryExistsProcForAlarmList(alarmProcList);
        } else {
            return null;
        }
    }
