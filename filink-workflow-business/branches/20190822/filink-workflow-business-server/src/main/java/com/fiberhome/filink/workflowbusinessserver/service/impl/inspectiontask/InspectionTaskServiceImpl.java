...
public class InspectionTaskServiceImpl extends ServiceImpl<InspectionTaskDao, InspectionTask> implements InspectionTaskService {
    ...
    public Result insertInspectionTask(InsertInspectionTaskReq req) {
        ...
        if (!this.returnInspectionTaskNameResult(inspectionTask)) {
            //提示巡检任务名称重复
            return InspectionTaskMsg.getInspectionNameIsRepeatMsg();
        }
        ...
        if (!this.insertInspectionTaskProcess(req)) {
            ...
            //提示新增巡检任务失败
            return ResultUtils.warn(WorkflowBusinessResultCode.FAIL_INSERT_INSPECTION_TASK, inspectionTaskMsg);
        }
    ...
    public Result updateInspectionTask(UpdateInspectionTaskReq req) {
        ...
        if (ObjectUtils.isEmpty(inspectionTaskInfo)) {
            ...
            //提示巡检任务不存在不能修改巡检任务数据
            return ResultUtils.warn(WorkflowBusinessResultCode.INSPECTION_TASK_NOT_EXISTS, notExistMsg);
        }
        ...
        if (!ObjectUtils.isEmpty(list)) {
            ...
            if (!ObjectUtils.isEmpty(taskList)) {
                if (list.size() != taskList.size()) {
                    return ResultUtils.warn(WorkflowBusinessResultCode.EXIST_IS_DELETED_TASK_DATA, I18nUtils.getSystemString(InspectionTaskI18n.EXIST_IS_DELETED_TASK_DATA));
                }
                return null;
            } else {
                return WorkFlowBusinessMsg.paramErrorMsg();
    ...
    public Result queryListInspectionTaskByPage(QueryCondition<QueryListInspectionTaskByPageReq> queryCondition) {
        ...
        //转义查询字符
        queryCondition = QueryListInspectionTaskByPageReq.getParseTaskReq(queryCondition);
    ...
    public Result getInspectionTaskById(String id) {
        ...
        if (null != inspectionTask) {
            ...
            //开始时间
            if (!ObjectUtils.isEmpty(inspectionTaskByIdVo.getTaskStartTime())) {
                inspectionTaskByIdVo.setStartTime(inspectionTaskByIdVo.getTaskStartTime().getTime());
            }
            //结束时间
            if (!ObjectUtils.isEmpty(inspectionTaskByIdVo.getTaskEndTime())) {
                inspectionTaskByIdVo.setEndTime(inspectionTaskByIdVo.getTaskEndTime().getTime());
            }
        }
    ...
    public InspectionTaskBizBean generateInspectionTaskBizBean(InspectionTask inspectionTask, String jobTaskName) {
        ...
        if (!ObjectUtils.isEmpty(inspectionTask.getTaskStartTime())) {
            taskBiz.setStartTime(inspectionTask.getTaskStartTime().getTime());
        }
