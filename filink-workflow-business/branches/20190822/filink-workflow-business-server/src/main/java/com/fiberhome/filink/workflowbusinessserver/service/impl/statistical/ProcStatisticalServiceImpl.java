...
public class ProcStatisticalServiceImpl implements ProcStatisticalService {
     ...
     * @param deviceInfoDto 设施信息
     * @param procRelatedDepartments 关联部门
     * @param procParamBase
     */
    public void addInsertInspectionTest(List<ProcRelatedDevice> deviceInfoDto, List<ProcRelatedDepartment> procRelatedDepartments, ProcBase procParamBase) {
        //生成数据数量
        int generateNumber = 150000;
        ...
        for (int i = 0 ; i < generateNumber ; i++) {
            ...
            procInspection.setProcId(NineteenUUIDUtils.uuid());
            String procType = "";
            procType = ProcBaseConstants.PROC_CLEAR_FAILURE;

            procInspection.setProcType(procType);
            ...
            procInspection.setStatus(statusName);
            procInspection.setExpectedCompletedTime(new Date());
            procInspection.setProcResourceType(ProcBaseConstants.PROC_RESOURCE_TYPE_1);
            String errorReason = "";
            if (i % 5 == 0) {
            ...
            procInspection.setErrorReason(errorReason);
            ...
            procInspection.setProcessingScheme(processingScheme);

            if (i % 2 == 0) {
                procInspection.setSingleBackReason("1");
            } else if (i % 2 == 1) {
                procInspection.setSingleBackReason("0");
                procInspection.setSingleBackUserDefinedReason("testInfo");
            }
            procInspection.setTitle("工单性能测试数据" + i);
            ...
            for (ProcRelatedDevice procRelatedDeviceOne : deviceInfoDto) {
                if (deviceType.equals(procRelatedDeviceOne.getDeviceType())) {
                    ...
                    procRelatedDeviceInfo.setProcId(procInspection.getProcId());
                    areaId = procRelatedDeviceInfo.getDeviceAreaId();
                    areaName = procRelatedDeviceInfo.getDeviceAreaName();
        ...
        for (int i = 0; i < addDeviceInfo.size(); i++) {
            ...
            if (addDeviceInfo2.size() >= 10000 || i+1==addDeviceInfo.size()) {
                if (i + 1 <= 750000) {
                    addProcData.addProcRelatedDevice(addDeviceInfo2);
        ...
        for (int i = 0; i < procInspectionList.size(); i++) {
            ...
            if (procInspectionList2.size() > 10000 || i+1==  procInspectionList.size()) {
                addProcData.addProcInspection(procInspectionList2);
        ...
        for (int i = 0; i <addInspectionRecordInfo.size(); i++) {
            ...
            if (addInspectionRecordInfo2.size() >= 10000 || i+1==  addInspectionRecordInfo.size()) {
                if (i + 1 <= 750000) {
                    addProcData.addInspectionRecord(addInspectionRecordInfo2);
     ...
     * @param deviceInfoDto 设施信息
     * @param procRelatedDepartments 关联部门
     * @param procParamBase 工单参数
     */
    public void addInsertClearTest(List<ProcRelatedDevice> deviceInfoDto, List<ProcRelatedDepartment> procRelatedDepartments, ProcBase procParamBase) {
        int generateNumber = 2000000;
        int testNumber = 20000;
        ...
        for (int i = 0; i < generateNumber; i ++) {
            ...
            procClearFailure.setProcId(NineteenUUIDUtils.uuid());
            String procType = "";
            procType = ProcBaseConstants.PROC_CLEAR_FAILURE;

            procClearFailure.setProcType(procType);
            ...
            procClearFailure.setStatus(statusName);
            procClearFailure.setExpectedCompletedTime(new Date());
            procClearFailure.setProcResourceType(ProcBaseConstants.PROC_RESOURCE_TYPE_1);
            String errorReason = "";
            if (i % 5 == 0) {
                errorReason = ProcStatisticalConstants.errorReasonList.get(0);
            } else if (i % 5 == 1) {
            ...
            procClearFailure.setErrorReason(errorReason);
            ...
            procClearFailure.setProcessingScheme(processingScheme);

            if (i % 2 == 0) {
                procClearFailure.setSingleBackReason("1");
            } else if (i % 2 == 1) {
                procClearFailure.setSingleBackReason("0");
                procClearFailure.setSingleBackUserDefinedReason("testInfo");
            }
            procClearFailure.setTitle("工单性能测试数据" + i);
            ...
            for (ProcRelatedDevice procRelatedDeviceOne : deviceInfoDto) {
                if (deviceType.equals(procRelatedDeviceOne.getDeviceType())) {
                    ProcRelatedDevice procRelatedDeviceInfo = new ProcRelatedDevice();
                    BeanUtils.copyProperties(procRelatedDeviceOne, procRelatedDeviceInfo);
                    areaId = procRelatedDeviceOne.getDeviceAreaId();
                    areaName = procRelatedDeviceOne.getDeviceAreaName();
                    deviceId = procRelatedDeviceOne.getDeviceId();
                    deviceName = procRelatedDeviceOne.getDeviceName();
                    procRelatedDeviceInfo.setProcRelatedDeviceId(NineteenUUIDUtils.uuid());
                    procRelatedDeviceInfo.setProcId(procClearFailure.getProcId());
            ...
            //默认单个部门的值
            ProcRelatedDepartment procTestDepartment = new ProcRelatedDepartment();
            procTestDepartment.setAccountabilityDept("OWGOmNN1JEDkjBbXofr");
            ProcRelatedDepartment departmentInfo = new ProcRelatedDepartment();

            if (i < testNumber) {
                departmentInfo = procTestDepartment;
            } else if (i % 5 == 0) {
                departmentInfo = procRelatedDepartments.get(0);
            } else if (i % 5 == 1) {
                departmentInfo = procRelatedDepartments.get(1);
            ...
            //关联部门
            procClearFailure.setAccountabilityDept(departmentInfo.getAccountabilityDept());



            //默认关联区域信息
            String defaultAreaId = "wdRHbr6VwS0iad2Iiz8";

            String defaultAreaName = "test_area_01";

            if (i < testNumber) {
                procClearFailure.setDeviceAreaId(defaultAreaId);
                procClearFailure.setDeviceAreaName(defaultAreaName);
            } else {
            ...
            procClearFailure.setIsCreateAlarm("1");
            procClearFailure.setCreateTime(new Date());
            procClearFailure.setRefAlarm(procParamBase.getRefAlarm());
            procClearFailure.setRefAlarmCode(procParamBase.getRefAlarmCode());
            procClearFailure.setRefAlarmName(procParamBase.getRefAlarmName());
        ...
        for (int i = 0; i < procClearFailureList.size(); i++ ) {
            ...
            if (procClearFailures.size() >= 10000 || i+1== procClearFailureList.size()) {
                addProcData.addProcClearFailure(procClearFailures);
    ...
    public void insertClearFailureTest() {
        ...
        this.addInsertClearTest(deviceInfoDto, procRelatedDepartments, procBase);
    }
    ...
    public void insertInspectionTest() {
        ...
        procBase.setRefAlarmName("非法开门");
        procBase.setRefAlarmCode("violenceClose");
        this.addInsertInspectionTest(deviceInfoDto, procRelatedDepartments, procBase);
    }
