...
public class ProcBaseExceptionHandler extends GlobalExceptionHandler {
	...
	public Result handleFilinkObtainDeptInfoException(FilinkObtainDeptInfoException ex) {
		log.error("获取部门信息异常", ex);
		return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getSystemString(ProcBaseI18n.FAILED_TO_OBTAIN_DEPARTMENT_INFORMATION));
	}
	...
	public Result handleFilinkObtainDeviceInfoException(FilinkObtainDeviceInfoException ex) {
		...
		return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getSystemString(ProcBaseI18n.FAILED_TO_OBTAIN_DEVICE_INFORMATION));
	}
}
