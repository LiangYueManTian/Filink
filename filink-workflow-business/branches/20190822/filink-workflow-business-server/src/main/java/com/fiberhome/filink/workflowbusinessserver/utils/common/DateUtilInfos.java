...
public class DateUtilInfos {
     ...
     * @param date
     */
    public static boolean checkDateIsMonday(Long date) {
        Calendar calendar = new GregorianCalendar();
        calendar.setTimeInMillis(date);
        int weekDay = calendar.get(Calendar.DAY_OF_WEEK);
        int monDayInfo = 2;
        if (monDayInfo == weekDay) {
            //是星期一
            return true;
        }
        //不是星期一
     ...
     * @return 获取周统计时间信息
     */
    public static List<Long> getWeekDateList(Long startDate, int week, Long endDate) {
        boolean startDateIsMonday = DateUtilInfos.checkDateIsMonday(startDate);
        boolean endDateIsMonday = DateUtilInfos.checkDateIsMonday(endDate);
        List<Long> weekList = new ArrayList<>();
        if (week >= 0) {
            Calendar calendar = null;
            for (int i = 0; i <= week; i++) {
                //开始时间是否为周一,不为周一当周不会被计算
                if (0 == i && !startDateIsMonday) {
                    continue;
                }

                //结束时间是否为周一，不为周一当周不会被计算
                if (i == week - 1 && !endDateIsMonday) {
                    break;
                }
    ...
    public static int getWeekNum(Long startTime, Long endTime) {
        Calendar beforeStartDate = new GregorianCalendar();
        beforeStartDate.setTimeInMillis(startTime);
        beforeStartDate.add(Calendar.DATE, -1);
        Long startDateTime = beforeStartDate.getTimeInMillis();
        ...
        Calendar beforeEndDate = new GregorianCalendar();
        beforeEndDate.setTimeInMillis(endTime);
        beforeEndDate.add(Calendar.DATE, -1);
        Long beforeDateTime = beforeEndDate.getTimeInMillis();

        //结束周
        Calendar endDate = new GregorianCalendar();
        endDate.setTimeInMillis(beforeDateTime);
