...
public class CastStatisticalUtil {
    ...
    public static void main(String [] args) {
        ...
        NoticeProcInfo info = (NoticeProcInfo) aliYunPushMsgBean.getData();
        System.out.println(info.getTitle());

        /*Calendar date = new GregorianCalendar();
        date.setTimeInMillis(1562253314000L);
        // 2、取得时间偏移量：
        int zoneOffset = date.get(java.util.Calendar.ZONE_OFFSET);

        // 3、取得夏令时差：
        int dstOffset = date.get(java.util.Calendar.DST_OFFSET);

        // 4、从本地时间里扣除这些差量，即可以取得UTC时间：
        date.add(java.util.Calendar.MILLISECOND, -(zoneOffset + dstOffset));
