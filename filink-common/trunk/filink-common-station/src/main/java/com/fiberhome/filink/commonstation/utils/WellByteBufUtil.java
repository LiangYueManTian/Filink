...
public class WellByteBufUtil {
    ...
    public static void setDataBuf(String type, String dataSource, Integer length, ByteBuf dataBuf) {
        ...
        if (DataType.INT.equalsIgnoreCase(type) || DataType.UNSIGNED_INT.equalsIgnoreCase(type)) {
            ...
            if (length == BYTE_LENGTH) {
                dataBuf.writeByte(WellDataUtil.intToByte1(dataSourceInt)[0]);
            } else if (length == SHORT_LENGTH) {
                dataBuf.writeBytes(WellDataUtil.intToByte2(dataSourceInt));
            } else if (length == INT_LENGTH) {
                dataBuf.writeBytes(WellDataUtil.intToByte4(dataSourceInt));
            } else {
                logger.error("dataSourceLength is error!!!!!!!!!!!!!!!!");
            }
            //String类型
        } else if ("normalInt".equalsIgnoreCase(type)) {
            int dataSourceInt = Integer.decode(dataSource);
            if (length == BYTE_LENGTH) {
        ...
        } else if ("hexString".equalsIgnoreCase(type)) {
            dataBuf.writeBytes(HexUtil.hexStringToByte(dataSource));
        } else {
    ...
    public static String getIntDataSource(Integer length, ByteBuf dataBuf) {
        return String.valueOf(bytes2ToInt(dataBuf, length));
    }
    ...
    public static int bytes2ToInt(ByteBuf byteBuf, int length) {
