...
import com.fiberhome.filink.scheduleserver.job.ProcCountWeekStatisticalJob;
...
@Component
@Order(value = 21)
public class ProcCountWeekStatisticalInitRunner implements ApplicationRunner {

    @Autowired
    private JobService jobService;

    private static final String JOB_NAME = "procCountWeekStatistical";

    private static final String TRIGGER_NAME = "procCountWeekStatisticalTrigger";
    ...
    public void run(ApplicationArguments applicationArguments) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(taskInfoList)) {
            for (TaskInfo taskInfoOne : taskInfoList) {
                if (ProcCountWeekStatisticalInitRunner.JOB_NAME.equals(taskInfoOne.getJobName())) {
                    //已存在任务，不新增任务
                    return;
                }
            }
        ...
        taskInfo.setJobName(ProcCountWeekStatisticalInitRunner.JOB_NAME);
        taskInfo.setTriggerGroup(JobTriggerEnum.STATISTICS);
        taskInfo.setTClass(ProcCountWeekStatisticalJob.class);
        taskInfo.setCron(JobCronEnum.MONDAY_TWO_CLOCK);
        taskInfo.setTriggerName(ProcCountWeekStatisticalInitRunner.TRIGGER_NAME);
