...
import com.fiberhome.filink.scheduleserver.job.AlarmSourceIncrementalMonthJob;
...
@Component
@Order(value = 14)
public class AlarmSourceIncrementalMonthRunner implements ApplicationRunner {
     ...
     * 定时任务名称
     */
    private static final String JOB_NAME = "INCREMENTAL_EXPIRE_MONTH";
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(taskInfoList)) {
            for (TaskInfo taskInfoOne : taskInfoList) {
                if (AlarmSourceIncrementalMonthRunner.JOB_NAME.equals(taskInfoOne.getJobName())) {
                    //已存在任务，删除任务
                    jobService.deleteJob(JOB_NAME, JobGroupEnum.ALARM_SOURCE_INCREMENTAL_MONTH);
                }
            }
        }
        ...
        taskInfo.setJobGroup(JobGroupEnum.ALARM_SOURCE_INCREMENTAL_MONTH);
        taskInfo.setJobName(AlarmSourceIncrementalMonthRunner.JOB_NAME);
        taskInfo.setTriggerGroup(JobTriggerEnum.ALARM_SOURCE_INCREMENTAL_MONTH);
        taskInfo.setTClass(AlarmSourceIncrementalMonthJob.class);
        taskInfo.setCron(JobCronEnum.EVERY_MONTH_FIRST_HALF_PASS_TWO);
        taskInfo.setTriggerName(AlarmSourceIncrementalMonthRunner.TRIGGER_NAME);
