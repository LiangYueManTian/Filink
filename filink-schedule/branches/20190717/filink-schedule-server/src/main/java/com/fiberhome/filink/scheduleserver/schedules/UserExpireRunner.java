...
import com.fiberhome.filink.scheduleserver.job.UserExpireTaskJob;
...
@Component
@Order(value = 5)
public class UserExpireRunner implements ApplicationRunner {
     ...
     * 定时任务名称
     */
    private static final String JOB_NAME = "USER_EXPIRE";

    /**
     * 定时任务触发器名称
     */
    private static final String TRIGGER_NAME = "userExpireTrigger";
    ...
    public void run(ApplicationArguments applicationArguments) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(taskInfoList)) {
            for (TaskInfo taskInfoOne : taskInfoList) {
                if (UserExpireRunner.JOB_NAME.equals(taskInfoOne.getJobName())) {
//                    //已存在任务，不会新建任务
//                    return;
        ...
        taskInfo.setJobName(UserExpireRunner.JOB_NAME);
        taskInfo.setTriggerGroup(JobTriggerEnum.USEREXPIRE);
        taskInfo.setTClass(UserExpireTaskJob.class);
        taskInfo.setCron(JobCronEnum.TWO_HOUR);
        taskInfo.setTriggerName(UserExpireRunner.TRIGGER_NAME);
