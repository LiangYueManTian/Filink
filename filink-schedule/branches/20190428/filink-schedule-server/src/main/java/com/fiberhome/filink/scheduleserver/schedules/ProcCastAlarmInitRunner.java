...
import com.fiberhome.filink.scheduleserver.job.ProcCastAlarmJob;
...
@Component
@Order(value = 1)
public class ProcCastAlarmInitRunner implements ApplicationRunner {

    @Autowired
    private JobService jobService;

    private static final String JOB_NAME = "procCastAlarm";

    private static final String TRIGGER_NAME = "procCastAlarmTrigger";
    ...
    public void run(ApplicationArguments applicationArguments) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(taskInfoList)) {
            for (TaskInfo taskInfoOne : taskInfoList) {
                if (ProcCastAlarmInitRunner.JOB_NAME.equals(taskInfoOne.getJobName())) {
                    //已存在任务，不新增任务
                    return;
                }
            }
        ...
        taskInfo.setJobGroup(JobGroupEnum.PROC);
        taskInfo.setJobName(ProcCastAlarmInitRunner.JOB_NAME);
        taskInfo.setTriggerGroup(JobTriggerEnum.PROC);
        taskInfo.setTClass(ProcCastAlarmJob.class);
        taskInfo.setCron(JobCronEnum.TEN_MINUTE);
        taskInfo.setTriggerName(ProcCastAlarmInitRunner.TRIGGER_NAME);
