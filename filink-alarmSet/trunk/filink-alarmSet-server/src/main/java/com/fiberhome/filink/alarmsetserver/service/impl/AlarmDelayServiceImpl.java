...
public class AlarmDelayServiceImpl extends ServiceImpl<AlarmDelayDao, AlarmDelay> implements AlarmDelayService {
    ...
    public Result selectAlarmDelay() {
        ...
        RedisUtils.set(alarmDelay.getId() + AppConstant.ALARM_REDIS_DELAY, alarmDelay);
    ...
    public Result updateAlarmDelay(AlarmDelay alarmDelay) {
        ...
        if (updateById != 1) {
            return ResultUtils.warn(AlarmSetResultCode.UPDATE_TIAM_FAILED, I18nUtils.getSystemString(AppConstant.UPDATE_TIAM_FAILED));
        }
        RedisUtils.remove(alarmDelay.getId() + AppConstant.ALARM_REDIS_DELAY);
        updateLog(alarmDelay);
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getSystemString(AppConstant.UPDATE_TIAM_SUCCESS));
    }
     ...
     */
    private void updateLog(AlarmDelay alarmDelay) {
        systemLanguageUtil.querySystemLanguage();
        // 获取日志类型
        String logType = LogConstants.LOG_TYPE_OPERATE;
        AddLogBean addLogBean = logProcess.generateAddLogToCallParam(logType);
        addLogBean.setDataId(AppConstant.ALARM_ID);
        addLogBean.setDataName(AppConstant.DELAY);
