...
public class AlarmSetController {
    ...
    public Result updateAlarmCurrentSet(@RequestBody AlarmName alarmName) {
        if (alarmName.getId() == null) {
            return ResultUtils.success(AlarmSetResultCode.ALARM_NAME_ID_NULL,
                    I18nUtils.getSystemString(AppConstant.ALARM_NAME_ID_NULL));
        }
    ...
    public Result updateAlarmLevel(@RequestBody AlarmLevel alarmLevel) {
        ...
        // 参数验证
        if (alarmLevel.getPlayCount() < AlarmSetConstant.ALARM_MIN
                || alarmLevel.getPlayCount() > AlarmSetConstant.PLAY_WU) {
            return ResultUtils.success(AlarmSetResultCode.PLAY_TIMES_WRONG,
                    I18nUtils.getSystemString(AppConstant.PLAY_TIMES_WRONG));
        }
        if (alarmLevel.getId() == null) {
            return ResultUtils.success(AlarmSetResultCode.ALARM_LEVEL_ID_NULL,
                    I18nUtils.getSystemString(AppConstant.ALARM_LEVEL_ID_NULL));
        }
    ...
    public Result updateAlarmDelay(@RequestBody AlarmDelay alarmDelay) {
        ...
        if (alarmDelay.getDelay() <= AlarmSetConstant.ALARM_MIN || alarmDelay.getDelay() > AlarmSetConstant.ALARM_DELAY_MAX) {
            return ResultUtils.success(AlarmSetResultCode.INCORRECT_RANGE_VALUE, I18nUtils.getSystemString(AppConstant.INCORRECT_RANGE_VALUE));
        }
