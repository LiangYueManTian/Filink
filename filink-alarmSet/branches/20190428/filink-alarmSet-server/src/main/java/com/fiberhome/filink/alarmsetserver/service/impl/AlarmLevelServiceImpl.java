...
public class AlarmLevelServiceImpl extends ServiceImpl<AlarmLevelDao, AlarmLevel> implements AlarmLevelService {
    ...
    public Result queryAlarmLevelList() {
        ...
        log.info("key = " + RedisUtils.hasKey(AppConstant.REDIS_LEVEL));
        RedisUtils.set(AppConstant.REDIS_LEVEL, alarmLevel);
    ...
    public Result updateAlarmLevel(AlarmLevel alarmLevel) {
        ...
        if (alarmLevelTwo != null) {
            return ResultUtils.success(LogFunctionCodeConstant.COLOR_USED, I18nUtils.getString(AppConstant.COLOR_USED));
        }
        Integer result = alarmLevelDao.updateById(alarmLevel);
        if (result != 1) {
            throw new FilinkAlarmDelayException(I18nUtils.getString(AppConstant.SET_UP_FAILED));
        }
        RedisUtils.remove(AppConstant.REDIS_LEVEL);
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(AppConstant.SET_UP_SUCCESS));
    }
    ...
    public Result queryAlarmLevelById(String alarmId) {
        ...
        if (RedisUtils.hasKey(AppConstant.REDIS_LEVEL)) {
            alarmLevels = (List<AlarmLevel>) RedisUtils.get(AppConstant.REDIS_LEVEL);
            // 判断缓存是否有数据
        ...
        if (alarmLevel == null) {
            return ResultUtils.success(ResultCode.FAIL, I18nUtils.getString(AppConstant.ALARM_ID_FAILED));
        }
        return ResultUtils.success(alarmLevel);
    }
    ...
    public Result queryAlarmLevel() {
        ...
        if (ListUtil.isEmpty(alarmLevels)) {
            return ResultUtils.success(ResultCode.FAIL, I18nUtils.getString(AppConstant.ALARM_LEVEL_NULL));
        }
    ...
    public AlarmLevel queryAlarmLevelSetFeign(String alarmLevelCode) {
        AlarmLevel alarmLevel = alarmLevelDao.queryAlarmLevelSetFeign(alarmLevelCode);
        return alarmLevel;
    }
}
