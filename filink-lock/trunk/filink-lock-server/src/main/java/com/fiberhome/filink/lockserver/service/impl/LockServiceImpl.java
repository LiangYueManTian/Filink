...
public class LockServiceImpl implements LockService {
    ...
    public Result openLockByQrCode(QrCodeBean qrCodeBean) {
        ...
        if (FALSE.equals(permission)) {
            throw new FiLinkOpenLockException("don't have permission to open lock");
        }
        ...
        return ResultUtils.success(LockResultCode.SUCCESS,
                I18nUtils.getSystemString(LockI18n.OPEN_LOCK_SUCCESS));
    ...
    public Result openLockForPda(OpenLockBean openLockBean) {
        ...
        if (FALSE.equals(permission)) {
            throw new FiLinkOpenLockException("don't have perssion to open lock");
        }
    ...
    public Result queryLockStatusByDeviceId(OpenLockBean lockBean) {
        ...
        //查询门锁信息
        LockStatusForPda lockStatusForPda = queryLockAndControlByDeviceId(deviceId);
        //指定设施的门锁信息不需要设施名称的显示
    ...
    public Result operateControlAndLock(Control control) {
        ...
        if (operateCode.equals(OPERATE_ADD)) {
            ...
            try {
                ...
                log.info("[addControl] : app add the control {}", controlParam.getHostId());
            } catch (Exception e) {
                //添加失败 ，删除平台设备注册信息
    ...
    public List<String> queryDeviceIdListOfLock() {
        return lockDao.queryDeviceIdListOfLock();
    }
     ...
     * @return 门锁状态
     */
    private LockStatusForPda queryLockAndControlByDeviceId(String deviceId) {
    ...
    private void checkAddConstraintOfControl(String deviceId, ControlParam controlParam, List<Lock> locks) {
        ...
        if (!StringUtils.isEmpty(lockList)) {
            ...
            if (oldDoorNameList.size() > 0) {
                throw new FiLinkDeviceDoorNameReusedException();
            }
        }
    ...
    private Result setPlatformId(String deviceName, ControlParam controlParam) {
        ...
        if (!ObjectUtils.isEmpty(cloudPlatform)) {
            if (cloudPlatform.equals(OceanConnect.getCode())) {
                ...
                CreateDevice createDevice = new CreateDevice(title, controlParam.getImei(),
                        controlParam.getImsi(), controlParam.getProductId());
    ...
    private void updateControlAndLock(ControlParam controlParam, DeviceInfoDto deviceInfo, List<Lock> locks) {
        ...
        //设施该主控下的原有映射
        List<Lock> oldLockListWithTheControl = oldLockList.stream().
                filter((Lock lock) -> lock.getControlId().equals(controlParam.getHostId())).collect(Collectors.toList());
        //设施其他主控下的原有映射
        ...
        //分组
        inspectLockList(locks, updateLockList, addLockList, doorNumList, oldDoorNumList);
     ...
     *
     * @param locks          锁集合
     * @param updateLockList 要更新的锁的集合
     * @param addLockList    要添加的锁的集合
     * @param doorNumList    门编号集合
     * @param oldDoorNumList 已有门编号的集合
     */
    private void inspectLockList(List<Lock> locks, List<Lock> updateLockList,
                                 List<Lock> addLockList, List<String> doorNumList, List<String> oldDoorNumList) {
        //获得当前时间戳
        long currentTime = System.currentTimeMillis();
        for (Lock lock : locks) {
