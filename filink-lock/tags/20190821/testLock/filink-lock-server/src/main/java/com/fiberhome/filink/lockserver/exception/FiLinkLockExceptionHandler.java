...
public class FiLinkLockExceptionHandler {
     ...
     * @return Result
     */
    @ExceptionHandler(FiLinkLockFuseException.class)
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public Result handleFiLinkLockFuseException(FiLinkLockFuseException ex) {
        log.error("Bad things:", ex);
        return ResultUtils.warn(ControlResultCode.CALL_BLOW,
                ex.getMessage());
    }
