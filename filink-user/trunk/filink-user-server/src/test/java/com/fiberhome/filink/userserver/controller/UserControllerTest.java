...
public class UserControllerTest {
    ...
    public void importUserInfo() throws Exception {
        FileInputStream fileInputStream = new FileInputStream(new File("C:\\workFile\\zqq\\svn\\Filink_Project\\10-Sys Code\\filink-user\\trunk\\filink-user-server\\src\\test\\java\\com\\fiberhome\\filink\\userserver\\controller\\UserControllerTest.java"));
        String fileName = "name";
        MultipartFile file = new MockMultipartFile(fileName, fileInputStream);
        new Expectations() {
            {
                importService.importUserInfo((MultipartFile) any);
                result = ResultUtils.success();
            }
        };
    ...
    @Test
    public void queryEmailIsExistTest() {
        userController.queryEmailIsExist(new QueryCondition<>());
    }

    @Test
    public void queryUserDetailByIdsTest() {
        userController.queryUserDetailByIds(new ArrayList<>());
    }
}
