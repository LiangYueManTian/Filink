...
public class UserExtraServiceImpl extends ServiceImpl<UserDao, User> implements UserExtraService {
    ...
    public Integer validateUserLogin(UserParameter userParameter) {
        ...
        if (!UserConst.PC_WEBSITE.equals(loginSource)) {
            ...
            Permission appPermission = permissionList.stream().filter(permission -> UserConst.APP_LOGIN_PERMISSION.equals(permission.getId())).findFirst().orElse(null);
    ...
    public Result exportUserList(ExportDto exportDto) {
        ...
        Export export = null;
        try {
            export = userListExport.insertTask(exportDto, SERVER_NAME, I18nUtils.getString(UserI18n.OPERATE_USER_LIST_NAME));
        } catch (FilinkExportNoDataException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(UserConst.EXPORT_NO_DATA, I18nUtils.getString(UserI18n.EXPORT_NO_DATA));
            ...
            return ResultUtils.warn(UserConst.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS,
                    I18nUtils.getString(UserI18n.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            e.printStackTrace();
            return ResultUtils.warn(UserConst.FAILED_TO_CREATE_EXPORT_TASK, I18nUtils.getString(UserI18n.FAILED_TO_CREATE_EXPORT_TASK));
        }
        userListExport.exportData(export);

        //添加导出安全日志
        universalLog(RequestInfoUtils.getUserId(), UserConst.EXPORT_USER_LIST, LogConstants.LOG_TYPE_SECURITY);

        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(UserI18n.THE_EXPORT_TASK_WAS_CREATED_SUCCESSFULLY));
    }
    ...
    public Result sendMessage(String phoneNumber) throws ClientException {
        ...
        if (user == null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.PHONE_NOT_EXIST));
        }
        ...
        if (RedisUtils.hasKey(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token)) {
            ...
            if (userObject != null) {
                ...
                if (userObject instanceof User) {
                    ...
                    return ResultUtils.warn(ResultCode.SUCCESS, I18nUtils.getString(UserI18n.MODIFY_USER_LOGIN_DEVICE_SUCCESS));
                }
            }
        }

        return ResultUtils.warn(UserConst.USER_NOT_EXIST, I18nUtils.getString(UserI18n.USER_NOT_EXIST));
    }
    ...
    private Result getExportToLargeMsg(FilinkExportDataTooLargeException fe) {
        fe.printStackTrace();
        String string = I18nUtils.getString(UserI18n.EXPORT_DATA_TOO_LARGE);
