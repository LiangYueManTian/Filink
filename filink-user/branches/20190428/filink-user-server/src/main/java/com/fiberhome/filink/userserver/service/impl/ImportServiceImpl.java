...
public class ImportServiceImpl implements ImportService {
    ...
    public Result importUserInfo(MultipartFile file) throws IOException, InvalidFormatException {
        if (!file.isEmpty()) {
            ...
            try {
                ...
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.IMPORT_DATA_ERROR));
            }
            ...
            for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
                ...
                if (strategyResult == null || strategyResult.getData() == null) {
                    return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.STRATEGY_SERVER_ERROR));
                }
                AccountSecurityStrategy strategy = JSON.
                        parseObject(JSON.toJSONString(strategyResult.getData()), AccountSecurityStrategy.class);

                for (int j = sheet.getFirstRowNum() + UserConst.UP_ROW; j <= sheet.getLastRowNum(); j++) {
                    row = sheet.getRow(j);
                    int beginColoum = 0;
                    //处理传入的数据，进行判空处理
    ...
    public void removeBlankRow(Sheet sheet) {
        ...
        if (lastRowNum <= UserConst.DATA_SIZE_IS_ZERO) {
            return;
        }
        boolean flag = false;
        ...
        for (int j = beginLine; j <= sheet.getLastRowNum(); j++) {
            ...
            if (row == null) {
                sheet.shiftRows(j + UserConst.UP_ROW, sheet.getLastRowNum(), UserConst.DOWN_ROW);
                j--;
                continue;
            }
            flag = false;
    ...
    public Result checkDataFormat(User user, AccountSecurityStrategy strategy) {
        ...
        boolean userCodeMatche = user.getUserCode().matches(UserConst.USERCODEPATTERN);
        boolean phoneNumMatche = user.getPhonenumber().matches(UserConst.PHONENUMBERPATTERN);
        if (!phoneNumMatche) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                    + user.getUserCode() + I18nUtils.getString(UserI18n.PHONE_NUMBER_ERROR));
        }
        boolean emailMatch = user.getEmail().matches(UserConst.EMAILPATTERN);
        String emailAccount = user.getEmail();
        if (!emailMatch || emailAccount.length() > UserConst.EMAIL_MAX_LENGTH) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                    + user.getUserCode() + I18nUtils.getString(UserI18n.EMAIL_ERROR));
        }
        if (user.getAddress() != null) {
            boolean addressMatche = user.getAddress().matches(UserConst.USERCODEPATTERN);
            if (!addressMatche) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.ADDRESS_ERROR));
            }
        }
        //判断用户昵称的格式
        if (user.getUserNickname() != null) {
            boolean nickNameMatch = user.getUserNickname().matches(UserConst.USERCODEPATTERN);
            if (!nickNameMatch) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.USER_NICK_NAME_ERROR));
            }
        }
        //判断用户名的格式
        if (user.getUserName() != null) {
            boolean userNameMatch = user.getUserName().matches(UserConst.USERCODEPATTERN);
            if (!userNameMatch) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.USER_NAME_ERROR));
            }
        }
        //判断用户备注的格式
        if (user.getUserdesc() != null) {
            boolean userDescMatch = user.getUserdesc().matches(UserConst.USERCODEPATTERN);
            if (!userDescMatch) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.USER_DESC_ERROR));
            }
        }
        ...
        if (userCodeMatche) {
            ...
            if (importUserCode.length() > UserConst.MAX_USER_CODE_LENGTH || importUserCode.length() < minLength) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.USER_CODE_OUT_RANGE));
            }

            //用户昵称长度限制
            String userNickname = user.getUserNickname();
            if (StringUtils.isNotEmpty(userNickname) && userNickname.length() > UserConst.MAX_USER_NICK_NAME_LENGTH) {
                return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                        + user.getUserCode() + I18nUtils.getString(UserI18n.USER_CODE_OUT_RANGE));
            }

            if (user.getLoginType().equals(UserConst.SINGLE_LOGIN_TIP)) {
                if (user.getMaxUsers() != UserConst.ONLY_ONE_DATA) {
                    return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                            + user.getUserCode() + I18nUtils.getString(UserI18n.MAX_USER_NUM_FALL_OUTER_SIDE));
                }
                return null;
            } else {
                if (user.getMaxUsers() > UserConst.MAX_USER_MAX || user.getMaxUsers() < UserConst.MAX_USER_MIN) {
                    return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                            + user.getUserCode() + I18nUtils.getString(UserI18n.MAX_USER_NUM_FALL_OUTER_SIDE));
                }
                return null;
            ...
            for (int i = 0; i < userSize - 1; i++) {
                ...
                for (int j = i + 1; j < userSize; j++) {
                    ...
                    if (pairUser.getUserCode().equals(user.getUserCode())) {
                        return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.USER_CODE_IS)
                                + user.getUserCode() + I18nUtils.getString(UserI18n.REPEAT_IMPORT));
                    }
                    //判断是否重复导入用户手机号
                    if (pairUser.getPhonenumber().equals(user.getPhonenumber())) {
                        return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.PHONE_NUMBER_IS)
                                + user.getPhonenumber() + I18nUtils.getString(UserI18n.REPEAT_IMPORT));
                    }
                    //判断是否重复导入用户邮箱
                    if (pairUser.getEmail().equals(user.getEmail())) {
                        return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.EMAIL_IS)
                                + user.getEmail() + I18nUtils.getString(UserI18n.REPEAT_IMPORT));
                    }
                }
            }
        }
    ...
    public Result batchAddUser(List<User> userList) {
        ...
        try {
            ...
            if (userNum + userList.size() > maxUserName) {
                return ResultUtils.warn(UserConst.USER_NUM_OVER_MAX_USER_NUM,
                        I18nUtils.getString(UserI18n.USER_NUM_OVER_MAX_USER_NUM));
            } else {
        ...
        if (flag) {
            ...
            return ResultUtils.warn(UserConst.DEVICE_SERVER_ERROR,
                    I18nUtils.getString(UserI18n.DEVICE_SERVER_ERROR));
        }


        String userId = RequestInfoUtils.getUserId();
        //添加日志信息
        universalLog(userId, UserConst.IMPORT_USER_MODEL, LogConstants.LOG_TYPE_SECURITY);
    ...
    public User addUserFromExcel(Row row, int beginColoum, List<Department> departmentList, List<Role> roleList) {
        ...
        user.setUserStatus(userStatus.equals(UserConst.START_STATUS) ? UserConst.START_STATUS_TIP : UserConst.STOP_STATUS_TIP);
        //角色名称
        ...
        user.setLoginType(loginType.equals(UserConst.SINGLE_LOGIN_TYPE) ? UserConst.SINGLE_LOGIN_TIP : UserConst.MORE_LOGIN_TIP);
        //最多用户数
        Cell useCountCell = row.getCell(beginColoum++);
        if (useCountCell == null) {
            user.setMaxUsers(UserConst.SINGLE_LOGIN_NUM);
        } else {
            String userCount = NameUtils.removeBlank(String.valueOf(getCellValue(useCountCell)));
            if (UserConst.SINGLE_LOGIN_TYPE.equals(loginType)) {
                user.setMaxUsers(UserConst.SINGLE_LOGIN_NUM);
            } else {
        ...
        if (valiDayCell == null) {
            ...
            if (cellType != Cell.CELL_TYPE_BLANK) {
                ...
                if (StringUtils.isNotEmpty(valiDay)) {
                    ...
                    user.setCountValidityTime(valiTime + UserConst.DAY_TIP);
                }
            }
        }
        //描述
        Cell remarkCell = row.getCell(UserConst.USER_DESC_SITE);
        if (remarkCell == null) {
        ...
        user.setPassword(PasswordUtils.passwordEncrypt(UserConst.DEFAULT_PWD));
        return user;
    }
    ...
    private Result checkDataNull(Row row) {
        //用户代码，不能为空
        Cell userCodeCell = row.getCell(UserConst.USER_CODE_SITE);
        ...
        //用户姓名
        Cell userNameCell = row.getCell(UserConst.USER_NAME_SITE);
        ...
        //用户状态
        Cell userStatusCell = row.getCell(UserConst.USER_STATU_SITE);
        ...
        //用户部门
        Cell departmentCell = row.getCell(UserConst.DEPARTMENT_SITE);
        ...
        //用户角色
        Cell roleCell = row.getCell(UserConst.ROLE_SITE);
        ...
        //用户模式
        Cell loginTypeCell = row.getCell(UserConst.LOGINT_TYPE_SITE);
        ...
        //用户手机号码
        Cell phoneNumberCell = row.getCell(UserConst.PHONE_NUMBER_SITE);
        if (phoneNumberCell == null || getCellType(phoneNumberCell) == Cell.CELL_TYPE_BLANK) {
            return ResultUtils.warn(UserConst.PHONE_NUMBER_NULL, I18nUtils.getString(UserI18n.PHONE_NUMBER_NULL));
        } else {
        ...
        //邮箱
        Cell emailCell = row.getCell(UserConst.EMAIL_SITE);
        if (emailCell == null || getCellType(emailCell) == Cell.CELL_TYPE_BLANK) {
            return ResultUtils.warn(UserConst.EMAIL_NULL, I18nUtils.getString(UserI18n.EMAIL_NULL));
        } else {
        ...
        if (remarkCell != null) {
            ...
            if (remark.length() >= UserConst.MAX_USER_DESC_LENGTH) {
                return ResultUtils.warn(UserConst.USER_DESC_TOO_LONG, I18nUtils.getString(UserI18n.USER_DESC_TOO_LONG));
            }
        }

        //地址信息最大长度
        Cell addressCell = row.getCell(UserConst.USER_ADDRESS_SITE);
        if (addressCell != null) {
            String address = String.valueOf(getCellValue(addressCell));
            if (address.length() > UserConst.MAX_USER_ADDRESS_LENGTH) {
                return ResultUtils.warn(UserConst.USER_ADDRESS_TOLONG, I18nUtils.getString(UserI18n.USER_ADDRESS_TOLONG));
            }
        }

        //用户有效天数为数字类型
        Cell validataCell = row.getCell(UserConst.VALIDATE_TIME);
        if (validataCell != null && getCellType(validataCell) != Cell.CELL_TYPE_BLANK) {
            ...
            if (StringUtils.isNotEmpty(validay)) {
                ...
                if (validataType != Cell.CELL_TYPE_NUMERIC) {
                    return ResultUtils.warn(UserConst.VALIDATA_TIME_ABNORMAL_FORMAT, I18nUtils.getString(UserI18n.VALIDATA_TIME_ABNORMAL_FORMAT));
                }
            }
        }

        //最多用户数
        Cell maxNumCell = row.getCell(UserConst.MAXUSER_SITE);
        if (maxNumCell != null) {
            Integer maxNumType = getCellType(maxNumCell);
            if (maxNumType != Cell.CELL_TYPE_NUMERIC) {
                return ResultUtils.warn(UserConst.MAX_USER_NUMBER_ABNORMAL_FORMAT, I18nUtils.getString(UserI18n.MAX_USER_NUMBER_ABNORMAL_FORMAT));
            }
        }
    ...
    private Result checkData(Row row, int beginColoum, List<User> userList, List<Role> roleList,
        ...
        //用户手机号码唯一
        Cell phoneCell = row.getCell(UserConst.PHONE_NUMBER_SITE);
        ...
        if (valiDayCell != null) {
            ...
            if (cellType != Cell.CELL_TYPE_BLANK) {
                ...
                if (StringUtils.isNotEmpty(valiDay)) {
                    int valiTime = Integer.parseInt(valiDay);
                    if (valiTime > UserConst.VALIDATE_TIME_MAX || valiTime < UserConst.VALIDATE_TIME_MIN) {
                        return ResultUtils.warn(
                                ResultCode.FAIL, I18nUtils.getString(UserI18n.VALID_TIME_OUT_RANGE));
                    }
                }
            }
        }
    ...
    public Result checkEmail(Cell emailCell, List<User> userList) {
        ...
        if (emailCell == null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.EMAIL_NULL));
        }
        //获取邮箱
        ...
        if (equalUser != null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.EMAIL_IS)
                    + email + I18nUtils.getString(UserI18n.HAS_USED));
        }
        return null;
    }
    ...
    public Result checkPhoneNumber(Cell phoneCell, List<User> userList) {
        ...
        if (phoneCell == null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.PHONE_NUMBER_NULL));
        }
        //获取手机号，校验用户码唯一性
        ...
        if (equalUser != null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(UserI18n.PHONE_NUMBER_IS)
                    + phoneNumber + I18nUtils.getString(UserI18n.HAS_USED));
        }
        return null;
    }
    ...
    public Result checkDepartmentName(Cell departmentCell, List<Department> departmentList) {
        ...
        if (departmentFullNameList.size() == UserConst.DATA_SIZE_IS_ZERO) {
        ...
        return ResultUtils.success(departmentFullNameList.get(UserConst.LIST_FIRST));
    }
    ...
    public void getDeptFullName(StringBuffer fullName, Department department) {
        ...
        fullName.insert(UserConst.BEGIN_INSERT, department.getDeptName() + UserConst.SPLIT_SIGN);
    ...
    private Result checkRoleName(Cell roleCell, List<Role> roleList) {
        ...
        //boolean hasRole = roleList.stream().anyMatch(subRole -> subRole.getRoleName().equals(roleName));
    ...
    public Object getCellValue(Cell cell) {
        ...
        DecimalFormat decimalFormat = new DecimalFormat(UserConst.ZERO_DECIMAL_FORMAT);
