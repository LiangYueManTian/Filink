...
//             return ResultUtils.warn(UserConst.ADMIN_ROLE_IS_NOT_UPDATE,I18nUtils.getString(UserI18n.ADMIN_ROLE_IS_NOT_UPDATE));
//         }
        //去除前后空格
        role.setRoleName(NameUtils.removeBlank(role.getRoleName()));
        role.setRemark(NameUtils.removeBlank(role.getRemark()));
...
//            });
            ...
            //更新操作权限后添加到安全日志
            if (!(permissionList.size() == role.getPermissionIds().size() && permissionList.stream().allMatch(u -> role.getPermissionIds().contains(u.getId())))) {
...
//        }
        ...
        if (CheckEmptyUtils.collectEmpty(deviceTypeIds)) {
            ...
            if (!(roleDeviceTypeList.size() ==deviceTypeIds.size() && roleDeviceTypeList.stream().allMatch(u -> deviceTypeIds.contains(u.getDeviceTypeId())))) {
...
//                    + I18nUtils.getString(UserI18n.PERMISSION_NAME_IS) + permissionName;
            //设置操作数据信息
