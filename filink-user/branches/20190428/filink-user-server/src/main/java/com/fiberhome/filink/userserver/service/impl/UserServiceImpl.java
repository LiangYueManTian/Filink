...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public Result queryUserInfoById(String userId) {
        ...
        if (user == null) {
            return ResultUtils.warn(UserConst.USER_HAS_DELETED, I18nUtils.getString(UserI18n.USER_HAS_DELETED));
        }
    ...
    public Result updateUser(User user) {
        ...
        if (UserConst.ADMIN_ID.equals(user.getId()) && !updateUserId.equals(user.getId())) {
            return ResultUtils.warn(UserConst.ADMIN_USER_NOT_OTHER_UPDATE, I18nUtils.getString(UserI18n.ADMIN_USER_NOT_OTHER_UPDATE));
        }
    ...
    public Result deleteUser(Parameters deleteUser) {
        ...
        //被删除的用户必须存在
        List<User> deleteUserList = userDao.batchQueryUserByIds(deleteUser.getFirstArrayParamter());
        if (deleteUser.getFirstArrayParamter().length != deleteUserList.size()) {
            return ResultUtils.warn(UserConst.EXIST_HAS_DELETE_USER,
                    I18nUtils.getString(UserI18n.EXIST_HAS_DELETE_USER));
        }
        ...
        if (getUser != null) {
            return ResultUtils.warn(UserConst.USER_IN_USE_MODEL,
                    I18nUtils.getString(UserI18n.USER_IN_USE_MODEL));
        }

        //不能删除有工单任务的账户
        boolean orderFlag = procBaseFeign.queryIsExistsAssignUser(Arrays.asList(deleteUser.getFirstArrayParamter()));
        if (orderFlag) {
            return ResultUtils.warn(UserConst.USER_IN_USE_ORDER,
                    I18nUtils.getString(UserI18n.USER_IN_USE_ORDER));
        }
        ...
        if (deleteFlag[0] && !deleteUser.isFlag()) {
            ...
            //删除用户的时候要一起删除配置信息
            deviceMapConfigFeign.deletedConfigByUserIds(Arrays.asList(deleteUser.getFirstArrayParamter()));
    ...
    public User queryUserByName(UserParameter userParameter) {
        ...
        if (userParameter.getToken() != null) {
            ...
            RedisUtils.set(UserConst.USER_PREFIX + getUser.getId() + UserConst.REDIS_SPLIT + userParameter.getToken(), getUser, noOperationTime * 60);
    ...
    public Result updateUserStatus(int userStatus, String[] userIdArray) {
        ...
        if (userIdArray == null || userIdArray.length == 0) {
            return ResultUtils.warn(UserConst.SELECT_NEED_OPER_DATA,
                    I18nUtils.getString(UserI18n.SELECT_NEED_OPER_DATA));
        }
        ...
        if (bo) {
            return ResultUtils.warn(UserConst.ADMIN_USER_NOT_FORBIDDEN,
                    I18nUtils.getString(UserI18n.ADMIN_USER_NOT_FORBIDDEN));
        }
    ...
    public Result queryUserByDept(Parameters parameters) {
        ...
        if (ArrayUtils.isNotEmpty(parameters.getFirstArrayParamter())) {
            users = userDao.queryUserByDepts(parameters.getFirstArrayParamter());
        }
