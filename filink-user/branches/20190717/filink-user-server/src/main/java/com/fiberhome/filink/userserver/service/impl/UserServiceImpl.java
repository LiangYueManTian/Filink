...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public Result queryUserInfoById(String userId) {
        ...
        if (user == null) {
            return ResultUtils.warn(UserConst.USER_HAS_DELETED, I18nUtils.getSystemString(UserI18n.USER_HAS_DELETED));
        }
    ...
    @AddLogAnnotation(value = UserConst.ADD_USER, logType = UserConst.USER_LOG_TYPE,
            functionCode = UserConst.ADD_USER_MODEL_CODE, dataGetColumnName = UserConst.LOG_USER_NAME,
            dataGetColumnId = UserConst.LOG_USER_ID)
    @Override
    public Result addUser(User user) {

        systemLanguageUtil.querySystemLanguage();
        if (user.getEmail() != null && user.getEmail().length() > EMAIN_LENGTH) {
            return ResultUtils.warn(UserConst.EMAIL_TOO_LONG, I18nUtils.getSystemString(UserI18n.EMAIL_TOO_LONG));
        }
        ...
        if (userByCode != null) {
            return ResultUtils.warn(UserConst.ADD_USER_FAIL, I18nUtils.getSystemString(UserI18n.USER_EXIST));
        }
        ...
        //添加用户的时候加锁
        String lockKey = UserConst.LOCK_ADD_USER;
        //等待获取锁的时间，单位ms
        ...
        //释放锁
        RedisUtils.releaseLock(lockKey, lockIdentifier);
        return ResultUtils.warn(UserConst.ADD_USER_FAIL, I18nUtils.getSystemString(UserI18n.ADD_USER_FAIL));
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    @AddLogAnnotation(value = UserConst.UPDATE_USER, logType = UserConst.USER_LOG_TYPE,
            functionCode = UserConst.UPDATE_MODEL_CODE, dataGetColumnName = UserConst.LOG_USER_NAME,
            dataGetColumnId = UserConst.LOG_USER_ID)
    @Override
    public Result updateUser(User user) {
        ...
        user.setUpdateTime(FiLinkTimeUtils.getUtcZeroTimeStamp());
        ...
        //其他人不能修改admin账号
        if (UserConst.ADMIN_ID.equals(user.getId()) && !updateUserId.equals(user.getId())) {
            return ResultUtils.warn(UserConst.ADMIN_USER_NOT_OTHER_UPDATE, I18nUtils.getSystemString(UserI18n.ADMIN_USER_NOT_OTHER_UPDATE));
        }

        //admin账号不能修改角色
        if (UserConst.ADMIN_ID.equals(user.getId()) && !updateUser.getRoleId().equals(user.getRoleId())) {
            return ResultUtils.warn(UserConst.ADMIN_USER_NOT_OTHER_UPDATE, I18nUtils.getSystemString(UserI18n.ADMIN_USER_NOT_OTHER_UPDATE));
        }
    ...
    public Result deleteUser(Parameters deleteUser) {
        ...
        if (deleteUser.getFirstArrayParameter().length != deleteUserList.size()) {
            return ResultUtils.warn(UserConst.EXIST_HAS_DELETE_USER,
        ...
        if (getUser != null) {
            return ResultUtils.warn(UserConst.USER_IN_USE_MODEL,
        ...
        if (orderFlag) {
            return ResultUtils.warn(UserConst.USER_IN_USE_ORDER,
        ...
        if (!deleteUser.isFlag()) {
            deleteUserList.forEach(user -> {
                Set<String> keys = RedisUtils.keys(UserConst.USER_PREFIX + user.getId() + UserConst.REDIS_SPLIT + UserConst.REDIS_WILDCARD);
        ...
        boolean contains = Arrays.asList(deleteUser.getFirstArrayParameter()).contains(UserConst.ADMIN_ID);
        if (deleteFlag[0] && !deleteUser.isFlag()) {
            return ResultUtils.warn(UserConst.ONLINE_USER, I18nUtils.getSystemString(UserI18n.ONLINE_USER));
        } else if (contains) {
            ...
            websocketSendMessage(UserConst.DELETE_USER_WEBSOCKET,
                    UserConst.DELETE_USER_WEBSOCKET, deleteUser.getFirstArrayParameter());

            //删除用户信息
            Integer deleteNum = userDao.deleteUser(deleteUser.getFirstArrayParameter(), UserConst.USER_DELETED_CODE);
    ...
    public User queryUserByName(UserParameter userParameter) {
        User getUser = userDao.queryUserByName(userParameter.getUserName());
        log.info("登录的用户信息={}", getUser);
        //如果有token值则为查询用户信息，没有token则是查询用户密码
        if (userParameter.getToken() != null) {
            //将登录时间修改成上次登录时间
            log.info("loginIp = {} ", userParameter.getLoginIp());
            ...
            if (UserConst.PC_WEBSITE.equals(loginSourse)) {
                getUser.setLoginSourse(UserConst.PC_WEBSITE);
            } else {
                getUser.setLoginSourse(UserConst.APP_WEBSITE);
            }

            //设置当前时间为用户的登录时间
            getUser.setLoginTime(FiLinkTimeUtils.getUtcZeroTimeStamp());
            //用户的登录时间ip等信息更新到用户表中
            ...
            log.info("账号的有效期 = {}", noOperationTime * 60);
            RedisUtils.set(UserI18n.USER_EXPIRE_TIME, noOperationTime * 60);
            getUser.setExpireTime(noOperationTime * 60);
            //添加用户信息到redis中,存储方式(userid_token,userinfo,过期时间)
            log.info("userId_token = {}", getUser.getId() + UserConst.REDIS_SPLIT + userParameter.getToken());
            Role role = getUser.getRole();
            Department department = getUser.getDepartment();
            getUser.setDepartment(null);
            getUser.setRole(null);
            RedisUtils.set(UserConst.USER_PREFIX + getUser.getId() + UserConst.REDIS_SPLIT
            ...
            String key = loginIp + UserConst.REDIS_IP_PORT_SPLIT + getUser.getId();
    ...
    public Result updateUserStatus(int userStatus, String[] userIdArray) {
        ...
        if (userIdArray == null || userIdArray.length == 0) {
            return ResultUtils.warn(UserConst.SELECT_NEED_OPER_DATA,
                    I18nUtils.getSystemString(UserI18n.SELECT_NEED_OPER_DATA));
        }

        //更新用户含有admin用户,不能被停用
        boolean bo = Arrays.stream(userIdArray).anyMatch(u -> u.equals(UserConst.ADMIN_ID));
        if (bo) {
            return ResultUtils.warn(UserConst.ADMIN_USER_NOT_FORBIDDEN,
    ...
    public Result checkPWD(PasswordDto pwd) {
        if (pwd == null) {
            return ResultUtils.warn(UserConst.NEWPWD_IS_NOT_NULL, I18nUtils.getSystemString(UserI18n.NEWPWD_IS_NOT_NULL));
        }
    ...
    public Result resetPWD(PasswordDto pwd) {
        ...
        userList.add(user);
        addLogByUsers(userList, UserConst.RESET_PWD_MODEL);
        //设置默认新密码 并加密
        pwd.setNewPWD(PasswordUtils.passwordEncrypt(UserConst.USER_RESET_NEW_PWD));
        //重置密码
        Integer modifyNum = userDao.modifyPWD(pwd);
    ...
    public void modifyOnlineUser(List<OnlineUser> loginList) {
        ...
        Set<String> keySet = RedisUtils.keys(UserConst.USER_PREFIX + UserConst.REDIS_WILDCARD);
        //后来又登录的用户信息
    ...
    public User queryCurrentUser(String userId, String token) {
        ...
        log.info("key = {}", RedisUtils.hasKey(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token));
        if (RedisUtils.hasKey(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token)) {
            User basicUser = (User) RedisUtils.get(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token);
    ...
    public Result forceOffline(Map<String, String> userMap) {
        ...
        if (idSet.contains(UserConst.USER_PREFIX + currentUserId + UserConst.REDIS_SPLIT + currentToken)) {
        ...
        if (CheckEmptyUtils.collectEmpty(idSet)) {
            idSet.forEach(removeId -> {
                String id = removeId.substring(UserConst.USER_PREFIX.length());
    ...
    public Result logout(String userId, String token) {
        ...
        RedisUtils.remove(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token);
        return ResultUtils.success(ResultCode.SUCCESS);
    }
    ...
    public boolean updateLoginTime(String userId, String token) {
        ...
        if (RedisUtils.hasKey(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token)) {
            ...
            RedisUtils.expire(UserConst.USER_PREFIX + userId + UserConst.REDIS_SPLIT + token, expireTime);
            return true;
        }
        return false;
    }
