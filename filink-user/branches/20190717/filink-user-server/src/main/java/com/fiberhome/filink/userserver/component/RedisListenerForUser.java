...
public class RedisListenerForUser extends KeyExpirationEventMessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        if (!expiredKey.contains(UserConst.REDIS_SPLIT)) {
            return;
        }
        String userId = expiredKey.substring(UserConst.USER_PREFIX.length(), expiredKey.indexOf(UserConst.REDIS_SPLIT));
        if (StringUtils.isNotEmpty(userId)) {
            log.info("过期用户的id = {}", userId);
