...
public class UnifyAuthController {
    ...
    public String addTest() {
        for (int i = 0; i < 10000; i++)  {
            ...
            unifyauth.setName("1W测试授权");
            unifyauth.setUserId("1");
            unifyauth.setAuthUserId("1");
            unifyauth.setAuthEffectiveTime(System.currentTimeMillis()-i*1000);
            unifyauth.setAuthExpirationTime(System.currentTimeMillis()+i*1000);
            unifyauth.setAuthStatus(2);
            unifyauth.setRemark("大数据测试授权");
            unifyauth.setCreateTime(System.currentTimeMillis());
            unifyauth.setCreateUser("1");
            Result result = unifyauthService.addUnifyAuth(unifyauth);
            System.out.println(result.getCode());
            System.out.println("插入数据第"+i+"条完成");
        }
        return "插入数据完成";
    }
