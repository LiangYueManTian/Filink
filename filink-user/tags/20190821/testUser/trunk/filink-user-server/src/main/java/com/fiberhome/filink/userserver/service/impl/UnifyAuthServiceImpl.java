...
public class UnifyAuthServiceImpl extends ServiceImpl<UnifyauthDao, UnifyAuth> implements UnifyAuthService {
   ...
   // @Transactional(rollbackFor = Exception.class)
    //@AddLogAnnotation(value = LogConstants.DATA_OPT_TYPE_ADD, logType = LogConstants.LOG_TYPE_SECURITY,
       //     functionCode = FunctionCodeConstant.ADD_UNIFY_AUTH_FUNCTION_CODE, dataGetColumnName = "name", dataGetColumnId = "id")
    public Result addUnifyAuth(UnifyAuth unifyauth) {
        ...
        unifyauth.setIsDeleted(UserConstant.UNIFYAUTH_DEFAULT_DELETED);
        ...
        if (insert != UserConstant.ADD_ONE_INFO || addNum != authDevices.size()) {
    ...
    public Result queryUnifyAuthByCondition(QueryCondition<UnifyAuthParameter> unifyAuthCondition) {
        ...
        if (!UserConstant.ADMIN_ID.equals(RequestInfoUtils.getUserId())) {
    ...
    public Result modifyUnifyAuth(UnifyAuth unifyauth) {
        ...
        if (modiyUnifyAuth == null) {
            return ResultUtils.warn(UserResultCode.UNIFYAUTH_HAS_DELETED,
    ...
    public Result deleteUnifyAuthById(String id) {
        ...
        if (UserConstant.UNIFY_AUTH_START_STATUS.equals(authStatus) && modiyUnifyAuth.getAuthExpirationTime() > System.currentTimeMillis()) {
            return ResultUtils.warn(UserResultCode.UNIFY_AUTH_IS_ENABLED_STATE,
                    I18nUtils.getSystemString(UserI18n.UNIFY_AUTH_IS_ENABLED_STATE));
        }

        Integer integer = unifyauthDao.deleteUnifyAuthById(id);
        if (integer != UserConstant.ADD_ONE_INFO) {
            throw new FilinkUnifyAuthException(I18nUtils.getSystemString(UserI18n.UNIFYAUTH_OPER_ERROR));
        }
    ...
    public Result batchDeleteUnifyAuth(String[] ids) {
        ...
        UnifyAuth firstUnifyAuth = unifyAuthList.stream().filter(auth ->
                UserConstant.UNIFY_AUTH_START_STATUS.equals(auth.getAuthStatus()) && auth.getAuthExpirationTime() > System.currentTimeMillis()
    ...
    public Result batchModifyUnifyAuthStatus(UnifyAuthParameter unifyAuthParameter) {
        ...
        for (UnifyAuth unifyAuth : unifyAuthList) {
            if (unifyAuth.getAuthExpirationTime() < System.currentTimeMillis()) {
                return ResultUtils.warn(UserResultCode.EXPIRETIME_MUST_GT_CURRENTTIME,
                        I18nUtils.getSystemString(UserI18n.AUTHORIZATION_THAT_HAS_EXPIRED_CANNOT_CHANGE_STATUS));
            }
        }
