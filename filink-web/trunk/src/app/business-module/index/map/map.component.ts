 ...
 constructor(
  ...
  public facilityChangeHook() {
    ...
    this.$indexMissionService.facilityChangeHook.subscribe(data => {
      ...
      } else if (this.facilityNoticeArr.indexOf(type) > -1) {
        // 设施收藏更新
  ...
  public queryHomeDeviceArea() {
    this.$mapService.queryHomeDeviceArea().subscribe((result: Result) => {
      if (result.code === 0) {
        // 移除消息
        this.$message.remove();
        // 缓存区域数据
        this.$mapStoreService.areaDataList = result.data.areaDeviceNumList;
        // hugeData："0"小数据，"1"大数据
        this.$mapStoreService.hugeData = result.data.hugeData === index_huge_data;
        // exceedLimit：true提示超限，false没有超限
        this.$mapStoreService.exceedLimit = result.data.isExceedLimit;
        // 如果超过预设限制，提示用户选择是否加载
        if (this.$mapStoreService.exceedLimit) {
          // 提示弹框
          this.modalService.confirm({
            nzTitle: this.indexLanguage.prompt,
            nzContent: this.indexLanguage.areaIndexTip,
   ...
   */
  mapEvent(event) {
    if (event.type === index_map_type.mapClick) {
      // 关闭设施详情面板
      this.closePanel();
      this.mainMap.closeRightClick();
