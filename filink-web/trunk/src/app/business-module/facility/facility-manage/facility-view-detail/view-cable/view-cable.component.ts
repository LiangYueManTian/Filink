...
export class ViewCableComponent implements OnInit {
  ...
  // 光缆段终止节点类型
  public terminationNodeDeviceType;
  // 光缆段起始节点类型
  public startNodeDeviceType;
  // 光缆段终止节点名称
  public terminationNodeName;
  // 光缆段起始节点名称
  ...
  // 选中设施终止节点名称
  public selectEndUnitName;
  // 设施节点的显示隐藏
  public isVisible: boolean = false;
  // 设施节点的显示隐藏
  public isEndVisible: boolean = false;
  // 光缆纤芯筛选输入值
  public cableCoreInputValue;
  // 光缆纤芯筛选默认等于
  public cableCoreSelectValue = Operate;
  // 长度筛选输入值
  public lengthInputValue;
  // 长度筛选默认等于
  public lengthSelectValue = Operate;
  // 光缆芯数筛选输入值
  public cableCoreNumInputValue;
  // 光缆芯数筛选默认等于
  ...
  public title
  // 光缆名称
  @ViewChild('cableName') cableNameTemp: TemplateRef<any>;
  // 起始节点选择器
  @ViewChild('UnitNameSearch') unitNameSearch: TemplateRef<any>;
  // 终止节点选择器
  @ViewChild('EndUnitNameSearch') EndUnitNameSearch: TemplateRef<any>;
  // 光缆纤芯
  @ViewChild('cableCoreTemp') cableCoreTemp: TemplateRef<any>;
  // 光缆长度
  @ViewChild('lengthTemp') lengthTemp: TemplateRef<any>;
  // 光缆芯数
  @ViewChild('cableCoreNumTemp') cableCoreNumTemp: TemplateRef<any>;
  // 调整智能标签坐标
  @ViewChild('adjustMap') adjustMap: AdjustMapComponent;
  // 智能标签列表
  @ViewChild('smartLabel') smartLabel: SmartLabelComponent;
  // 熔纤和查看熔纤
  @ViewChild('coreFusion') coreFusion: CoreFusionComponent;
  // 巡检区域
  @ViewChild('areaSelector') private areaSelectorTemp;
  // 设施选择器配置
  deviceObjectConfig: AlarmObjectConfig;
  // 结束光缆设施选择器配置
  deviceObjectConfigEnd: AlarmObjectConfig;
  // 已选设施
  checkDeviceObject = {
    ids: [],
    name: ''
   ...
   * 光缆段列表生成导出条件
   */
  createSectionExportParams(event) {
    this.exportParams.queryCondition = this.queryCondition;
    if (event.selectItem.length > 0) {
   ...
   * param event
   */
  selectEndDataChange(event) {
    let selectArr = [];
    this.selectEndUnitName = '';
    if (event.length > 0) {
      selectArr = event.map(item => {
        this.selectEndUnitName += `${item.deviceName},`;
        return item.deviceId;
      });
    } else {
    }
    this.selectEndUnitName = this.selectEndUnitName.substring(0, this.selectEndUnitName.length - 1);
    if (selectArr.length === 0) {
      this.filterValue['filterValue'] = null;
    } else {
      this.filterValue['filterValue'] = selectArr;
    }
    this.$facilityUtilService.setTreeNodesStatus(this.treeEndNodes, selectArr);
  }

  /**
   *光缆名称点击筛选光缆段信息列表
   */
  navigateToCableSegment(data) {
      ...
      operation: [
        ...
        { // 删除
          ...
          handle: (data) => {
            ...
            this.$facilityService.deviceIdCheckUserIfDevicePermission(deviceList).subscribe((result: Result) => {
              if (result.data === true) {
                this.$facilityService.deleteCableSectionById(data.opticCableSectionId).subscribe((result: Result) => {
                  if (result.code === 0) {
                    this.$modalService.success(result.msg);
                    // 删除之后返回到第一页
