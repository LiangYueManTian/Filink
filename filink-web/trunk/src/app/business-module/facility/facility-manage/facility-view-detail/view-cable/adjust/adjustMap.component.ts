import {AfterViewInit, Component, Input, Output, OnInit, TemplateRef, ViewChild, EventEmitter} from '@angular/core';
...
export class AdjustMapComponent implements OnInit, AfterViewInit {
  ...
  mapService: MapSelectorService | GMapSelectorService; // 地图服务
  public mapType: string = 'baidu'; // 判断地图类型
  @Input() isVisible = false; // 模态框 关闭
  @Input() mapData = []; // 模态框传入数据
  @Input() id = ''; // 传入ID获取详情
  @Output() close = new EventEmitter(); // 关闭事件
  markMap = new Map(); // 百度地图数据
  gMarkMap = new Map(); // 谷歌地图数据
  marker; // 谷歌地图坐标点
  gFun = []; // 坐标点存入数组
  deviceMapData = []; // 存入设施点
  dataId = [];
  ...
  getGmap(data) {
    ...
    // const bounds = new google.maps.LatLngBounds(...this.gFun);
