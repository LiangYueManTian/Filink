...
export class InspectionWorkOrderDetailComponent implements OnInit, OnDestroy {
  ...
  departmentSelectorDisabled: boolean = false; // 责任人单位模板button属性绑定
  inspectionFacilitiesSelectorDisabled = true; // 巡检设施模板button属性绑定
  ...
  selectArr; // 责任单位名称集合
  isSelectAll;  // 是否巡检全集 // 1:是  0:否 // 默认状态
  inspectionStartTime;
  inspectionEndTime;
  lastDays; // 剩余天数
  inspectionName; // 工单名称
    ...
    this.formColumn = [
      ...
      {// 设施类型
        ...
        modelChange: (controls, event, key, formOperate) => {
          ...
          if (this.changeDevice && event !== null) {
    ...
    if (this.pageType === 'update' || this.pageType === 'updates') {
      ...
      this.$inspectionService.getUpdateWorkUnfinished(this.procId).subscribe((result: Result) => {
        ...
        if (result.code === this.WorkOrder.ZERO) {
          ...
          result.data['inspectionEndTime'] = new Date(CommonUtil.convertTime(result.data['inspectionEndTime']));
  ...
  add() {
    ...
    if (new Date(this.dateEnd) < nowDate) {
      ...
      if (this.pageType === 'update' || this.pageType === 'updates') {
        ...
        // CommonUtil.sendBackEndTime(new Date(formData.ecTime).getTime());
        data.inspectionStartDate = CommonUtil.sendBackEndTime(new Date(data.inspectionStartDate).getTime()) || this.inspectionStartTime;
        data.inspectionEndDate = CommonUtil.sendBackEndTime(new Date(data.inspectionEndDate).getTime()) || this.inspectionEndTime;
      } else {
        data.inspectionStartDate = CommonUtil.sendBackEndTime(new Date(data.inspectionStartDate).getTime());
        data.inspectionEndDate = CommonUtil.sendBackEndTime(new Date(data.inspectionEndDate).getTime());
      }
  ...
  mapSelectDataChanges(event) {
    ...
    if (this.isSelectAll === '1') {
      ...
      this.inspectionFacilitiesSelectorName = this._deviceName;
    }
  }
  ...
  private initMapSelectorConfig() {
    this.mapSelectorConfig = {
      ...
      selectedColumn: [
        ...
        {
          title: this.InspectionLanguage.facilityType, key: '_deviceType', width: 60,
        },
        {
  ...
  private getUpdateWorkUnfinished(id) {
    this.$inspectionService.getUpdateWorkUnfinished(id).subscribe((result: Result) => {
      ...
      this._deviceName = result.data.deviceName;
    });
  }
  ...
  confirmButtonIsGray() {
    ...
    if (this.pageType === 'update' && this.status !== 'assigned') {
      ...
      if (this.dateEnd && this.dateStart && this.inspectionName && this.areaName && this.changeDevice &&
