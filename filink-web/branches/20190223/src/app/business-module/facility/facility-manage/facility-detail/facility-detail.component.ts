...
export class FacilityDetailComponent implements OnInit, AfterViewInit {
  ...
  private initColumn() {
    this.formColumn = [
        ...
        asyncRules: [
          {
            asyncRule: (control: FormControl) => {
              return Observable.create(observer => {
                this.$facilityService.queryDeviceNameIsExist({deviceId: this.deviceId, deviceName: control.value})
      ...
      {label: this.language.remarks, key: 'remarks', type: 'input', rule: [{maxLength: 200}]},
    ];
  }
