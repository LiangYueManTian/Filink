...
export class MapSelectorService extends MapAbstract {
  ...
  createMap(documentId) {
    const mapInstance = new BMap.Map(documentId, {enableMapClick: false});  // 创建Map实例
    // 添加地图类型控件
  ...
  addMarkerClusterer(markers, fn?) {
    ...
    const markerClusterer = new BMapLib.MarkerClusterer(this.mapInstance, {markers: markers, maxZoom: 19}, (event, data) => {
