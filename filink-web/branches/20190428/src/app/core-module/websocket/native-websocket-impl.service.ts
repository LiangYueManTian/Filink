...
export class NativeWebsocketImplService {
  ...
  connect() {
    ...
    // this.socket = new WebSocket(`wss://witstest.fi-link.net/websocket/${SessionUtil.getToken()}`);
    // this.socket = new WebSocket(`wss://10.5.24.224:10150/websocket/${SessionUtil.getToken()}`);
    // this.socket = new WebSocket(`ws://10.5.33.51:9001/websocket/${SessionUtil.getToken()}`);
    this.socket = new WebSocket(`wss://${location.host}/websocket/${SessionUtil.getToken()}`);
    // this.socket = new WebSocket(`wss://10.5.24.224:2181/websocket/${SessionUtil.getToken()}`);
