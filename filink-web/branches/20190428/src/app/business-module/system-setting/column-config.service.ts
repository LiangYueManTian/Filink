...
import {getDeployStatus} from '../facility/facility.config';

@Injectable()
export class ColumnConfigService extends BasicConfig {
  constructor(public $nzI18n: NzI18nService, private $ruleUtil: RuleUtil, private $systemSettingService: MenuManageService) {
    super($nzI18n);
  }
  ...
  getAccessControlFormConfig(initData, ipTpl): FormItem[] {
    const formColumn: FormItem[] = [
      ...
      },
      {
        ...
        rule: [],
        // IPV4 IPV6校验待定
        asyncRules: [AsyncRuleUtil.IPV4Reg(this.language.systemSetting.serviceInterfaceAddressFormatIncorrect)]
      },
      {
    ...
    const formColumn: FormItem[] = [
      {
        ...
        asyncRules: [AsyncRuleUtil.mustInt()]
      },
      {
  ...
  getShowSystemParameterFormConfig(initData): FormItem[] {
    const formColumn: FormItem[] = [
      ...
      },
      {
        ...
        require: true,
        type: 'input',
        rule: [{require: true}, {min: 0}, {max: 99}],
        asyncRules: [AsyncRuleUtil.mustInt()]
      },
      {
    ...
    const formColumn: FormItem[] = [
      {
        ...
        rule: [{require: true}],
        require: true,
        asyncRules: [AsyncRuleUtil.mailReg()]
      },
      {
      ...
      },
      {
        ...
        require: false,
        rule: [{maxLength: 255}],
        asyncRules: [AsyncRuleUtil.emailRegExp()]
      }
    ];
    ...
    const formColumn: FormItem[] = [
      {
        ...
        initialValue: initData.code || '',
        rule: [],
        asyncRules: [AsyncRuleUtil.IPV4Reg()]
      },
      {
    ...
    const formColumn: FormItem[] = [
      {
        ...
        placeholder: 'http://',
        asyncRules: [AsyncRuleUtil.HttpReg('格式不正确！')]
      },
      // {
      ...
      // },
      // {
      //   label: this.language.systemSetting.enableLinkRestriction,
      //   key: 'enabled',
      //   labelWidth: 160,
      //   type: 'radio',
      //   radioInfo: {
      ...
      // },
      // {
      //   label: this.language.systemSetting.permissibleNumberOfLinks,
      //   key: 'maxActive',
      //   labelWidth: 160,
      //   type: 'input',
      //   col: 24,
      //   rule: [{require: true}, {min: 10}, {max: 10000}],
      //   asyncRules: [AsyncRuleUtil.mustInt()]
      // },
    ];
    return formColumn;
    ...
    const formColumn: FormItem[] = [
      {
        ...
        placeholder: 'https://',
        rule: [{require: true}, {minLength: 1}, {maxLength: 50}],
        asyncRules: [AsyncRuleUtil.HttpsReg('格式不正确！')]
      },
    ];
    // const formColumn: FormItem[] = this.getHttpServeFormConfig(initData);
    // formColumn.push({
  ...
  getLicenseColumnConfig(templateObj: any) {
    const config: Array<ColumnConfig> = [
      {
        type: 'serial-number', width: 62, title: '序号',
