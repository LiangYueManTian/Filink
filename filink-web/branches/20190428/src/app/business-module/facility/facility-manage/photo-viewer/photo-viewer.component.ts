...
export class PhotoViewerComponent implements OnInit {
  ...
  showFilter = false;
  // 列表查询条件 默认每页30
  queryConditions = {bizCondition: {}, pageCondition: {
      'pageSize': 60,
      'pageNum': 1
    }};
  // 当前查看图片信息
  ...
  deleteDisable() {
    let delImgList = [];
    Object.values(this.selectPicObj).forEach(item => {delImgList = delImgList.concat(item); });
  ...
  scrollListener(item) {
    // 判断是否还有数据
    if (!this.loadMore && this.queryConditions.pageCondition.pageNum + 1 <= this.totalPage) {
