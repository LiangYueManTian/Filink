...
export class BusinessWebsocketMsgService {
  ...
  public dealMsg(msg) {
    ...
    try {
      ...
      const id_token = `${userInfo.id}||${SessionUtil.getToken()}`;
      if (data) {
        const channelId = data.channelId;
        const channelKey = data.channelKey;
        if (channelKey === 'alarm' && data.msg && data.msg.alarmLevelCode) {
          return;
        }
        ...
        } else if (channelKey === 'export' && channelId === SessionUtil.getToken()) {
          if (SessionUtil.isMessageNotification()) {
            this.$noticeMusicService.noticeMusic();
            this.$exportMessagePush.messagePush(data.msg);
          }
  ...
  private dealMenuUpdate(msg) {
    if (SessionUtil.getMsgSetting().messageRemind === '1') {
      ...
      this.$nzNotificationService.blank(this.commonLanguage.systemMsg, JSON.parse(msg.data).msg, {nzClass: 'menu-blank'});
    }
  }
  ...
  private dealLockStatusChange(type, msg) {
    if (SessionUtil.isMessageNotification()) {
      ...
      this.$nzNotificationService.blank(this.commonLanguage.openLockMsg, msg);
    }
