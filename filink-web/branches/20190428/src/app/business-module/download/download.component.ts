...
export class DownloadComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('operation') operationTemp: TemplateRef<any>;
  ...
  Retry(data) {
    ...
    if (data.listName === this.language.facilityPictureList) {
      ...
      againObj = {
        ...
        taskId: data.taskInfoId,
        columnInfoList: [{
          'columnName': data.columnInfos[0].columnName,
          'propertyName': data.columnInfos[0].propertyName
        },
          {
            'columnName': data.columnInfos[1].columnName,
            'propertyName': data.columnInfos[1].propertyName
          }]
      };
    }
