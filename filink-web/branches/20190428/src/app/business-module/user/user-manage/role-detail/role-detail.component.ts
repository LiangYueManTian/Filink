...
import { DeviceTypeCode, getDeviceType } from '../../../facility/facility.config';
  ...
  ngOnInit() {
    ...
    if (this.pageType !== 'add') {
      ...
      this.getPermission().then((data) => {
        this.leftNodes = data;
        this.rightNodes = rightTreeNodes;
        // this.rightNodes = rightTreeNodes.map(item => {
        //   item.name = getDeviceType(this.$nzI18n, item.name);
        //   return item;
        // });
        this.initRoleSelectorConfig(this.leftNodes, this.rightNodes);
      });
 ...
 */
  ...
  selectDataChange(event) {
    ...
    if (event.left && event.left.length > 0) {
      ...
      const permissionIds = [];
      leftData.forEach(item => { permissionIds.push(item.id); });
    ...
    if (event.right && event.right.length > 0) {
      ...
      this.deviceTypeIds = [];
      rightData.forEach(item => { this.deviceTypeIds.push(item.deviceTypeId); });
      this.$userUtilService.resetRightTreeNodeStatus(rightTreeNodes);  // 改变状态前，先重置右树的勾选状态
      this.$userUtilService.setFacilityTreeNodesStastus(this.rightNodes, this.deviceTypeIds);
    } else {
      this.deviceTypeIds = [];
  ...
  ngOnDestroy() {
    ...
    this.$userUtilService.resetRightTreeNodeStatus(rightTreeNodes);
  }
