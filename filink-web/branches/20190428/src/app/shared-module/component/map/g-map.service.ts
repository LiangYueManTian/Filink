...
export class GMapService extends MapAbstract {
  ...
  constructor(mapConfig: MapConfig) {
    ...
    // this.mapInstance.enableScrollWheelZoom();   // 启用滚轮放大缩小，默认禁用
    // this.mapInstance.disableDoubleClickZoom();   // 禁止双击放大
  ...
  addEventListenerToMap() {
    ...
    this.mapInstance.addListener('maptypeid_changed', () => {
      console.log(this.mapInstance.getMapTypeId());
    });
  }
  ...
  addListenerToClusterer() {
    ...
    // console.dir(this.markerClusterer.clusters_);
    ...
    if (bol) {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
          this.setCenterAndZoom(position.coords.longitude, position.coords.latitude, GMapConfig.defalutZoom);
        }, () => {
  ...
  pointToOverlayPixel(lng, lat) {
    // return this.mapInstance.getProjection().fromLatLngToPoint(new google.maps.LatLng(lat, lng));
