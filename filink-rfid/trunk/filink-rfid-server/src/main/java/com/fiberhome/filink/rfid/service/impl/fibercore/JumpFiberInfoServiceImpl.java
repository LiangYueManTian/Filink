...
public class JumpFiberInfoServiceImpl extends ServiceImpl<JumpFiberInfoDao, JumpFiberInfo> implements JumpFiberInfoService {
    ...
    public Result queryJumpFiberInfoByPortInfo(QueryJumpFiberInfoReq queryJumpFiberInfoReq) {
        //todo:需求变更  这里的数据 都是端口标签的数据 20190809
    ...
    public Result deleteJumpFiberInfoById(DeleteJumpFiberInfoReq deleteJumpFiberInfoReq) {
        ...
        //删除跳接信息
        jumpFiberInfoDao.deleteJumpFiberInfoById(deleteJumpFiberInfoReq.getJumpFiberIdList(), UtcTimeUtil.getUtcTime(), RequestInfoUtils.getUserId());
        ...
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.DELETE_JUMP_CORE_SUCCESS));
    }
    ...
    public Result queryJumpFiberInfoByPortInfoForApp(QueryJumpFiberInfoReqForApp queryJumpFiberInfoReqForApp) {
        // 数据检验
        if (ObjectUtils.isEmpty(queryJumpFiberInfoReqForApp)) {
        ...
        // 根据本端rfid 去获取对应的状态
        List<String> rfidCodes = jumpFiberInfoRespForAppList.stream().map(JumpFiberInfoRespForApp::getRfidCode).collect(Collectors.toList());
        // 对端rfid
        List<String> oppositeRfidCode = jumpFiberInfoRespForAppList.stream().map(JumpFiberInfoRespForApp::getOppositeRfidCode).collect(Collectors.toList());
        // 组装本端rfid信息
        if (rfidCodes != null && rfidCodes.size() > 0) {
            List<RfidInfoResp> infoResps = rfidInfoDao.queryRfidInfoByRfidCodes(rfidCodes);
            if (infoResps != null && infoResps.size() > 0) {
                jumpFiberInfoRespForAppList.stream().forEach(obj -> {
                    infoResps.stream().forEach(rfids -> {
                        if (obj.getRfidCode().equals(rfids.getRfidCode())) {
                            obj.setRfidStatus(rfids.getRfidStatus());
                        }
                    });
                });
            }
        ...
        // 组装对端信息
        if (oppositeRfidCode != null && oppositeRfidCode.size() > 0) {
            List<RfidInfoResp> infoResps = rfidInfoDao.queryRfidInfoByRfidCodes(oppositeRfidCode);
            if (infoResps != null && infoResps.size() > 0) {
                jumpFiberInfoRespForAppList.stream().forEach(obj -> {
                    infoResps.stream().forEach(rfids -> {
                        if (obj.getOppositeRfidCode().equals(rfids.getRfidCode())) {
                            obj.setOppositeRfidStatus(rfids.getRfidStatus());
                        }
                    });
                });
            }
    ...
    public Result operationJumpFiberInfoReqForApp(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        ...
        if (jumpFiberMaxNum < batchOperationJumpFiberInfoForApp.getOperationJumpFiberInfoReqForAppList().size()) {
            return ResultUtils.warn(RfIdResultCode.EXCEED_JUMP_CORE_MAX_NUM, I18nUtils.getSystemString(RfIdI18nConstant.EXCEED_JUMP_CORE_MAX_NUM));
        }
        //修改时需要先删除
        ...
        if (AppConstant.OPERATOR_TYPE_UPDATE.equals(batchOperationJumpFiberInfoForApp.getUploadType()) || AppConstant.OPERATOR_TYPE_SAVE.equals(batchOperationJumpFiberInfoForApp.getUploadType())) {
            ...
            if (JumpCoreConstant.RESULT_TYPE_QUERY_REALPOSITION_DEVICE_ID_ERROR.equals(flag)) {
                ...
                return ResultUtils.warn(RfIdResultCode.PORT_HAVE_JUMP_CORE, I18nUtils.getSystemString(RfIdI18nConstant.PORT_HAVE_JUMP_CORE));
    ...
    public String checkParamsToSaveForApp(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        ...
        //设置虚拟标签id 生成本端 uuid
        String rfidCode = NineteenUUIDUtils.uuid();
        for (OperationJumpFiberInfoReqForApp operationJumpFiberInfoReqForApp : batchOperationJumpFiberInfoForApp.getOperationJumpFiberInfoReqForAppList()) {
            if (ObjectUtils.isEmpty(operationJumpFiberInfoReqForApp.getRfidCode())) {
                operationJumpFiberInfoReqForApp.setRfidCode(rfidCode);
            }
            if (ObjectUtils.isEmpty(operationJumpFiberInfoReqForApp.getOppositeRfidCode())) {
                operationJumpFiberInfoReqForApp.setOppositeRfidCode(NineteenUUIDUtils.uuid());
            }
        }
        //校验当前智能标签code是否存在
        Set<String> rfidCodeList = new HashSet<>();
    ...
    public Result setResult(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        if (AppConstant.OPERATOR_TYPE_SAVE.equals(batchOperationJumpFiberInfoForApp.getUploadType())) {
            //新增
            return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.SAVE_JUMP_CORE_SUCCESS));
        } else if (AppConstant.OPERATOR_TYPE_UPDATE.equals(batchOperationJumpFiberInfoForApp.getUploadType())) {
            //修改
            return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.UPDATE_JUMP_CORE_SUCCESS));
            ...
            //修改智能标签id
            return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.UPDATE_RFID_CODE_SUCCESS));
        } else {
            return ResultUtils.warn(RfIdResultCode.PARAMS_ERROR, I18nUtils.getSystemString(RfIdI18nConstant.PARAMS_ERROR));
        }
    }
    ...
    public void updateJumpFiberInfoReqForApp(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        for (OperationJumpFiberInfoReqForApp operationJumpFiberInfoReqForApp : batchOperationJumpFiberInfoForApp.getOperationJumpFiberInfoReqForAppList()) {
            ...
            jumpFiberInfoReqForApp.setUpdateTime(UtcTimeUtil.getUtcTime());
            //如果是分路器
            if (JumpCoreConstant.BRANCHING_UNIT_1.equals(jumpFiberInfoReqForApp.getBranchingUnit())) {
                ...
                // 这里需求更改
                /**
                 * 修改时 本对端 调换  需要修改其他对端的适配器类型
                 */
                //本端rfIdCode
                String rfIdCode = jumpFiberInfoReqForApp.getRfidCode();
                //对端rfidCode
                String oppositeRfidCode = jumpFiberInfoReqForApp.getOppositeRfidCode();
                // 适配器类型
                Integer adapterType = jumpFiberInfoReqForApp.getAdapterType();
                // 修改适配器类型
                jumpFiberInfoDao.updateJumpAdapterType(adapterType, rfIdCode);
                jumpFiberInfoDao.updateJumpAdapterType(adapterType, oppositeRfidCode);
            } else {
    ...
    public void deleteJumpFiberInfoReqForApp(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        ...
        for (OperationJumpFiberInfoReqForApp operationJumpFiberInfoReqForApp : batchOperationJumpFiberInfoForApp.getOperationJumpFiberInfoReqForAppList()) {
            ...
            JumpFiberInfoReqForApp jumpFiberInfoReqForApp = new JumpFiberInfoReqForApp();
            BeanUtils.copyProperties(operationJumpFiberInfoReqForApp, jumpFiberInfoReqForApp);
            jumpFiberInfoReqForApp.setCreateTime(UtcTimeUtil.getUtcTime());
    ...
    public void saveJumpFiberInfoReqForApp(BatchOperationJumpFiberInfoForApp batchOperationJumpFiberInfoForApp) {
        ...
        for (OperationJumpFiberInfoReqForApp operationJumpFiberInfoReqForApp : batchOperationJumpFiberInfoForApp.getOperationJumpFiberInfoReqForAppList()) {
            ...
            operationJumpFiberInfoReqForApp.setCreateTime(UtcTimeUtil.getUtcTime());
     ...
     * app修改跳接智能标签id
     *
     * @param forApp app跳接信息
     * @return void
     */
    public void updateRfIdCodeForApp(BatchOperationJumpFiberInfoForApp forApp) {
        /**
         * 备注:
         * 这里的跳纤标签更换 只修改智能标签信息
         * 并 OperationJumpFiberInfoReqForApp 中的
         * rfIdCode 为旧标签
         * oppositeRfidCode 为新标签
         */
        if (forApp != null && forApp.getOperationJumpFiberInfoReqForAppList() != null && forApp.getOperationJumpFiberInfoReqForAppList().size() > 0) {
            List<OperationJumpFiberInfoReqForApp> appList = forApp.getOperationJumpFiberInfoReqForAppList();
            appList.stream().forEach(obj -> {
                //1 更换跳接标签 jump
                //修改本端智能标签id
                jumpFiberInfoDao.updateThisRfIdCodeForApp(obj);
                //修改对端端智能标签id
                jumpFiberInfoDao.updateOppositeRfIdCodeForApp(obj);
                //2 更换智能标签表
                rfidInfoService.changeLabel(obj.getOppositeRfidCode(), obj.getRfidCode());
            });
        }
//
