...
public class OdnFacilityResourcesStatisticsServiceImpl implements OdnFacilityResourcesStatisticsService {
    ...
    private String getPortCount(Integer boxSide, String frameNo, String discNo, Integer discSide, String portNo) {
        StringBuffer buffer = new StringBuffer();
        return buffer.append(boxSide).append(frameNo).append(discNo).append(discSide).append(portNo).toString().trim();
    }
    ...
    private Integer usedPortNum(String deviceId, List<JumpConnectionStatisticsResp> list) {
        Integer usedPort = 0;
        // 如果是分路器的话 端口需要去重
        Set<String> portCount = new HashSet<>();
        for (JumpConnectionStatisticsResp resp : list) {
            if (deviceId.equals(resp.getDeviceId())) {
                //本端
                portCount.add(getPortCount(resp.getBoxSide(), resp.getFrameNo(), resp.getDiscNo(), resp.getDiscSide(), resp.getPortNo()));
            }
            if (deviceId.equals(resp.getOppositeDeviceId())) {
                portCount.add(getPortCount(resp.getOppositeBoxSide(), resp.getOppositeFrameNo(), resp.getOppositeDiscNo(), resp.getOppositeDiscSide(), resp.getOppositePortNo()));
            }
