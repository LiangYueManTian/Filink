...
public class MobileServiceImpl implements MobileService {
    ...
    public Result uploadFacilityInfo(FacilityUploadDto uploadDto) {
        ...
        if (UploadTypeEnum.INSERT.ordinal() == uploadType) {
            ...
            //数据上传类型异常
            throw new BizException(RfIdResultCodeConstant.UPDATE_TYPE_IS_WRONG,
    ...
    private void updateLabelInfo(FacilityUploadDto uploadDto) {
        if (isNotEmpty(uploadDto.getBoxList())) {
            ...
            boxInfoBean.setLastUpdateTime(System.currentTimeMillis());
    ...
    private void updatePortInfo(FacilityUploadDto uploadDto) {
        if (isNotEmpty(uploadDto.getPortList())) {
            ...
            portInfoBean.setLastUpdateTime(System.currentTimeMillis());
            ...
            if (isNotEmpty(portInfoBeans)) {
                ...
                if (portInfoBean.getFrameNo().intValue() != existPortLabel.getFrameNo()
                    ...
                    if (isNotEmpty(beanList)) {
                        //目标已经有标签
                        throw new BizException(RfIdResultCodeConstant.THE_TARGET_IS_ALREADY_BOUND_TO_THE_LABEL,
                ...
                //数据库标签已经不存在了
                throw new BizException(RfIdResultCodeConstant.CAN_NOT_FIND_INFO_WITH_THIS_LABEL,
    ...
    private void updateBoardInfo(FacilityUploadDto uploadDto) {
        if (isNotEmpty(uploadDto.getBoardList())) {
            ...
            boardInfoBean.setLastUpdateTime(System.currentTimeMillis());
            //修改设备信息需要走删除再新增(涉及到设备的信息 盘 端口 改动的)
            //查询现在数据库内的信息(比较设施的信息是否发生改变)
    ...
    public Result queryFacilityINfo(FacilityQueryBean queryBean) {
        ...
        boxInfoBeans.forEach(boxInfoBean -> {
            boxInfoBean.setDeviceName(getDeviceName(queryBean.getDeviceId()));
        });
        boardInfoBeans.forEach(boardInfoBean -> {
            boardInfoBean.setDeviceName(getDeviceName(queryBean.getDeviceId()));
        });
        portInfoBeans.forEach(portInfoBean -> {
            portInfoBean.setDeviceName(getDeviceName(queryBean.getDeviceId()));
        });
    ...
    private Result addDevice(DeviceUploadDto uploadDto) {
        ...
        if (isExists) {
            throw new BizException(RfIdResultCodeConstant.DEVICE_IS_EXISTS_BUS,
...
//        数据转换
    ...
    private Result updateDevice(DeviceUploadDto uploadDto) {
            ...
            if (obj.getBoardEntity() != null && obj.getBoardEntity().size() != 0) {
                obj.getBoardEntity().forEach(board -> {
                    ...
                    if (board.getId() != null) {
                        ...
                        disc.setPutState(BoardPutStateEnum.LAY.ordinal());
        ...
        if (addFrameList.size() == 0 && addDiscList.size() == 0) {
            ...
            // t_box_real_position 坐标不会有变化 编号规则和走向也不会有变化 后面三张表会有改变
            ...
            if (boxReal == null || boxReal.size() == 0) {
                return ResultI18Utils.convertWarnResult(RfIdResultCodeConstant.QUERY_POSITION_INFO_ERROR,
    ...
    private Result checkUploadType(Integer uploadType) {
        //上传数据类型错误
        return ObjectUtils.isEmpty(uploadType) ? ResultUtils.warn(RfIdResultCodeConstant.UPLOAD_DATA_TYPE_ERROR,
    ...
    private void throwLabelIsExisted() {
        //提示用户标签已存在
        throw new BizException(RfIdResultCodeConstant.LABEL_IS_EXISTED,
