...
public class OpticCableInfoServiceImpl extends ServiceImpl<OpticCableInfoDao, OpticCableInfo> implements OpticCableInfoService {
    ...
    public Result addOpticCable(InsertOpticCableInfoReq insertOpticCableInfoReq) {
        ...
        if (cableCoreMaxNum < Integer.parseInt(insertOpticCableInfoReq.getCoreNum())) {
            return ResultUtils.warn(RfIdResultCode.OPTIC_CABLE_EXCEED_THE_CORE_MAXIMUM, I18nUtils.getSystemString(RfIdI18nConstant.OPTIC_CABLE_EXCEED_THE_CORE_MAXIMUM));
        }
        //名称校验
        if (this.checkOpticCableName(insertOpticCableInfoReq.getOpticCableId(), insertOpticCableInfoReq.getOpticCableName())) {
            return ResultUtils.warn(RfIdResultCode.OPTIC_CABLE_NAME_SAME, I18nUtils.getSystemString(RfIdI18nConstant.OPTIC_CABLE_NAME_SAME));
        }
        ...
        //保存utc标准时间
        insertOpticCableInfoReq.setCreateTime(UtcTimeUtil.getUtcTime());
        ...
        log.info("保存操作日志成功");
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.SAVE_OPTIC_CABLE_SUCCESS));
    }
    ...
    public Result queryOpticCableById(String id) {
        ...
        if (ObjectUtils.isEmpty(opticCableInfoDetail)) {
            return ResultUtils.warn(RfIdResultCode.OPTIC_CABLE_IS_NOT_EXIST, I18nUtils.getSystemString(RfIdI18nConstant.OPTIC_CABLE_IS_NOT_EXIST));
        }
    ...
    public Result updateOpticCableById(UpdateOpticCableInfoReq updateOpticCableInfoReq) {
        ...
        //保存utc标准时间
        updateOpticCableInfoReq.setUpdateTime(UtcTimeUtil.getUtcTime());
        //修改光缆信息
        ...
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.UPDATE_OPTIC_CABLE_SUCCESS));
    }
    ...
    public Result deleteOpticCableById(String id) {
        ...
        if (!ObjectUtils.isEmpty(opticCableSectionInfoList)) {
            return ResultUtils.warn(RfIdResultCode.OPTIC_CABLE_CONTAINS_OPTIC_CABLE_SECTION, I18nUtils.getSystemString(RfIdI18nConstant.OPTIC_CABLE_CONTAINS_OPTIC_CABLE_SECTION));
        }

        //删除记录
        opticCableInfoDao.updateOpticCableIsDeletedById(id, OpticCableConstant.IS_DELETED, RequestInfoUtils.getUserId(), UtcTimeUtil.getUtcTime());
        log.info("删除光缆信息成功");
        ...
        logProcess.addOperateLogInfoToCall(addLogBean, LogConstants.ADD_LOG_LOCAL_FILE);
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.DELETE_OPTIC_CABLE_SUCCESS));
    }
    ...
    public Result exportOpticCableList(ExportDto exportDto) {
        ...
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.THE_EXPORT_TASK_WAS_CREATED_SUCCESSFULLY));
    }
