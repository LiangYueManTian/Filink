...
public class CoreCoreInfoServiceImpl extends ServiceImpl<CoreCoreInfoDao, CoreCoreInfo> implements CoreCoreInfoService {
    ...
    public Result saveCoreCoreInfo(List<InsertCoreCoreInfoReq> insertCoreCoreInfoReqList) {
        ...
        coreCoreInfoReq.setUpdateTime(UtcTimeUtil.getUtcTime());
        ...
        for (InsertCoreCoreInfoReq insertCoreCoreInfoReq : insertCoreCoreInfoReqList) {
            ...
            insertCoreCoreInfoReq.setCreateTime(UtcTimeUtil.getUtcTime());
        }
        ...
        return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.SAVE_CORE_CORE_SUCCESS));
    }
    ...
    public Result operationCoreCoreInfoReqForApp(BatchOperationCoreCoreInfoReqForApp batchOperationCoreCoreInfoReqForApp) {
        ...
        if (AppConstant.OPERATOR_TYPE_UPDATE.equals(batchOperationCoreCoreInfoReqForApp.getUploadType())) {
            for (OperationCoreCoreInfoReqForApp operationCoreCoreInfoReqForApp : batchOperationCoreCoreInfoReqForApp.getOperationCoreCoreInfoReqForAppList()) {
                ...
                coreCoreInfoForApp.setUpdateTime(UtcTimeUtil.getUtcTime());
        ...
        if (AppConstant.OPERATOR_TYPE_UPDATE.equals(batchOperationCoreCoreInfoReqForApp.getUploadType()) || AppConstant.OPERATOR_TYPE_SAVE.equals(batchOperationCoreCoreInfoReqForApp.getUploadType())) {
            // 删除数据 当保存的时候
            List<DeleteCoreInfoReq> list = Lists.newArrayList();

            for (OperationCoreCoreInfoReqForApp obj : batchOperationCoreCoreInfoReqForApp.getOperationCoreCoreInfoReqForAppList()) {
                //统一设置uuid
                obj.setCoreCoreId(NineteenUUIDUtils.uuid());
                obj.setCreateTime(UtcTimeUtil.getUtcTime());
                obj.setCreateUser(RequestInfoUtils.getUserId());
                DeleteCoreInfoReq de1 = new DeleteCoreInfoReq();
                de1.setIntermediateNodeDeviceId(obj.getIntermediateNodeDeviceId());
                de1.setCableCoreNo(obj.getCableCoreNo());
                de1.setResource(obj.getResource());

                DeleteCoreInfoReq de2 = new DeleteCoreInfoReq();
                de2.setIntermediateNodeDeviceId(obj.getIntermediateNodeDeviceId());
                de2.setResource(obj.getOppositeResource());
                de2.setCableCoreNo(obj.getOppositeCableCoreNo());
            ...
            if (list != null && list.size() != 0) {
                coreCoreInfoDao.deleteCoreCoreINnfoForApp(list);
            }
        ...
        if (AppConstant.OPERATOR_TYPE_SAVE.equals(batchOperationCoreCoreInfoReqForApp.getUploadType())) {
            ...
            return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.UPDATE_CORE_CORE_SUCCESS));
            //删除
        } else if (AppConstant.OPERATOR_TYPE_DELETE.equals(batchOperationCoreCoreInfoReqForApp.getUploadType())) {
            return ResultUtils.success(RfIdResultCode.SUCCESS, I18nUtils.getSystemString(RfIdI18nConstant.DELETE_CORE_CORE_SUCCESS));
        } else {
            return ResultUtils.warn(RfIdResultCode.PARAMS_ERROR, I18nUtils.getSystemString(RfIdI18nConstant.PARAMS_ERROR));
        }
    }
