...
public class MobileAsync {
    ...
    public void calcOldPosition(DeviceUploadDto uploadDto, List<RealRspDto> boxReal, List<RealRspDto> upFrameList,
        ...
        if (boxReal == null || boxReal.size() == 0) {
            ...
            throw new BizException(RfIdResultCode.NOT_QUERY_DATA, RfIdI18nConstant.NOT_QUERY_DATA);
        }
        ...
        boxReal.stream().forEach(boxs -> {
            //获取根据boxId 获取下面的框
            frameRealPosition.stream().filter(obj -> boxs.getId().equals(obj.getParentId())).forEach(frame -> {
                ...
                //刷盘的坐标
                discRealPosition.stream().filter(obj -> frame.getId().equals(obj.getParentId())).forEach(disc -> {
                    //这里的模板编号变了
                    ...
                    // 这里需要看模板是否是竖着的
                    BaseMould mould = uploadDto.getBoardMould();
                    boolean boardPutState = frameMould.getPutState() == BoardPutStateEnum.STELLEN.ordinal();
                    int portCol;
                    int portRow;
                    if (boardPutState) {
                        portCol = mould.getRow();
                        portRow = mould.getCol();
                    } else {
                        portCol = mould.getCol();
                        portRow = mould.getRow();
                    }
                    portRealPosition.stream().filter(obj -> disc.getId().equals(obj.getParentId())).forEach(port -> {
                        templateHelper.appCalcQueenCoordinate(disc, TemplateTypeEnum.TEMPLATE_TYPE_DISC.ordinal(),
                                portCol, portRow, port);
    ...
    public void calcNewPosition(DeviceUploadDto uploadDto, List<RealRspDto> boxReal, List<RealRspDto> addFrameList,
        ...
        if (boxReal == null || boxReal.size() == 0) {
            ...
            throw new BizException(RfIdResultCode.ADD_DEVICE_ENTITY_PARAM_BOX_DATA_NULL,
        ...
        if (addFrameList == null || addFrameList.size() == 0) {
            ...
            throw new BizException(RfIdResultCode.ADD_DEVICE_ENTITY_PARAM_NEW_FRAME_DATA_NULL,
        ...
        if (addDiscList == null || addDiscList.size() == 0) {
            ...
            throw new BizException(RfIdResultCode.ADD_DEVICE_ENTITY_PARAM_NEW_DISC_DATA_NULL,
        ...
        switch (ruleEnum) {
            ...
            case TEMPLATE_TREND_RIGHT_DOWN:
                ...
                if (trendFlag) {
                    ...
                    int n = modNum / col;
                    realRspDto.setColNo(m == 0 ? 1 : col + 1 - m);
                    realRspDto.setRowNo(m == 0 ? row + 1 - n : row - n);
                }
                break;
            default:
