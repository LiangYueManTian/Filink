...
public class AlarmDisposeServiceImpl extends ServiceImpl<AlarmReceiveDao, AlarmReceive> implements AlarmDisposeService {
    ...
    public void alarmForward(List<AlarmForwardRule> alarmForwardRules, AlarmCurrent alarmCurrent) throws ClientException {
        for (AlarmForwardRule alarmForwardRule : alarmForwardRules) {
            ...
            for (Object object : userList) {
                ...
                if (pushType == 1) {
                    //发送短信
                    log.info("alarm report: send sms,user phone:{}", user.getPhoneNumber());
                    sendSms(user, alarmCurrent);
                } else {
                    //发送邮件
                    log.info("alarm report: send email,user email:{}", user.getEmail());
    ...
    private void sendEmail(User user, AlarmCurrent alarmCurrent) throws ClientException {
        ...
        alarmNotice.setAlarmtime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").
                format(new Date(alarmCurrent.getAlarmBeginTime())));
    ...
    public void imageDispose(AlarmPictureMsg alarmPictureMsg) {
        ...
        String deviceId = alarmPictureMsg.getDeviceId();
        ...
        batchUploadPicReq.setDeviceId(deviceId);
        if (ListUtil.isNotEmpty(alarmCurrents)) {
            ...
            if (ListUtil.isNotEmpty(alarmFilterList)) {
        ...
        try {
            ...
            log.info("alarm picture: save alarm picture success");
        } catch (Exception e) {
            log.error("alarm picture: upload the picture error", e);
        }
    ...
    public void autoCleanAlarm(AlarmCurrentInfo alarmCurrentInfo) {
        ...
        if (ObjectUtils.isEmpty(orderDeviceInfo)) {
            log.warn("alarm report--alarm clean: error, no find device");
            return;
        }
    ...
    public void alarmCastOrder(AlarmOrderRule alarmOrderRule, AlarmCurrent alarmCurrent) {
        ...
        if (orderType == AlarmCurrentConstants.ALARM_STATUS_TWO) {
            //消障工单，目前的需求只会产生消障工单
    ...
    private void cleanAlarm(Query query, AlarmCurrentInfo alarmCurrentInfo, OrderDeviceInfo orderDeviceInfo) {
        ...
        for (AlarmCurrent alarmCurrent : alarmCurrents) {
            ...
            if (null != alarmSystemNearTime && alarmSystemNearTime < alarmCurrentInfo.getAlarmBeginTime()) {
                ...
                log.info("alarm report--alarm clean: clean alarm {}", alarmCurrentInfo.getAlarmCode());
                AlarmCurrent alarmCurrentList = mongoTemplate.findOne(query1, AlarmCurrent.class);
                // 不存在更新主控id状态
                log.info("alarm report--alarm clean: update control status");
     ...
     * 批量上传告警图片列表
     *
     * @param batchUploadPicReqList 图片列表
     * @return Result
     */
    public Boolean uploadImagesForAlarm(List<BatchUploadPicReq> batchUploadPicReqList, String time) throws Exception {
        return this.uploadImages(batchUploadPicReqList, time);
    }
     ...
     * @return Boolean
     */
    public Boolean uploadImages(List<BatchUploadPicReq> batchUploadPicReqList, String time) throws Exception {
        //必填参数校验
        if (this.checkProcParamsForBatchUpload(batchUploadPicReqList)) {
            return false;
        }
        //上传图片文件信息
        ...
        //解析图片信息
        if (!this.getFileInfo(batchUploadPicReqList, picMap, picRelationInfoList)) {
            return false;
        }
        //批量上传图片
        if (!this.uploadFileImage(picMap, picsUrlMap, picRelationInfoList)) {
            return false;
        }
        for (PicRelationInfo picRelationInfo : picRelationInfoList) {
            picRelationInfo.setCreateTime(new Date(Long.parseLong(time)));
        }
        //远程调用图片服务批量保存图片记录
        devicePicFeign.saveImagesDataForFeign(picRelationInfoList);
        return true;
    }
     ...
     * 获取文件信息方法
     *
     * @param batchUploadPicReqList 批量上传图片请求
     * @param picMap             上传图片信息
     * @param picRelationInfoList   保存图片信息
     * @return Boolean
     */
    private Boolean getFileInfo(List<BatchUploadPicReq> batchUploadPicReqList, Map<String, ImageUploadBean> picMap,
                                List<PicRelationInfo> picRelationInfoList) {
        //获取图片信息
        for (int i = 0; i < batchUploadPicReqList.size(); i++) {
            //校验图片类型
            if (!ImageExtensionEnum.containExtension(batchUploadPicReqList.get(i).getType())) {
                return false;
            }
            PicRelationInfo picRelationInfo = new PicRelationInfo();
            ImageUploadBean imageUploadBean = new ImageUploadBean();
            String uuid = UUID.randomUUID().toString().replaceAll("-", "");
            String type = batchUploadPicReqList.get(i).getType();
            byte[] bytes = HexUtil.hexStringToBytes(batchUploadPicReqList.get(i).getHexString());
            int size = HexUtil.getLenFromByte(bytes);
            BeanUtils.copyProperties(batchUploadPicReqList.get(i), picRelationInfo);
            ...
            imageUploadBean.setFileHexData(batchUploadPicReqList.get(i).getHexString());
     ...
     * @param picRelationInfoList 保存图片记录
     * @return Boolean
     */
    protected Boolean uploadFileImage(Map<String, ImageUploadBean> picMap, Map<String, ImageUrl> picsUrlMap,
        ...
        //获取图片url
        for (int i = 0; i < picRelationInfoList.size(); i++) {
            picRelationInfoList.get(i).setPicUrlBase(picsUrlMap.get(picRelationInfoList.get(i).getDevicePicId()).getOriginalUrl());
            picRelationInfoList.get(i).setPicUrlThumbnail(picsUrlMap.get(picRelationInfoList.get(i).getDevicePicId()).getThumbUrl());
        }
        return true;
    }
    ...
    public String deviceTypeEnum(String code) {
        ...
        switch (code) {
            ...
            case "150":
                ...
                break;
            case "210":
                name = systemLanguageUtil.getI18nString(AppConstant.OUTDOOR_CABINET);
                break;
            default:
        }
        return name;
