...
public class RedisListenerForAlarmCurrent extends KeyExpirationEventMessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        if (expiredKey.startsWith(AppConstant.ALARM_MONGODB_PRE)) {
            ...
            // 添加到历史告警中
            log.info("alarm current to history: save history, alarm current id: {}",
                    mongodbId);
            Result result = alarmHistoryFeign.insertAlarmHistoryFeign(alarmHistory);
            if (result == null) {
                log.warn("alarm current to history: save history failed, alarm current id: {}",
                        mongodbId);
                return;
            }
            // 删除已清除的数据
            mongoTemplate.remove(alarmCurrent);
            log.info("alarm current to history: remove current success");
        }
