...
public class AlarmMsgDevice {
    ...
    public void alarmMsg(Map alarmMsgMap) {
        try {
            if (null != alarmMsgMap.get(AlarmCurrent18n.PARAMS)) {
                ...
                for (AlarmMsg alarmMessage : dataList) {
                    ...
                    if (AppConstant.ONE.equals(alarmMessage.getAlarmStatus())) {
                        ...
                        log.info("alarm report--alarm clean: device id:{}, control id:{}, alarm code:{}",
                                alarmMessage.getAlarmSource(), alarmMessage.getEquipmentId(), alarmMessage.getAlarmCode());
                        alarmDisposeService.autoCleanAlarm(alarmCurrentInfo);
                    } else {
                        //如果是告警，调用 AlarmReceiveService 中告警解析的接口
                        log.info("alarm report: device id:{}, control id:{}, alarm code:{}",
                                alarmMessage.getAlarmSource(), alarmMessage.getEquipmentId(), alarmMessage.getAlarmCode());
            ...
            log.error("alarm report: alarm msg error:{}", e);
        }
    }
    ...
    public void alarmAdviceReceives(AlarmInfo alarmInfo) throws ClientException {
        if (!ObjectUtils.isEmpty(alarmInfo)) {
            ...
            log.info("alarm report: receive the kafka of send forward and transfer workflow business, " +
                    "deviceId:{}, control id:{}, alarm name:{}",
                    alarmCurrent.getAlarmSource(), alarmCurrent.getControlId(), alarmCurrent.getAlarmName());
            //告警灯推送，调用websocket+
            ...
            alarmMessageSource.setAlarmSourceTypeId(alarmCurrent.getAlarmSourceTypeId());
            alarmMessageSource.setResponsibleDepartment(alarmCurrent.getResponsibleDepartmentId());
            ...
            log.info("alarm report: send WebSocket");
            alarmStreams.webSocketoutput().send(message);
            // 是否满足告警远程通知
            List<AlarmForwardRule> alarmForwardRule = alarmInfo.getAlarmForwardRuleList();
            if (!ListUtil.isEmpty(alarmForwardRule)) {
                // 告警远程通知
                log.info("alarm report: send alarm forward");
                alarmDisposeService.alarmForward(alarmForwardRule, alarmCurrent);
            }
            if (null != alarmInfo.getAlarmOrderRule()) {
                log.info("alarm report: send the kafka of transfer workflow business");
                alarmMsgSender.sendWorkFlow(alarmInfo);
            }
        }
    }
     ...
     * @param alarmInfo 告警信息
     */
    public void alarToWorkFlow(AlarmInfo alarmInfo) {
        AlarmCurrent alarmCurrent = alarmInfo.getAlarmCurrent();
        AlarmOrderRule alarmOrderRule = alarmInfo.getAlarmOrderRule();
        // 告警转工单
        log.info("alarm report: receive the kafka of transfer workflow business, deviceId:{}, control id:{}, alarm name:{}",
                alarmCurrent.getAlarmSource(), alarmCurrent.getControlId(), alarmCurrent.getAlarmName());
    ...
    public void alarmPictureDevice(Map alarmPictureMap) {
        try {
            //此处只有一张图片, alarmCode 告警编码   deviceId 设施ID doorNumber 门编号 suffix 图片后缀  pictureInfo 16进制图片信息
            ...
            //告警图片处理逻辑
            log.info("alarm picture: receive the kafka of alarm picture, alarm code:{}, device id:{}",
                    alarmPictureMsg.getAlarmCode(), alarmPictureMsg.getDeviceId());
