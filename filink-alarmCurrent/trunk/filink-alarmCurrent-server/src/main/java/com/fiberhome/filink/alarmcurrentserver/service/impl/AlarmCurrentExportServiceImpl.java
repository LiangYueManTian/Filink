...
public class AlarmCurrentExportServiceImpl implements AlarmCurrentExportService {
    ...
    public Result exportAlarmList(ExportDto exportDto) {
        ...
        try {
            ...
            fe.printStackTrace();
            return ResultUtils.warn(AlarmCurrentResultCode.EXPORT_NO_DATA, I18nUtils.getSystemString(AppConstant.EXPORT_NO_DATA));
        } catch (FilinkExportDataTooLargeException fe) {
            return getExportToLargeMsg(fe);
        } catch (FilinkExportTaskNumTooBigException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(AlarmCurrentResultCode.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS,
                    I18nUtils.getSystemString(AppConstant.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            e.printStackTrace();
            return ResultUtils.warn(AlarmCurrentResultCode.FAILED_TO_CREATE_EXPORT_TASK,
    ...
    private Result getExportToLargeMsg(FilinkExportDataTooLargeException fe) {
        log.error("export alarm current error", fe);
        ...
        return ResultUtils.warn(AlarmCurrentResultCode.EXPORT_DATA_TOO_LARGE, msg);
    }
    ...
    public void updateAlarmCleanStatusCompensation() {
        ...
        // 添加到历史告警中
        log.info("schedule alarm current to history: save history, count: {}",
                alarmHistoryList.size());
        Result result = alarmHistoryFeign.insertAlarmHistoryListFeign(alarmHistoryList);
        if (result == null) {
            log.warn("schedule alarm current to history: save history failed");
            return;
        }
        // 删除已清除的数据
        mongoTemplate.remove(query1, AlarmCurrent.class);
        log.info("schedule alarm current to history: remove current success");
    }
