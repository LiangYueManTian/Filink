...
public class AlarmStatisticsServiceImpl extends ServiceImpl<AlarmStatisticsTempDao, AlarmStatisticsTemp>
    ...
    public Result queryAlarmCountByLevel(QueryCondition<AlarmStatisticsParameter> queryCondition) {
        ...
        Aggregation agg = Aggregation.newAggregation(
                ...
                Aggregation.group(AppConstant.AREA_ID).count().as("groupNum"),
        ...
        Map<String, AlarmAreaRateStatisticsDto> map = new HashMap<>(64);
        list.forEach(u -> {
    ...
    public Result alarmIncrementalStatistics(QueryCondition<AlarmStatisticsParameter> queryCondition, String timeType, boolean flag) {
        ...
        longs.add(DateUtil.getTimeZeroTime(alarmStatisticsParameter.getBeginTime()));
        longs.add(alarmStatisticsParameter.getEndTime());
        if (flag) {
            criteria = getIncremental(criteria, timeType, longs);
        } else {
            criteria = Criteria.where("timeType").is(timeType).and("groupArea").in(alarmStatisticsParameter.getAreaList())
                    ...
                    .and("groupTime").lte(longs.get(1)).gte(longs.get(0));
        }
        //根据时间分组，统计告警的数量
        Aggregation agg = Aggregation.newAggregation(
                Aggregation.match(criteria),
        ...
        if (AppConstant.DAY.equals(timeType)) {
            timeList = DateUtil.getTimeDayBetween(longs.get(0), longs.get(1));
        } else if (AppConstant.WEEK.equals(timeType)) {
            timeList = DateUtil.getTimeWeekBetween();
        } else if (AppConstant.MONTH.equals(timeType)) {
            timeList = DateUtil.getTimeMonthBetween();
        }
        ...
        //将list时间戳格式化
        list.forEach(u -> u.setGroupLevel(new SimpleDateFormat("yyyy-MM-dd").format(Long.valueOf(u.getGroupLevel()))));
        List<AlarmStatisticsGroupInfo> listStatistics = new ArrayList<>(list);
        //返回完整的时间间隔
        timeList.forEach(u -> {
            if (list.stream().noneMatch(h -> h.getGroupLevel().equals(u))) {
     ...
     * 告警增量统计权限
     *
     * @param criteria mongodb条件信息
     * @param longs    统计条件信息
     * @param timeType 告警时间类型
     * @return 告警增量统计权限信息
     */
    private Criteria getIncremental(Criteria criteria, String timeType, List<Long> longs) {
        ...
        if (!user.getId().equals(AppConstant.ONE)) {
            ...
            if (ListUtil.isEmpty(areaIds) || ListUtil.isEmpty(deviceTypes)) {
                criteria = Criteria.where("groupTime").lte(longs.get(1)).gte(longs.get(0))
                        .and("timeType").is(timeType)
                        .and("groupType").in((Object) null).and("groupArea").in((Object) null);
            } else {
                criteria = Criteria.where("groupTime")
                        .lte(longs.get(1)).gte(longs.get(0)).and("timeType").is(timeType)
                        .and("groupType").in(deviceTypes).and("groupArea").in(areaIds);
            }
        } else {
            criteria = Criteria.where("groupTime")
                    .lte(longs.get(1)).gte(longs.get(0)).and("timeType").is(timeType);
    ...
    private List<AlarmStatisticsGroupInfo> queryMongodbData(AlarmStatisticsParameter alarmStatisticsParameter) {
        ...
        if (alarmStatisticsParameter.getAlarmStatisticsName().equals(AlarmCurrent18n.DEVICE_ID)) {
            ...
            criteria = Criteria.where(AppConstant.AREA_ID).in(alarmStatisticsParameter.getAreaList())
                    .and(AppConstant.ALARM_SOURCE_TYPE_ID).in(alarmStatisticsParameter.getDeviceIds())
                    .and(AppConstant.ALARM_BEGIN_TIME).lt(alarmStatisticsParameter.getEndTime())
                    .gt(alarmStatisticsParameter.getBeginTime());
            //分组查询根据
        ...
        Aggregation agg = Aggregation.newAggregation(
                Aggregation.match(criteria),
                Aggregation.group(alarmStatisticsParameter.getAlarmStatisticsName(), AppConstant.AREA_ID).count().as("groupNum"),
                Aggregation.project().and(alarmStatisticsParameter.getAlarmStatisticsName()).as("groupLevel")
                        .and(AppConstant.AREA_ID).as("groupArea").and("groupNum").as("groupNum").andExclude("_id"));
        //查询当前表的数据
        AggregationResults<AlarmStatisticsGroupInfo> results =
                this.mongoTemplate.aggregate(agg, AppConstant.ALARM_CURRENT, AlarmStatisticsGroupInfo.class);
        //查询历史表的数据
    ...
    public void queryStatisticsData(AlarmSourceHomeParameter alarmSourceHomeParameter) {
        ...
        if (type.equals(AppConstant.DAY)) {
            ...
            Aggregation agg = Aggregation.newAggregation(
                    Aggregation.match(criteria),
                    Aggregation.group(AppConstant.ALARM_SOURCE_TYPE_ID, "area_id").count().as("groupNum"),
                    Aggregation.project().and(AppConstant.ALARM_SOURCE_TYPE_ID).as("groupLevel")
                            .and("area_id").as("groupArea").and("groupNum").as("groupNum").andExclude("_id"));
            //当前告警
            AggregationResults<AlarmStatisticsGroupInfo> results =
                    this.mongoTemplate.aggregate(agg, AppConstant.ALARM_CURRENT, AlarmStatisticsGroupInfo.class);
            //历史告警
            ...
            for (AlarmStatisticsGroupInfo history : listHistory) {
                boolean has = true;
                for (AlarmStatisticsGroupInfo current : list) {
                    if (history.getGroupArea().equals(current.getGroupArea())
                            && history.getGroupLevel().equals(current.getGroupLevel())) {
                        has = false;
                        current.setGroupNum(current.getGroupNum() + history.getGroupNum());
                        break;
                    }
                }
                if (has) {
    ...
    public Result queryAlarmNameGroup(QueryCondition<AlarmStatisticsParameter> queryCondition) {
        ...
        // 排序
        List<AlarmDeviceParameter> collect = deviceParameterList.stream().sorted(Comparator.comparing(AlarmDeviceParameter::getCount)
                .reversed()).limit(alarmStatisticsParameter.getTopCount()).collect(Collectors.toList());
    ...
    public Result queryAlarmNameHomePage(AlarmHomeParameter alarmHomeParameter) {
        ...
        Criteria criteria = Criteria.where(AppConstant.ALARM_SOURCE).is(alarmHomeParameter.getDeviceId())
                .and(AppConstant.ALARM_CLEAN_STATUS).is(AlarmCurrentConstants.ALARM_STATUS_THREE)
    ...
    public void querySourceIncremental(AlarmSourceHomeParameter alarmSourceHomeParameter) {
        // 日统计
        if (alarmSourceHomeParameter.getSource() == AlarmCurrentConstants.ALARM_STATUS_ONE) {
            // 开始时间为前2天
            alarmSourceHomeParameter.setBeginTime(DateUtil.getAdvanceNumberDay(1));
            // 结束时间为前一天
            alarmSourceHomeParameter.setEndTime(DateUtil.getAdvanceNumberEndDay());
            ...
            //数据并集
            list = new ArrayList<>(list);
            for (AlarmDeviceParameter u : listHistory) {
                if (list.stream().noneMatch(h -> h.getAlarmSource().equals(u.getAlarmSource()))) {
                    list.add(u);
                } else {
                    list.forEach(h -> {
            ...
            // 周统计
        } else if (alarmSourceHomeParameter.getSource() == AlarmCurrentConstants.ALARM_STATUS_TWO) {
            //开始时间为前15周
            alarmSourceHomeParameter.setBeginTime(DateUtil.getAdvanceNumberWeek(1));
            //结束时间为上一周
            alarmSourceHomeParameter.setEndTime(DateUtil.getAdvanceNumberEndWeek());
    ...
    public Result queryAlarmSourceIncremental(AlarmSourceHomeParameter alarmSourceHomeParameter) {
        //查询mongodb数据
        List<String> timeList = new ArrayList<>();
        Long beginTime = alarmSourceHomeParameter.getBeginTime();
        Long endTime = alarmSourceHomeParameter.getEndTime();
        String type = alarmSourceHomeParameter.getType();
        String deviceId = alarmSourceHomeParameter.getDeviceId();
        long current = System.currentTimeMillis();
        Long instantBegin = null;
        Long instantEnd = null;
        if (AppConstant.DAY.equals(type)) {
            timeList = DateUtil.getTimeDayBetween(beginTime, endTime);
            if (!(current < beginTime || current > endTime)) {
                instantBegin = DateUtil.getTimeZeroTime(current);
                instantEnd = DateUtil.getTimeLongDayEnd(current);
            }
        } else if (AppConstant.WEEK.equals(type)) {
            timeList = DateUtil.getTimeWeekBetween(beginTime, endTime);
            long weekBegin = DateUtil.getTimeLongWeekBegin(beginTime);
            if (!(current < weekBegin || current > endTime)) {
                instantBegin = DateUtil.getTimeLongWeekBegin(current);
                instantEnd = DateUtil.getTimeLongDayEnd(current);
            }
        }
        Criteria criteria = Criteria.where("type").is(type)
                .and("deviceId").is(deviceId)
                .and("groupTime").lte(String.valueOf(beginTime))
                .gte(String.valueOf(endTime));
        Query query = new Query(criteria);
        List<AlarmSourceIncremental> list = this.mongoTemplate.find(query, AlarmSourceIncremental.class);
        //将list时间戳格式化
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        list.forEach(u -> u.setGroupTime(format.format(Long.valueOf(u.getGroupTime()))));
        if (instantBegin != null) {
            Query queryTemp = new Query(Criteria.where(AppConstant.ALARM_BEGIN_TIME)
                    .gte(instantBegin).lte(instantEnd).and(AppConstant.ALARM_SOURCE).is(deviceId));
            long countCurrent = mongoTemplate.count(queryTemp, AlarmCurrent.class);
            long countHistory = mongoTemplate.count(queryTemp, AlarmHistory.class);
            AlarmSourceIncremental alarmSourceIncremental = new AlarmSourceIncremental();
            alarmSourceIncremental.setGroupTime(format.format(instantBegin));
            alarmSourceIncremental.setCount(countCurrent + countHistory);
            list.add(alarmSourceIncremental);
        }
        //返回完整的时间间隔
        List<AlarmSourceIncremental> listStatistics = new ArrayList<>(list);
        timeList.forEach(u -> {
            if (list.stream().noneMatch(h -> h.getGroupTime().equals(u))) {
                AlarmSourceIncremental alarmSourceIncremental = new AlarmSourceIncremental();
    ...
    private Criteria selectPermissionsStatus(Criteria criteria) {
        ...
        if (!user.getId().equals(AppConstant.ONE)) {
            ...
            criteria = Criteria.where(AppConstant.ALARM_CLEAN_STATUS).is(AlarmCurrentConstants.ALARM_STATUS_THREE);
        }
        return criteria;
    }
    ...
    private Criteria selectPermissionIds(Criteria criteria) {
        ...
        if (!userId.equals(AppConstant.ONE)) {
            User user = alarmCurrentService.getUser();
            List<String> deviceTypes = alarmCurrentService.getDeviceTypes(user);
            List<String> areaIds = alarmCurrentService.getUserAreaIds(user);
            if (ListUtil.isEmpty(areaIds) || ListUtil.isEmpty(deviceTypes)) {
                criteria = Criteria.where(AppConstant.ALARM_SOURCE_TYPE_ID).in((Object) null).and(AppConstant.AREA_ID).in((Object) null)
                        .and(AlarmCurrent18n.ALARM_TYPE).is(AlarmCurrentConstants.ALARM_STATUS_ONE);
            } else {
            ...
            criteria = Criteria.where(AlarmCurrent18n.ALARM_TYPE).is(AlarmCurrentConstants.ALARM_STATUS_ONE);
        }
        return criteria;
    }
        ...
        if (!user.getId().equals(AppConstant.ONE)) {
            ...
            if (ListUtil.isEmpty(areaIds) || ListUtil.isEmpty(areaIds)) {
                criteria = Criteria.where(AppConstant.ALARM_SOURCE_TYPE_ID).in((Object) null).and(AppConstant.AREA_ID).in((Object) null)
                        ...
                        .gt(alarmTime.getStartTime()).and(AlarmCurrent18n.ALARM_TYPE).is(AlarmCurrentConstants.ALARM_STATUS_ONE);
            } else {
                criteria = Criteria.where(AppConstant.ALARM_SOURCE_TYPE_ID).in(deviceTypes).and(AppConstant.AREA_ID).in(areaIds)
                        .and(AppConstant.ALARM_BEGIN_TIME).lt(alarmTime.getEndTime())
                        .gt(alarmTime.getStartTime()).and(AlarmCurrent18n.ALARM_TYPE).is(AlarmCurrentConstants.ALARM_STATUS_ONE);
