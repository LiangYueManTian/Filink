...
public class AlarmCurrent {
    ...
    public String getTranslationAlarmFixedLevel() {
        ...
        if (alarmFixedLevel != null) {
            if (AppConstant.ONE.equals(this.alarmFixedLevel)) {
    ...
    public String getTranslationAlarmContinousTime() {
        ...
        String continuousTimeStr = "";
        if (this.alarmCleanTime != null && this.alarmCleanTime.longValue() > 0) {
            continuousTime = this.alarmCleanTime - this.alarmBeginTime;
        } else {
            continuousTime = System.currentTimeMillis() - this.alarmBeginTime;
        }
        if (continuousTime < 0) {
            return continuousTimeStr;
        }
        long time = (long) Math.ceil(continuousTime / 1000 / 60 / 60);
        if (time == 0) {
            continuousTimeStr = '1' + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.HOUR);
        } else if (time >= AlarmCurrentConstants.ALARM_BQLO) {
            // 年
            continuousTimeStr = Math.toIntExact(time / 8760) + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.YEAR)
                    + (int) Math.ceil(time % 8760 / 720) + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.MONTH);
        } else if (time >= AlarmCurrentConstants.ALARM_QLO) {
            // 月
            continuousTimeStr = Math.toIntExact(time / 720) + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.MONTH)
                    + (int) Math.ceil(time % 720 / 24) + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.DAY);
        } else if (AlarmCurrentConstants.ALARM_ES <= time) {
            // 天
            continuousTimeStr = Math.toIntExact(time / 24)
            ...
            // 时
            continuousTimeStr = (int) Math.ceil(time) + I18nUtils.getString(ExportApiUtils.getExportLocales(), AppConstant.HOUR);
        }
        return continuousTimeStr;
    }
    ...
    public String getTranslationAlarmConfirmStatus() {
        return AlarmCurrentConstants.ALARM_STATUS_ONE == alarmConfirmStatus
