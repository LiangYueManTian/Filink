...
public class AlarmCurrentExportServiceImpl implements AlarmCurrentExportService {
    ...
    public Result exportAlarmList(ExportDto exportDto) {
        Export export;
        try {
            export = alarmCurrentExport.insertTask(exportDto, AppConstant.SERVER_NAME,
                    I18nUtils.getString(AppConstant.OPERATE_ALARM_LIST_NAME));
        } catch (FilinkExportNoDataException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(LogFunctionCodeConstant.EXPORT_NO_DATA, I18nUtils.getString(AppConstant.EXPORT_NO_DATA));
        ...
        alarmCurrentExport.exportData(export);
    ...
    private void addLogByExport(ExportDto exportDto) {
        ...
        addLogBean.setFunctionCode("1705005");
        //新增操作日志
