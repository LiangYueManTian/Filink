...
import com.fiberhome.filink.alarmcurrentserver.bean.AlarmNotices;
...
import com.fiberhome.filink.alarmcurrentserver.enums.DeviceTypeEnum;
...
public class AlarmDisposeServiceImpl extends ServiceImpl<AlarmReceiveDao, AlarmReceive> implements AlarmDisposeService {
    ...
    private void sendEmail(User user, AlarmCurrent alarmCurrent) throws ClientException {
        ...
        alarmNotice.setDevicetype(DeviceTypeEnum.getDesc(alarmCurrent.getAlarmSourceTypeId()));
        alarmNotice.setRegion(alarmCurrent.getAreaName());
        //TODO 暂时没有完全确定邮件模板，此处暂时中文硬编码
        String msg = I18nUtils.getString(AlarmCurrent18n.EMAIL_NAME_TITLE) + "<br>"
                + "&nbsp&nbsp&nbsp&nbsp" + I18nUtils.getString(AlarmCurrent18n.EMAIL_NAME_CONTENT)
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_ALARM_NAME) + alarmNotice.getAlarmname() + ""
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_DEVICE_NAME) + alarmNotice.getDevicename()
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_DEVICE_TYPE) + alarmNotice.getDevicetype()
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_NAME_REGION) + alarmNotice.getRegion()
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_ALARM_TIME) + alarmNotice.getAlarmtime()
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_ALARM_DES) + alarmNotice.getAlarmdes()
                + "<br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_NAME_TIPS)
                + "<br><br><br><br><br>" + I18nUtils.getString(AlarmCurrent18n.EMAIL_NAME_THINK);
        AliyunEmail aliyunEmail = new AliyunEmail();
        aliyunEmail.setAccountName("filink@fi-link.net");
        aliyunEmail.setFromAlias(I18nUtils.getString(AlarmCurrent18n.INFRASTRUC_TURE));
        aliyunEmail.setHtmlBody(msg);
        aliyunEmail.setSubject(I18nUtils.getString(AlarmCurrent18n.DEVICE_TI));
        //可以同时发给多个人，用逗号隔开
    ...
    public void imageDispose(AlarmPictureMsg alarmPictureMsg) {
        //根据告警编码，设施ID，门编号在当前告警表或者告警过滤表中找出对应的告警ID，撬门，撬锁告警才会拍照，一定包含门信息
        ...
        batchUploadPicReq.setResource(AppConstant.YI);
        ...
        try {
            this.uploadImagesForAlarm(picList);
            log.info("key");
        } catch (Exception e) {
            e.printStackTrace();
            log.error("key");
    ...
    public void autoCleanAlarm(AlarmCurrentInfo alarmCurrentInfo) {
        ...
        if (ListUtil.isEmpty(doorInfoList)) {
            ...
            for (DoorInfo doorInfo : doorInfoList) {
                if (AppConstant.YI.equals(doorInfo.getDoorStatus())) {
    ...
    public void alarmCastOrder(AlarmOrderRule alarmOrderRule, AlarmCurrent alarmCurrent) {
        ...
        if (orderType == LogFunctionCodeConstant.ALARM_STATUS_TWO) {
            ...
            //添加责任单位，多个
            if (null != alarmCurrent.getResponsibleDepartmentId()) {
                String[] deptArray = alarmCurrent.getResponsibleDepartmentId().toString().split(",");
                ...
                for (String dept : deptArray) {
                    ...
                    procRelatedDepartment.setAccountabilityDept(dept);
            ...
            log.info("key:" + alarmCurrent.getAlarmName());
        }
    }
    ...
    private void cleanAlarm(Query query, AlarmCurrentInfo alarmCurrentInfo, OrderDeviceInfo orderDeviceInfo) {
        ...
        if (alarmCurrents.size() == 1) {
            AlarmCurrent alarmCurrent = alarmCurrents.get(0);
            Long alarmSystemNearTime = alarmCurrents.get(0).getAlarmSystemNearTime();
            ...
            if (null != alarmSystemNearTime && alarmSystemNearTime < alarmCurrentInfo.getAlarmBeginTime()) {
                ...
                // 不存在更新主控id状态
                selectEquipmentId(alarmCurrentList, alarmCurrentInfo.getEquipmentId());
                log.info("key：" + orderDeviceInfo.getAlarmSource()
                        + "key：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").
     ...
     * 修改主控状态
     *
     * @param alarmCurrentList 当前告警信息
     * @param equipmentId 主控id
     */
    public void selectEquipmentId(List<AlarmCurrent> alarmCurrentList, String equipmentId) {
     ...
     * @return Result
     */
    public Boolean uploadImagesForAlarm(List<BatchUploadPicReq> batchUploadPicReqs) throws Exception {
        return this.uploadImages(batchUploadPicReqs);
    }
     ...
     * @return Boolean
     */
    public Boolean uploadImages(List<BatchUploadPicReq> batchUploadPicReqs) throws Exception {
        //必填参数校验
     ...
     * @return Boolean
     */
    private Boolean uploadFileImage(Map<String, ImageUploadBean> picMap, Map<String, ImageUrl> picsUrlMap,
                                    List<PicRelationInfo> picRelationInfos) {
