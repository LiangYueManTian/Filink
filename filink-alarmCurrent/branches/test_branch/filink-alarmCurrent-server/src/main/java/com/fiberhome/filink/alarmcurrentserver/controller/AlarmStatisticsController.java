...
public class AlarmStatisticsController {
    ...
    public Result queryAlarmIncrementalStatistics(@RequestBody QueryCondition<AlarmStatisticsParameter> queryCondition) {
        return alarmStatisticsTempService.alarmIncrementalStatistics(queryCondition, AppConstant.DAY);
    }
    ...
    public Result queryIncrementalStatistics(@PathVariable String timeType) {
        ...
        //大屏统计 admin用户没有地区与设施类型限制，其他用户有权限限制
        if (AppConstant.ONE.equals(user.getId())) {
            alarmStatisticsParameter.setAlarmStatisticsName(AppConstant.ADMIN_USER);
        }
        alarmStatisticsParameter.setAreaList(user.getDepartment().getAreaIdList());
        alarmStatisticsParameter.setDeviceIds(user.getRole().getRoleDevicetypeList().stream().map(
                u -> u.getDeviceTypeId()).collect(Collectors.toList()));
        ...
        return alarmStatisticsTempService.alarmIncrementalStatistics(queryCondition, timeType);
    }
    ...
    public Result queryAlarmSourceIncremental(@RequestBody AlarmSourceHomeParameter alarmSourceHomeParameter) {
        int i = 15 * 24 * 60 * 60 * 1000;
        // 是否大于15天
        boolean flag = (alarmSourceHomeParameter.getEndTime() - alarmSourceHomeParameter.getBeginTime()) > i;
        if (!flag) {
    ...
    public void tstTak() {
        Long beginTime = DateUtil.getAdvanceNumberDay(1);
        Long endTime = DateUtil.getAdvanceNumberEndDay();
//        alarmStatisticsTempService.queryIncrStatiData(beginTime, endTime, "day");
