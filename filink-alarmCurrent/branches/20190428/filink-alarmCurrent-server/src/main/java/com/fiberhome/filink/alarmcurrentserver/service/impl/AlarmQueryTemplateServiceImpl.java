...
public class AlarmQueryTemplateServiceImpl extends ServiceImpl
    ...
    public Result queryAlarmTemplateList(QueryCondition<AlarmQueryTemplateDto> queryCondition) {
      ...
      /*  alarmQueryTemplateDto.setSortField(queryCondition.getSortCondition().getSortField());
        alarmQueryTemplateDto.setSortRule(queryCondition.getSortCondition().getSortRule());*/
    ...
    public Result batchDeleteAlarmTemplate(String[] array) {
        ...
        if (!integer.equals(array.length)) {
            throw new FilinkAlarmCurrentException(I18nUtils.getString(AppConstant.DELETE_TEMPLATE_FAILED));
        }
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(AppConstant.DELETE_TEMPLATE_SUCCESS));
    }
    ...
    public Result queryAlarmTemplateById(String id) {
        ...
        if (alarmQueryTemplate == null) {
            return ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(AppConstant.ALARM_TEMPLATE_FAILED));
        }
    ...
    public Result queryAlarmQueryTemplateById(String id, QueryCondition queryCondition) {
        ...
        if (ListUtil.isEmpty(data)) {
            return ResultUtils.warn(LogFunctionCodeConstant.NO_DATA, I18nUtils.getString(AppConstant.NO_DATA));
        }
        return result;
    }
    ...
    public void setAlarmConditionTime(QueryCondition queryCondition, AlarmQueryTemplate alarmQueryTemplate,
        ...
        // 首次发生时间
        if (!ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmBeginFrontTime()) || !ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmBeginQueenTime())) {
            this.selectConditionTime("alarmBeginTime", "lt", alarmQueryTemplate.getAlarmBeginQueenTime(),
                    "gt", alarmQueryTemplate.getAlarmBeginFrontTime(), filterConditions);
        }

        // 确认时间
        if (!ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmConfirmFrontTime()) || !ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmConfirmQueenTime())) {
            this.selectConditionTime("alarmConfirmTime", "lt", alarmQueryTemplate.getAlarmConfirmQueenTime(),
                    "gt", alarmQueryTemplate.getAlarmConfirmFrontTime(), filterConditions);
        }

        //清除时间
        if (!ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmCleanFrontTime()) || !ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmCleanQueenTime())) {
            this.selectConditionTime("alarmCleanTime", "lt", alarmQueryTemplate.getAlarmCleanQueenTime(),
                    "gt", alarmQueryTemplate.getAlarmCleanFrontTime(), filterConditions);
        }

        // 最近发生时间
        if (!ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmNearFrontTime()) || !ObjectUtils.isEmpty(alarmQueryTemplate.getAlarmNearQueenTime())) {
            this.selectConditionTime("alarmNearTime", "lt", alarmQueryTemplate.getAlarmNearQueenTime(),
                    "gt", alarmQueryTemplate.getAlarmNearFrontTime(), filterConditions);
        }
    ...
    public List<FilterCondition> copy1QueryTemplateById(List<FilterCondition> filterConditions, AlarmQueryTemplate alarmQueryTemplate) {
        ...
        // 单位名称
        if (!ListUtil.isEmpty(alarmQueryTemplate.getDepartmentList())) {
            FilterCondition filterCondition = selectCondition("responsibleDepartment", "like",
//                    alarmQueryTemplate.getDepartmentList().stream().map(u ->u.getDepartmentName()).collect(Collectors.toList()));
...
//        //持续时间比较
//        if (alarmQueryTemplate.getAlarmContinous().getAlarmCmpare() != null) {
//            FilterCondition filterCondition = selectCondition("alarmCmpare", "eq",
//                    alarmQueryTemplate.getAlarmContinous().getAlarmCmpare());
//            filterConditions.add(filterCondition);
//        }
        // 附加消息
    ...
    @AddLogAnnotation(value = "update", logType = "1", functionCode = "1701803", dataGetColumnName = "templateName", dataGetColumnId = "id")
    @Override
    public Result updateAlarmTemplate(AlarmQueryTemplate alarmQueryTemplate) {
        ...
        if (integer != 1) {
            throw new FilinkAlarmCurrentException(I18nUtils.getString(AppConstant.UPDATE_TEMPLATE_FAILED));
        }
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(AppConstant.UPDATE_TEMPLATE_SUCCESS));
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    @AddLogAnnotation(value = "add", logType = "1", functionCode = "1701804", dataGetColumnName = "templateName", dataGetColumnId = "id")
    @Override
    public Result addAlarmTemplate(AlarmQueryTemplate alarmQueryTemplate) {
        ...
        if (insert != 1) {
            throw new FilinkAlarmCurrentException(I18nUtils.getString(AppConstant.ADD_TEMPLATE_FAILED));
        }
        ...
        return ResultUtils.warn(ResultCode.SUCCESS, I18nUtils.getString(AppConstant.ADD_TEMPLATE_SUCCESS));
    }
