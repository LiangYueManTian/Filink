...
public class AlarmMsgDevice {
    ...
    public void alarmMsg(Map alarmMsgMap) {
        try {
            ...
            log.info("收到kafka中告警信息，时间：" + ft.format(nowDate));
            //todo 以上时间log未实验代码，后期删除
            String param = "params";
            if (null != alarmMsgMap.get(param)) {
                List<AlarmMsg> alarmMsgList = saveAlarmInfo(alarmMsgMap, param);
                //遍历告警信息
                ...
                for (AlarmMsg alarmMessage : dataList) {
                    ...
                    if (AppConstant.YI.equals(alarmMessage.getAlarmStatus())) {
    ...
    public void alarmAdviceReceives(AlarmInfo alarmInfo) throws ClientException {
        if (null != alarmInfo) {
            ...
            socketMessage.setMsg(alarmMessageInfo);
            // 发送消息
