...
#    下载zookeeper
    docker inspect wurstmeister/zookeeper
    if [[ $? -eq 0 ]]; then
...
#    启动zookeeper
    ...
    if [[ $? -eq 0 ]]; then
        ...
        docker run -d --restart=always --privileged=true -v /etc/localtime:/etc/localtime:ro --name filink-zookeeper -p 2181:2181 -t wurstmeister/zookeeper
...
-e JAVA_OPTS="-XX:MetaspaceSize=56m \
-XX:MaxMetaspaceSize=128m \
-Xms128m -Xmx1024m -XX:+UseG1GC" \
-e PARAMS="--spring.cloud.config.server.svn.uri=https://10.199.101.2/svn/SODN/CodeV3/filink_path0/filink-config/ \
--spring.cloud.config.server.svn.username=caoliang \
--spring.cloud.config.server.svn.password=caoliang" \
-v /etc/localtime:/etc/localtime:ro \
--name filink-config \
...
-v /etc/localtime:/etc/localtime:ro \
-e JAVA_OPTS="-XX:MetaspaceSize=56m \
-XX:MaxMetaspaceSize=128m -Xms128m \
-Xmx1024m -XX:+UseG1GC" \
-e PARAMS="--spring.cloud.config.profile=pre \
