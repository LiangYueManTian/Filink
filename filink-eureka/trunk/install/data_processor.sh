...
function exportMysql() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        sed -i '1d' /data/export_data/filink.sql
    else
...
function exportEs() {
    checkDir
    mkdir /data/export_data/es
    yum install elasticdump

    elasticdump --input=http://localhost:9200/filink_alarm --output=/data/export_data/es/filink_alarm.json --searchBody '{"query":{"match_all": {}}}'

    echo "es 数据导出结束"
    echo "数据存放位置：/data/export_data/es/"
}
...
function exportData() {
    ...
    echo "请选择导出数据库类型"
    #导出mysql/mongo/es
    echo "导出mysql请输入：mysql ， 导出mongo请输入：mongo ， 导出es请输入：es ， 导出所有请输入：all"
    read chooseDatabase
    if [[ ${chooseDatabase} == "mysql" ]]; then
        exportMysql
    elif [[ ${chooseDatabase} == "mongo" ]]; then
        exportMongo
    elif [[ ${chooseDatabase} == "es" ]]; then
        exportEs
...
function importMysql() {
    ...
    if [[ $? -eq 0 ]]; then
        checkSql
        docker exec -i filink-mysql mysql -h localhost -uroot -pwistronits@123 --default-character-set=utf8  < /data/export_data/filink.sql
    else
        echo "filink-mysql容器不存在，请检查环境, 如果容器名称不是filink-mysql，请输入容器名称"
        read mysqlContainerName
        docker inspect ${mysqlContainerName}
        if [[ $? -eq 0 ]]; then
            docker exec -i ${mysqlContainerName} mysql -h localhost -uroot -pwistronits@123 --default-character-set=utf8  < /data/export_data/filink.sql
        else
            echo "该容器不存在"
            exit 1
        fi
...
function importData() {
    echo "请选择导入数据库类型"
    #导入mysql/mongo/es
    echo "导入mysql请输入：mysql ， 导入mongo请输入：mongo ， 导入es请输入：es ， 导入所有请输入：all"
    read chooseDatabase
    if [[ ${chooseDatabase} == "mysql" ]]; then
        importMysql
    elif [[ ${chooseDatabase} == "mongo" ]]; then
...
function processStart() {
    ...
    echo "mysql脚本请放置在：/data/script/mysql/ 下"
    echo "mongo脚本请放置在：/data/script/mongo/ 下"
    echo "es脚本请放置在：/data/script/es/ 下"
