...
public class AlarmHistoryExportServiceImpl implements AlarmHistoryExportService {
    ...
    public Result exportAlarmList(ExportDto exportDto) {
        ...
        try {
            ...
            fe.printStackTrace();
            return ResultUtils.warn(AlarmHistoryResultCode.EXPORT_NO_DATA,
                    I18nUtils.getSystemString(AppConstant.EXPORT_NO_DATA));
        } catch (FilinkExportDataTooLargeException fe) {
            return getExportToLargeMsg(fe);
        } catch (FilinkExportTaskNumTooBigException fe) {
            fe.printStackTrace();
            return ResultUtils.warn(AlarmHistoryResultCode.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS,
                    I18nUtils.getSystemString(AppConstant.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS));
        } catch (Exception e) {
            e.printStackTrace();
            return ResultUtils.warn(AlarmHistoryResultCode.FAILED_TO_CREATE_EXPORT_TASK,
    ...
    private Result getExportToLargeMsg(FilinkExportDataTooLargeException fe) {
        ...
        return ResultUtils.warn(AlarmHistoryResultCode.EXPORT_DATA_TOO_LARGE, msg);
    }
