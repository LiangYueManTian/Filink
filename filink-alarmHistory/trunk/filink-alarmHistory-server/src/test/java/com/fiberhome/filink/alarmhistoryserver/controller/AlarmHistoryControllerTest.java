...
//    }
    ...
    public void queryAlarmHistoryListTest() {
        new Expectations() {
            {
                alarmHistoryService.queryAlarmHistoryList((QueryCondition<AlarmHistoryEs>) any);
                result = new PageBean();
            }
        };
        QueryCondition<AlarmHistoryEs> queryCondition = new QueryCondition<>();
        Result result = alarmHistoryController.queryAlarmHistoryList(queryCondition);
        Assert.assertEquals(result.getCode(), 170318);
        queryCondition.setFilterConditions(new ArrayList<>());
        queryCondition.setPageCondition(new PageCondition());
        result = alarmHistoryController.queryAlarmHistoryList(queryCondition);
