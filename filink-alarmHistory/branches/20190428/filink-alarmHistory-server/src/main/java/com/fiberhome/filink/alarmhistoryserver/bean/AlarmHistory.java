...
public class AlarmHistory {
    ...
    public String getTranslationAlarmFixedLevel() {
        ...
        if (alarmFixedLevel != null) {
            if (AppConstant.YI.equals(this.alarmFixedLevel)) {
                alarmFixedLevelName = I18nUtils.getString(AppConstant.ALARM_LEVEL_NAME_ONE);
            } else if (AppConstant.TWO.equals(this.alarmFixedLevel)) {
                alarmFixedLevelName = I18nUtils.getString(AppConstant.ALARM_LEVEL_NAME_TWO);
            } else if (AppConstant.THREE.equals(this.alarmFixedLevel)) {
                alarmFixedLevelName = I18nUtils.getString(AppConstant.ALARM_LEVEL_NAME_THREE);
            } else if (AppConstant.FOUR.equals(this.alarmFixedLevel)) {
                alarmFixedLevelName = I18nUtils.getString(AppConstant.ALARM_LEVEL_NAME_FOUR);
            }
        }
    ...
    public String getTranslationAlarmSourceTypeId() {
        ...
        if (alarmSourceTypeId != null) {
            if (AppConstant.ALARM_DEVICE_ONE.equals(this.alarmSourceTypeId)) {
                alarmSourceTypeIdName = I18nUtils.getString(AppConstant.ALARM_DEVICE_NAME_ONE);
            } else if (AppConstant.ALARM_DEVICE_TWO.equals(this.alarmSourceTypeId)) {
                alarmSourceTypeIdName = I18nUtils.getString(AppConstant.ALARM_DEVICE_NAME_TWO);
            } else if (AppConstant.ALARM_DEVICE_THREE.equals(this.alarmSourceTypeId)) {
                alarmSourceTypeIdName = I18nUtils.getString(AppConstant.ALARM_DEVICE_NAME_THREE);
            } else if (AppConstant.ALARM_DEVICE_FOUR.equals(this.alarmSourceTypeId)) {
                alarmSourceTypeIdName = I18nUtils.getString(AppConstant.ALARM_DEVICE_NAME_FOUR);
            } else if (AppConstant.ALARM_DEVICE_FREE.equals(this.alarmSourceTypeId)) {
                alarmSourceTypeIdName = I18nUtils.getString(AppConstant.ALARM_DEVICE_NAME_FREE);
            }
        }
    ...
    public String getTranslationAlarmConfirmStatus() {
        return LogFunctionCodeConstant.ALARM_STATUS_ONE == alarmConfirmStatus
                ? I18nUtils.getString(AppConstant.ALARM_CONFIRM_NAME_TWO) : I18nUtils.getString(AppConstant.ALARM_CONFIRM_NAME_ONE);
    }
    ...
    public String getTranslationAlarmCleanStatus() {
        ...
        if (alarmCleanStatus != null) {
            if (LogFunctionCodeConstant.ALARM_STATUS_ONE == this.alarmCleanStatus) {
                alarmCleanStatusName = I18nUtils.getString(AppConstant.ALARM_CLEAR_NAME_ONE);
            } else if (LogFunctionCodeConstant.ALARM_STATUS_TWO == this.alarmCleanStatus) {
                alarmCleanStatusName = I18nUtils.getString(AppConstant.ALARM_CLEAR_NAME_THREE);
            } else if (LogFunctionCodeConstant.ALARM_STATUS_THREE == this.alarmCleanStatus) {
                alarmCleanStatusName = I18nUtils.getString(AppConstant.ALARM_CLEAR_NAME_TWO);
            }
        }
    ...
    public String getTranslationAlarmContinousTime() {
        ...
        if (time == 0) {
            continousTimeStr = '1' + I18nUtils.getString(AppConstant.HOUR);
        } else if (time >= 8760) {
            // 年
            continousTimeStr = Math.toIntExact(time / 8760) + I18nUtils.getString(AppConstant.YEAR)
                    + (int) Math.ceil(time % 8760 / 720) + I18nUtils.getString(AppConstant.MONTH);
        } else if (time >= 720) {
            // 月
            continousTimeStr = Math.toIntExact(time / 720) + I18nUtils.getString(AppConstant.MONTH)
                    + (int) Math.ceil(time % 720 / 24) + I18nUtils.getString(AppConstant.DAY);
        } else if (24 <= time) {
            // 天
            continousTimeStr = Math.toIntExact(time / 24) + I18nUtils.getString(AppConstant.DAY) + (int) Math.ceil(time % 24)
                    + I18nUtils.getString(AppConstant.HOUR);
        } else {
            // 时
            continousTimeStr = (int) Math.ceil(time) + I18nUtils.getString(AppConstant.HOUR);
        }
        return continousTimeStr;
    }
