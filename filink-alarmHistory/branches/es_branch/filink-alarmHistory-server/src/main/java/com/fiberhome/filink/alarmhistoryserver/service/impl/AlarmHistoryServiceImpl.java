...
public class AlarmHistoryServiceImpl extends ServiceImpl<AlarmHistoryDao, AlarmHistory> implements AlarmHistoryService {
    ...
    public Result insertAlarmHistoryFeign(AlarmHistory alarmHistory) {
        ...
        try {
            ...
            log.error("insert elasticsearch error", e);
        }
        return ResultUtils.success(ResultCode.SUCCESS);
    }
    /**
    ...
    private boolean deleteEsByDeviceIds(List<String> deviceIds) {
        ...
        try {
            ...
            log.info("花费时间：{},是否超时：{},总文档数：{},更新数：{},删除数：{},批量次数：{},跳过数：{},冲突数：{}---",
                    timeTaken, timedOut, totalDocs, updatedDocs, deletedDocs, batches, noops, versionConflicts);
            log.info("花费时间：" + timeTaken + ",是否超时：" + timedOut + ",总文档数：" + totalDocs + ",更新数：" +
                    updatedDocs + ",删除数：" + deletedDocs + ",批量次数：" + batches + ",跳过数：" + noops + ",冲突数：" + versionConflicts);
            return true;
        } catch (Exception e) {
            log.error("删除设施：删除历史告警--删除es数据报错", e);
            return false;
        }
    }
    ...
    public Integer deleteAlarmHistory(List<String> deviceIds) {
        ...
        if (!deleteEs) {
            log.warn("删除设施：删除历史告警--删除es数据报错,设备ID: {}", devices);
            return null;
        }
        log.info("删除设施：删除历史告警--删除es数据完成,设备ID: {}", devices);
        //删除mongoDb数据
