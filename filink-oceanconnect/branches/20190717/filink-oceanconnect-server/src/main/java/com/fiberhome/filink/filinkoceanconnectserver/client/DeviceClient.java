...
public class DeviceClient {
    ...
    public StreamClosedHttpResponse registryDevice(OceanDevice oceanDevice) {
        log.info("registry device to ocean connect : " + JSONObject.toJSONString(oceanDevice));
        try {
            String appId = oceanDevice.getAppId();
            //获取token
            String accessToken = authClient.getToken(appId);
            log.info("registry device token : " + accessToken);
            //注册地址
            ...
            log.info("device registry response: " + response.getContent());
            return response;
        } catch (Exception e) {
            e.printStackTrace();
            log.error("registry device failed>>> : " + oceanDevice.getDeviceId());
        }
        return null;
    }
    ...
    public StreamClosedHttpResponse deleteDeviceById(OceanDevice oceanDevice) {
        ...
        try {
            ...
            StreamClosedHttpResponse response = httpsClient.doDeleteWithParasGetStatusLine(urlDeleteDirectConnectedDevice,
                    null, header);
            log.info("delete device response : " + response.getContent());
            return response;
        } catch (Exception e) {
            e.printStackTrace();
            log.error("delete device failed>>> : " + deviceId);
        }
        return null;
    }
    ...
    public StreamClosedHttpResponse modifyDeviceById(OceanDevice oceanDevice) {
        ...
        try {
            ...
            log.info("modify device response : " + response.getContent());
            return response;
        } catch (Exception e) {
            e.printStackTrace();
            log.error("modify device failed>>>>>>>>>>>" + deviceId);
        }
        return null;
    }
    ...
    public void subscribe(String appId) {
        ...
        //回调地址
        String callBackUrl = OceanConnectUrl.REQUEST_HEADER + OceanParamsKey.LOCAL_ADDRESS + OceanConnectUrl.CALLBACK_RECEIVE;
        //获取token
        String accessToken = authClient.getToken(appId);
        //获取订阅类型
        ...
        for (String serviceNotifyType : serviceNotifyTypes) {
            ...
            log.info("SubscribeServiceNotification, notifyType:" + serviceNotifyType + ", callbackurl:" + callBackUrl + ", response content:");
            log.info(bodySubscribe);
        }
    }
}
