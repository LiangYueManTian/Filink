...
public class FiLinkOceanConnectRequestResolver implements RequestResolver {
    ...
    public String resolveUdpReq(AbstractReqParams udpParams) {
        ...
        try {
            fiLinkReqOceanConnectParams = (FiLinkReqOceanConnectParams) udpParams;
            //从redis中获取协议信息
            AbstractProtocolBean protocolBean = fiLinkReqOceanConnectParams.getProtocolBean();
        ...
        try {
            //生成data
            dataBuf = getDataBuf(fiLinkReqOceanConnectParams, fiLinkProtocolBean);
        } catch (Exception e) {
            e.printStackTrace();
        ...
        //生成公共头参数
        FiLinkCommonHeader commonHeader = setHeader(fiLinkReqOceanConnectParams, dataBuf.readableBytes(), commonHeaderList);
        //写入公共头信息
     ...
     * @return 公共头信息
     */
    private FiLinkCommonHeader setHeader(FiLinkReqOceanConnectParams fiLinkReqOceanConnectParams, int dataLength, List<HeaderParam> headerParams) {
        Integer cmdType = fiLinkReqOceanConnectParams.getCmdType();
     ...
     * @return ByteBuf
     */
    private ByteBuf getDataBuf(FiLinkReqOceanConnectParams fiLinkReqOceanConnectParams, FiLinkProtocolBean fiLinkProtocolXmlBean) throws Exception {
    ...
    private String getDataCodeByKey(String key, Map<String, Choose> chooseMap) {
        ...
        logger.error("there is no code mapped to the key: " + key);
