 ...
 */
@Slf4j
@Component("FiLinkOceanConnectWellSender")
public class FiLinkOceanConnectWellSender extends AbstractInstructSender {
    ...
    protected String getReqData(AbstractReqParams abstractReqParams) {
        ...
        if (CmdId.UNLOCK.equals(cmdId)) {
            ...
            //将开锁命令存入缓存，设备响应时重发
            RedisUtils.hSet(equipmentId + RedisKey.UNLOCK_CMD_RESEND_BUFFER, key, dataMap);
        } else if (CmdId.DEPLOY_STATUS.equals(cmdId)) {
            Map<String, Object> params = fiLinkReqOceanConnectParams.getParams();
            String deployStatus = (String) params.get(ParamsKey.DEPLOY_STATUS);
            dataMap.put(ParamsKey.DEPLOY_STATUS, deployStatus);
            RedisUtils.hSet(RedisKey.CMD_RESEND_BUFFER, key, dataMap);
            //部署状态存入缓存,每次设备激活进行重发
            RedisUtils.hSet(RedisKey.DEPLOY_CMD, equipmentId, dataMap);
        } else if (CmdId.SET_CONFIG.equals(cmdId)) {
            RedisUtils.hSet(RedisKey.CMD_RESEND_BUFFER, key, dataMap);
            RedisUtils.hSet(RedisKey.SET_CONFIG_CMD, equipmentId, dataMap);
        } else {
            RedisUtils.hSet(RedisKey.CMD_RESEND_BUFFER, key, dataMap);
        }
        return hexData;
