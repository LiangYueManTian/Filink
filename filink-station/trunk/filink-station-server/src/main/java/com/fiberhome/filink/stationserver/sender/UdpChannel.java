...
public class UdpChannel {
    ...
    public void send(String equipmentId, String data) {
        try {
            log.info("[netty] : udp channel send data {} of the control {}", data, equipmentId);
            DeviceMsg deviceMsg = (DeviceMsg) RedisUtils.hGet(RedisKey.DEVICE_KEY, equipmentId);
            if (deviceMsg == null) {
                log.info("[netty] : deviceMsg of the control {} redis is null", equipmentId);
            ...
            log.error("[netty] : udp channel send failed: {}", e);
        }
    }
}
