...
public class FiLinkAuthenticationFailureHandler extends ExceptionMappingAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest httpServletRequest,
        ...
        String phoneNumber = httpServletRequest.getParameter(UserConst.LOGIN_BY_PHONE);
        if (StringUtils.isNotEmpty(phoneNumber)) {
            httpServletResponse.getWriter().write(JSONObject.toJSONString(ResultUtils.success(
                    ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(I18Const.VERIFICATION_CODE_ERROR)))));
            return ;
        }
        ...
        switch (failFlag) {
            case UserConst.USER_HAS_LOCKED:
                httpServletResponse.getWriter().write(JSONObject.toJSONString(ResultUtils.success(
                        ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(I18Const.USER_HAS_LOCKED)))));
                break;
            case UserConst.USER_HAS_FORBIDDEN:
                httpServletResponse.getWriter().write(JSONObject.toJSONString(ResultUtils.success(
                        ResultUtils.warn(ResultCode.FAIL, I18nUtils.getString(I18Const.USER_HAS_FORBIDDEN)))));
                break;
            default:
                log.info("userparameter {}", userParameter);
    ...
    private Integer loginFailInfo(User getUser) {
        ...
        if (!ObjectUtils.isEmpty(unlockTime)) {
            ...
            if (unlockTime >= currentTime) {
                return UserConst.USER_HAS_LOCKED;
            }
        } else if (UserConst.USER_STOP_STATUS.equals(getUser.getUserStatus())) {
            return UserConst.USER_HAS_FORBIDDEN;
        }
