...
public abstract class AbstractExport {
    ...
    public void exportData(ExportRequestInfo exportRequestInfo) {
        Export export = exportRequestInfo.getExport();
        Map<String, Object> map = exportRequestInfo.getMap();
     ...
     * @return
     */
    public ExportRequestInfo insertTask(ExportDto exportDto, String serverName, String listName) {
        //存入操作用户
        exportDto.setUserId(RequestInfoUtils.getUserId());
        Map<String, Object> map = new HashMap(5);
        //存入时区信息
        map.put(ExportApiConstant.TIME_ZONE, ExportApiUtils.getZone());
        map.put(ExportApiConstant.USER_ID, exportDto.getUserId());
        map.put(ExportApiConstant.LOCALES, RequestInfoUtils.getLanguage());
        map.put(ExportApiConstant.OBJECT_LIST, exportDto.getObjectList());
        String token = RequestInfoUtils.getToken();
        map.put(ExportApiConstant.TOKEN, token);
        ExportApiUtils.RESOURCE.set(map);
        ...
        exportRequestInfo.setExport(export);
        exportRequestInfo.setMap(map);
