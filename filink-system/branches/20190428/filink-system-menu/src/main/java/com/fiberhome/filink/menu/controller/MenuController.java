...
public class MenuController {
    ...
    public Result queryMenuTemplateNameIsExists(@RequestBody MenuTemplate menuTemplate) {
        if (StringUtils.isEmpty(menuTemplate.getTemplateName())) {
            return ResultUtils.warn(MenuResultCodeConstant.MENU_NAME_NULL, I18nUtils.getString(MenuI18nConstant.MENU_NAME_NULL));
        }
    ...
    public Result deleteMenuTemplate(@RequestBody List<String> menuTemplateIds) {
        if (menuTemplateIds.size() == 0) {
            return ResultUtils.warn(MenuResultCodeConstant.PARAM_NULL, I18nUtils.getString(MenuI18nConstant.SELECT_THE_MENU_TEMPLATE));
        }
        try {
