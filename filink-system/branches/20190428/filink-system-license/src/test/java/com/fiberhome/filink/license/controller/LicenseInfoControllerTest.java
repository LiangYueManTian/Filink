...
public class LicenseInfoControllerTest {
    ...
    @Test
    public void uploadLicenseForAdmin() throws Exception {
        MultipartFile mockFile = mock(MultipartFile.class);
        when(licenseInfoService.uploadLicense(mockFile)).thenReturn(ResultUtils.success());
        Result result = licenseInfoController.uploadLicenseForAdmin(mockFile);
        Assert.assertTrue(result.getCode() == ResultUtils.success().getCode());
    }
    ...
    public void getCurrentLicense() {
        ...
        when(licenseInfoService.getCurrentLicense()).thenReturn(license);
        License currentLicense = licenseInfoController.getCurrentLicense();
        Assert.assertEquals(license, currentLicense);
    }

    @Test
    public void validateLicenseTime() throws Exception {
        when(licenseInfoService.validateLicenseTime()).thenReturn(true);
        Result result = licenseInfoController.validateLicenseTime();
        Assert.assertTrue(result.getCode() == ResultUtils.success().getCode());
    }
