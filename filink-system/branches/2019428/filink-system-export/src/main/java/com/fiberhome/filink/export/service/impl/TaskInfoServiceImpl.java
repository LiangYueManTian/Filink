...
import com.fiberhome.filink.export.bean.*;
...
import com.fiberhome.filink.export.utils.ExportResultCode;
...
public class TaskInfoServiceImpl extends ServiceImpl<TaskInfoDao, TaskInfo> implements TaskInfoService {
    ...
    public Result addTask(ExportDto exportDto) {
        ...
        if (i == maxTaskNum || i > maxTaskNum) {
            return ResultUtils.warn(ExportResultCode.EXCEED_THE_MAXIMUM_NUMBER_OF_TASKS);
        }
        //任务id为null 新增任务
        if (StringUtils.isEmpty(taskId)) {
            ...
            if (taskInfo == null) {
                return ResultUtils.warn(ExportResultCode.TASK_DOES_NOT_EXIST, I18nUtils.getString(ExportI18n.TASK_DOES_NOT_EXIST));
            }
            taskInfo.setTaskStatus(TaskStatusConstant.READY);
    ...
    public Result stopTask(String taskId) {
        ...
        if (taskInfo == null || ONE.equals(taskInfo.getIsDeleted())) {
            return ResultUtils.warn(ExportResultCode.TASK_DOES_NOT_EXIST, I18nUtils.getString(ExportI18n.TASK_DOES_NOT_EXIST));
        }
        ...
        if (1 == integer) {
            ...
            return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(ExportI18n.STOP_MISSION_SUCCESSFUL));
        }
        throw new FilinkExportDateBaseException();
    }
    ...
    public Result exportTaskList(QueryCondition queryCondition) {
        ...
        if (filterConditions == null) {
            return ResultUtils.warn(ExportResultCode.PARAM_NULL, I18nUtils.getString(ExportI18n.PARAM_NULL));
        }
        ...
        PageBean pageBean = myBatiesBuildPageBean(page, count, taskInfoList);
    ...
    public Result deleteTask(List<String> taskIdList) {
        ...
        if (taskInfoList == null || taskInfoList.size() == 0) {
            throw new FilinkExportDirtyDataException(I18nUtils.getString(ExportI18n.TASK_DOES_NOT_EXIST));
        }
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(ExportI18n.DELETE_MISSION_SUCCESSFUL));
    }
    ...
    public Boolean updateTaskFileNumById(ExportDto exportDto) {
        ...
        if (integer == 1) {
            if (flag) {
                ...
                socketMessage.setMsg(I18nUtils.getString(ExportI18n.ONE_EXPORT_TASK_COMPLETED));
    ...
    public Boolean changeTaskStatusToUnusual(String taskId) {
        ...
        //如果库中数据已经删除，不做任何操作
        if (dbTaskInfo == null || Constant.DELETED.equals(dbTaskInfo.getIsDeleted())) {
            return true;
        }
        TaskInfo taskInfo = new TaskInfo();
        taskInfo.setTaskInfoId(taskId);
