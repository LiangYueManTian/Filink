...
public class IpRangeServiceImpl extends ServiceImpl<IpRangeDao, IpRange> implements IpRangeService {
    ...
    public Result addIpRange(IpRange ipRange) {
        ...
        return ResultUtils.success(SecurityStrategyResultCode.SUCCESS, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_ADD_SUCCESS));
    }
    ...
    public Result updateIpRange(IpRange ipRange) {
        ...
        return ResultUtils.success(SecurityStrategyResultCode.SUCCESS, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_UPDATE_SUCCESS));
    }
    ...
    public Result updateIpRangeStatus(List<String> rangeIds, String rangeStatus) {
        ...
        return ResultUtils.success(SecurityStrategyResultCode.SUCCESS, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_ENABLE_DISABLE_SUCCESS));
    }
    ...
    public Result updateAllRangesStatus(IpRange ipRange) {
        ...
        if (SecurityStrategyConstants.ONE_TYPE.equals(ipRange.getRangeStatus())) {
            //存入缓存
            setIpRangeRedis();
            addLog(SecurityStrategyConstants.RANGE_ID, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_ALL),  SecurityStrategyConstants.ENABLE_IP_RANGES_ALL_CODE, LogConstants.DATA_OPT_TYPE_UPDATE);
        } else {
            addLog(SecurityStrategyConstants.RANGE_ID, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_ALL), SecurityStrategyConstants.DISABLE_IP_RANGES_AL_CODE, LogConstants.DATA_OPT_TYPE_UPDATE);
        }
        return ResultUtils.success(SecurityStrategyResultCode.SUCCESS, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_ENABLE_DISABLE_SUCCESS));
    }
    ...
    public Result deleteRanges(List<String> rangeIds) {
        ...
        return ResultUtils.success(SecurityStrategyResultCode.SUCCESS, I18nUtils.getString(SecurityStrategyI18n.IP_RANGE_DELETE_SUCCESS));
    }
    ...
    private void checkIpv6RangeOverlap(IpRange ipRange, List<IpRange> ipRanges) {
        if (ipRanges.size() > 0) {
            for (IpRange ipDb : ipRanges) {
                //判断输入IP是否在已有范围内
                if (inIpv6Range(ipRange.getStartIp(), ipDb) || inIpv6Range(ipRange.getEndIp(), ipDb)) {
                    throw new FilinkIpRangeOverlapException(getMsg(SecurityStrategyI18n.IP_RANGE_OVERLAP_ERROR, ipDb));
                }
                //判断输入IP是否包含已有范围
                if (inIpv6Range(ipDb.getStartIp(), ipRange)) {
                    throw new FilinkIpRangeContainException(getMsg(SecurityStrategyI18n.IP_RANGE_CONTAIN_ERROR, ipDb));
                }
            }
        }
    }
    ...
    private void checkIpv4RangeOverlap(IpRange ipRange, List<IpRange> ipRanges) {
        if (ipRanges.size() > 0) {
            for (IpRange ipDb : ipRanges) {
                //判断输入IP是否在已有范围内
                if (inIpv4Range(ipRange.getStartIp(), ipDb) || inIpv4Range(ipRange.getEndIp(), ipDb)) {
                    throw new FilinkIpRangeOverlapException(getMsg(SecurityStrategyI18n.IP_RANGE_OVERLAP_ERROR, ipDb));
                }
                //判断输入IP是否包含已有范围
                if (inIpv4Range(ipDb.getStartIp(), ipRange)) {
                    throw new FilinkIpRangeContainException(getMsg(SecurityStrategyI18n.IP_RANGE_CONTAIN_ERROR, ipDb));
                }
            }
        }
    ...
    private String getMsg(String i18nStr, IpRange ipRange) {
        //获取国际化信息
        String msg = I18nUtils.getString(i18nStr);
        //替换起始IP
