...
import com.fiberhome.filink.securitystrategy.bean.*;
...
public class SecurityStrategyServiceImpl implements SecurityStrategyService {
    ...
    public Result updateAccountStrategy(AccountParam accountParam) {
        //校验参数是否启用，启用则校验对应参数，禁用则置空对应参数
        changeAccountStrategy(accountParam.getAccountSecurityStrategy());
        //转成JSON
    ...
    /**
     * 校验参数策略是否启用，启用则校验对应参数，禁用则置空对应参数
     * @param accountSecurityStrategy 账号安全策略
     */
    private void changeAccountStrategy(AccountSecurityStrategy accountSecurityStrategy) {
        //账号锁定策略是否启用
        if (SecurityStrategyConstants.ONE_TYPE.equals(accountSecurityStrategy.getLockStrategy())) {
            //校验账号锁定时间
            if (accountSecurityStrategy.checkLockedTime()) {
                throw new FilinkSecurityStrategyParamException();
            }
        } else {
            //账号锁定时间置空
            accountSecurityStrategy.setLockedTime(null);
        }
        //账号禁用策略是否启用
        if (SecurityStrategyConstants.ONE_TYPE.equals(accountSecurityStrategy.getForbidStrategy())) {
            //校验账号连续未登录时间
            if (accountSecurityStrategy.checkNoLoginTime()) {
                throw new FilinkSecurityStrategyParamException();
            }
        } else {
            //账号连续未登录时间置空
            accountSecurityStrategy.setNoLoginTime(null);
        }
    }
