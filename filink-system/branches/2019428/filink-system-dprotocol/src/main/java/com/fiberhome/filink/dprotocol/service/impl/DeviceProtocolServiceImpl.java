...
public class DeviceProtocolServiceImpl extends ServiceImpl<DeviceProtocolDao, DeviceProtocol> implements DeviceProtocolService {
    ...
    public Result addDeviceProtocol(String protocolName, MultipartFile file){
        ...
        //远程调用解析数据存入Redis缓存
        if (!stationFeign.addProtocol(getProtocolDto(file))) {
        ...
        //新增设施协议
        deviceProtocolDao.addDeviceProtocol(deviceProtocol);
        //记录日志
    ...
    public Result deleteDeviceProtocol(List<String> protocolIds) {
        ...
        //数据库逻辑删除
        deviceProtocolDao.batchDeleteDeviceProtocolList(deviceProtocols, updateUser);
        //修改Redis缓存
        List<ProtocolDto> protocols = new ArrayList<>();
        for (DeviceProtocol protocol : deviceProtocolDbs) {
       ...
       if (!stationFeign.deleteProtocol(protocols)) {
    ...
    public String queryProtocol(DeviceProtocol deviceProtocol) {
        ...
        try {
            ...
            filePath = HexUtil.bytesToHexString(bytes);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            //关闭流
