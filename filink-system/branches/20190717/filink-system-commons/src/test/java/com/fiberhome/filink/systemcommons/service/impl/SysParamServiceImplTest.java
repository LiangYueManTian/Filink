...
public class SysParamServiceImplTest {
    ...
    public void querySystemLanguageTest() {
        ...
        sysParam.setPresentValue("{\"screenDisplay\":\"0\",\"screenScroll\":\"0\",\"screenScrollTime\":10,\"systemLanguage\":\"\",\"systemLogo\":\"local\",\"timeType\":\"local\"}");
        new Expectations(RedisUtils.class) {
            {
                RedisUtils.get(languageKey);
                result = null;

                RedisUtils.hasKey("DISPLAY_SETTINGS");
                result = true;

                RedisUtils.get("DISPLAY_SETTINGS");
                result = sysParam;
            }
        };
        String result = sysParamService.querySystemLanguage();
        Assert.assertNull(result);
        sysParam.setPresentValue("{\"screenDisplay\":\"0\",\"screenScroll\":\"0\",\"screenScrollTime\":10,\"systemLanguage\":\"CN\",\"systemLogo\":\"local\",\"timeType\":\"local\"}");
        new Expectations(RedisUtils.class) {
            {
                RedisUtils.set(languageKey, anyString);
                result = null;
            }
        };
        result = sysParamService.querySystemLanguage();
