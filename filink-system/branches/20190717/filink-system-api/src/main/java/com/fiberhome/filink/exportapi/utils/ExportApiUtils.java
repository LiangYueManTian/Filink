...
public class ExportApiUtils {
     ...
     */
    public static ThreadLocal <Map<String, Object>> RESOURCE = new ThreadLocal<>();
    ...
    public static String getZoneTime(Long time) {
        ...
        String timeZone = map.get(ExportApiConstant.TIME_ZONE).toString();
        if (timeZone == null) {
            timeZone = "GMT+08:00";
        }
    ...
    public static String getCurrentUserId() {
        ...
        return map.get(ExportApiConstant.USER_ID).toString();
    }
    /**
     * 获取当前用户token
     *
    ...
    public static String getCurrentUserToken() {
        ...
        return map.get(ExportApiConstant.TOKEN).toString();
    }
    /**
     * 获取语言环境
     *
     * @return
     */
    public static String getExportLocales() {
        Map<String, Object> map = RESOURCE.get();
        if (map == null) {
            log.error("Locales is null >>>");
            return null;
        }
        return map.get(ExportApiConstant.LOCALES).toString();
    }
    ...
    public static Object getObjectList() {
        ...
        return map.get(ExportApiConstant.OBJECT_LIST);
    }

    /**
     * 根据QueryCondition 截取list
     *
     * @return
     */
    public static List<Object> getObjList(QueryCondition condition) {
        ...
        if (pageSize * pageNum > objectList.size()) {
            objectList = objectList.subList((pageNum - 1) * pageSize, objectList.size());
        } else {
            objectList = objectList.subList((pageNum - 1) * pageSize, pageSize * pageNum);
        }
        return objectList;
    }
}
