...
public class AccountSecurityStrategy {
    ...
    public boolean checkValue() {
        if (StringUtils.isEmpty(lockStrategy) || StringUtils.isEmpty(forbidStrategy)) {
            return true;
        }
        if (checkIntegerValue()) {
            return true;
        }
        return !lockStrategy.matches(SecurityStrategyConstants.ONE_ZERO_REGEX)
                || !forbidStrategy.matches(SecurityStrategyConstants.ONE_ZERO_REGEX);
    }
    /**
     * 校验数值是否符合规范
     * @return true不符合 false符合
