...
public class MenuServiceImpl implements MenuService {
     ...
     * 状态常量，用于查询map
     */
    private static final String TEMPLATE_STATUS = "template_status";
    /**
     * 状态常量，用于查询map
     */
    private static final String MENU_TEMPLATE_ID = "menu_template_id";
    ...
    public Result queryMenuTemplateNameIsExists(MenuTemplate menuTemplate) {
        ...
        if (b) {
            return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(MenuI18n.NAME_IS_AVAILABLE));
        }
        return ResultUtils.warn(MenuRusultCode.NAME_IS_EXIST, I18nUtils.getString(MenuI18n.NAME_IS_EXIST));
    }
     ...
     * @return 增加结果
     */
    @AddLogAnnotation(value = "add", logType = "1", functionCode = "2101101", dataGetColumnName = "templateName", dataGetColumnId = "menuTemplateId")
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result addMenuTemplate(MenuTemplateAndMenuInfoTree menuTemplateAndMenuInfoTree) {
        ...
        return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(MenuI18n.INCREASE_DATA_SUCCESS));
    }
    ...
    public Result getMenuTemplateByMenuTemplateId(String menuTemplateId) {
        ...
        if (menuTemplate == null || ONE.equals(menuTemplate.getIsDeleted())) {
            return ResultUtils.warn(MenuRusultCode.MENU_TEMPLATE_HAS_BEEN_DELETED, I18nUtils.getString(MenuI18n.MENU_TEMPLATE_HAS_BEEN_DELETED));
        }
        //创建返回实体
     ...
     * @return 修改结果
     */
    @AddLogAnnotation(value = "update", logType = "1", functionCode = "2101102", dataGetColumnName = "templateName", dataGetColumnId = "menuTemplateId")
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result updateMenuTemplate(MenuTemplateAndMenuInfoTree menuTemplateAndMenuInfoTree) {
        ...
        if (!version.equals(dBVersion)) {
            return ResultUtils.warn(MenuRusultCode.INCONSISTENT_VERSION, I18nUtils.getString(MenuI18n.INCONSISTENT_VERSION));
        }
        //查询当前模板启用状态
        ...
        if (isUsing) {
            ...
            if (ZERO.equals(templateStatus2)) {
                return ResultUtils.warn(MenuRusultCode.MENU_TEMPLATE_IS_OPEN, I18nUtils.getString(MenuI18n.TEMPLATE_IN_USE));
            }
        }
        //构建模板表修改实体
            ...
            if (b2) {
                ...
                if (b1) {
                    if (isUsing) {
                        ...
                        String string = I18nUtils.getString(MenuI18n.USED_MENU_TEMPLATE_UPDATE);
                        socketMessage.setMsg(string);
                    ...
                    return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(MenuI18n.UPDATE_SUCCESS));
                }
            }
        }
        return ResultUtils.warn(MenuRusultCode.DIRTY_DATA, I18nUtils.getString(MenuI18n.DIRTY_DATA));
    ...
    public Result deleteMenuTemplate(List<String> menuTemplateIds) {
        ...
        if (dbMenuTemplateList == null || dbMenuTemplateList.size() != menuTemplateIds.size()) {
            return ResultUtils.warn(MenuRusultCode.DELETED_MENU_TEMPLATE_EXISTS, I18nUtils.getString(MenuI18n.DELETED_MENU_TEMPLATE_EXISTS));
        }
        ...
        if (!menuTemplateIsOpen) {
            return ResultUtils.warn(MenuRusultCode.MENU_TEMPLATE_IS_OPEN, I18nUtils.getString(MenuI18n.MENU_TEMPLATE_IS_OPEN));
        }
        //默认结果
        boolean flag = true;
        //遍历模板信息 查询关联信息
        ...
        if (flag && b) {
            ...
            return ResultUtils.success(ResultCode.SUCCESS, I18nUtils.getString(MenuI18n.SUCCESSFULLY_DELETED));
            //如果失败 全部回滚
        } else {
    ...
    private boolean findChild(List<MenuInfoTree> menuInfoTreeList, String menuTemplateId, List<MenuRelation> menuRelations) {
        ...
        for (MenuInfoTree menuInfoTree : menuInfoTreeList) {
            ...
            //生成菜单关系id并保存
            String uuid = UUID.randomUUID().toString().replaceAll("-", "");
            menuRelation.setMenuRelationId(uuid);
            //保存关系实体中对应的模板表Id
    ...
    private boolean insertMenuRelation(List<MenuRelation> menuRelations) {
        ...
        //判断传入的menuid个数与所对应的数据库中数据量是否相等
        int num = menuInfoService.selectCountByIds(menuInfoIds);
        if (menuRelations.size() != num) {
            return false;
        }
        //添加菜单关系信息到数据库
    ...
    private void addLogByMenuTemplates(List<MenuTemplate> menuTemplateList) {
        ...
        for (MenuTemplate menuTemplate : menuTemplateList) {
            ...
            addLogBean.setFunctionCode("2101103");
            //获得操作对象id
